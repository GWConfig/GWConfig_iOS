...
- (UIButton *)torchBtn {
    if (!_torchBtn) {
        ...
        [_torchBtn setBackgroundImage:LOADICON(@"MKCommureApp", @"MKCMQRToolBar", @"cm_scan_torch.png") forState:(UIControlStateNormal)];
...
- (void)torchBtn_action:(UIButton *)btn {
    if (btn.selected) {
        btn.selected = NO;
        [btn setBackgroundImage:LOADICON(@"MKCommureApp", @"MKCMQRToolBar", @"cm_scan_torch.png") forState:(UIControlStateNormal)];
        [SGTorch turnOffTorch];
        self.torchLab.text = @"Tap to turn light on";
    } else {
        btn.selected = YES;
        [btn setBackgroundImage:LOADICON(@"MKCommureApp", @"MKCMQRToolBar", @"cm_scan_torch_hl.png") forState:(UIControlStateNormal)];
...
- (UIImageView *)qrcodeImgView {
    if (!_qrcodeImgView) {
        ...
        _qrcodeImgView.image = [UIImage imageNamed:@"cm_scan_mine_qrcode"];
    }
...
- (UIImageView *)albumImgView {
    if (!_albumImgView) {
        ...
        _albumImgView.image = [UIImage imageNamed:@"cm_scan_album"];
    }
