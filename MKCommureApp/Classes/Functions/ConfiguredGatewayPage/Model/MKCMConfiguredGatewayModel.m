...
@interface MKCMConfiguredGatewayModel ()
...
- (void)dealloc {
    NSLog(@"MKCMConfiguredGatewayModel销毁");
    ...
    dispatch_async(self.readQueue, ^{
        ...
        if (status == -1) {
            moko_dispatch_main_safe(^{
                if (block) {
                    block(self.macAddress,cm_ConfiguredGatewayStatus_timeout);
                }
            })
            return;
        }
        if (status == 1) {
            moko_dispatch_main_safe(^{
                if (block) {
                    block(self.macAddress,cm_ConfiguredGatewayStatus_upgrading);
                }
            })
            return;
        }
...
- (void)receiveOTAResult:(NSNotification *)note {
    ...
    if (result == 1) {
        //成功
        if (self.otaBlock) {
            self.otaBlock(self.macAddress,cm_ConfiguredGatewayStatus_success);
        }
    }else {
        if (self.otaBlock) {
            self.otaBlock(self.macAddress,cm_ConfiguredGatewayStatus_failed);
        }
    }
}
