...
@implementation MKCMDeviceListCell

+ (MKCMDeviceListCell *)initCellWithTableView:(UITableView *)tableView {
    MKCMDeviceListCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCMDeviceListCellIdenty"];
    if (!cell) {
        cell = [[MKCMDeviceListCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCMDeviceListCellIdenty"];
    }
    return cell;
}
...
- (void)longPressEventAction:(UILongPressGestureRecognizer *)longPress {
    if (longPress.state == UIGestureRecognizerStateBegan && [self.delegate respondsToSelector:@selector(cm_cellDeleteButtonPressed:)]) {
        [self.delegate cm_cellDeleteButtonPressed:self.indexPath.row];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKCMDeviceListModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCMDeviceModel.class]) {
        return;
    }
    self.deviceNameLabel.text = SafeStr(_dataModel.deviceName);
    self.stateLabel.text = (_dataModel.onLineState == MKCMDeviceModelStateOnline ? @"Online" : @"Offline");
    self.stateLabel.textColor = (_dataModel.onLineState == MKCMDeviceModelStateOnline ? NAVBAR_COLOR_MACROS : UIColorFromRGB(0xcccccc));
    self.macLabel.text = SafeStr(_dataModel.macAddress);
    if (dataModel.onLineState == MKCMDeviceModelStateOffline) {
        //设备离线
        self.wifiIcon.image = LOADICON(@"MKCommureApp", @"MKCMDeviceListCell", @"cm_offline_wifi.png");
        return;
    }
    //设备在线
...
- (UIImageView *)wifiIcon {
    if (!_wifiIcon) {
        ...
        _wifiIcon.image = LOADICON(@"MKCommureApp", @"MKCMDeviceListCell", @"cm_offline_wifi.png");
    }
    return _wifiIcon;
}
...
- (UIImageView *)rightIcon {
    if (!_rightIcon) {
        ...
        _rightIcon.image = LOADICON(@"MKCommureApp", @"MKCMDeviceListCell", @"cm_goNextButton.png");
    }
    return _rightIcon;
}
