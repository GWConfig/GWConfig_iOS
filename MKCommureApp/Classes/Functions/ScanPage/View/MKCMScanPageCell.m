...
@implementation MKCMScanPageCell

+ (MKCMScanPageCell *)initCellWithTableView:(UITableView *)tableView {
    MKCMScanPageCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCMScanPageCellIdenty"];
    if (!cell) {
        cell = [[MKCMScanPageCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCMScanPageCellIdenty"];
    }
    return cell;
}
...
#pragma mark - setter
- (void)setDataModel:(MKCMScanPageModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCMScanPageModel.class]) {
        return;
    }
