...
@interface MKCMCommunicateModel ()
...
@implementation MKCMCommunicateModel
...
- (BOOL)readCommunicateTimeout {
    ...
    [MKCMMQTTInterface cm_readCommunicateTimeoutWithMacAddress:[MKCMDeviceModeManager shared].macAddress topic:[MKCMDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configCommunicateTimeout {
    __block BOOL success = NO;
    [MKCMMQTTInterface cm_configCommunicationTimeout:[self.timeout integerValue] macAddress:[MKCMDeviceModeManager shared].macAddress topic:[MKCMDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
