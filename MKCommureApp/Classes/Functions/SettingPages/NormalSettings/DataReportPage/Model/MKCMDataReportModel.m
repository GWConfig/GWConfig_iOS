...
@implementation MKCMDataReportModel
...
- (BOOL)readDataReportTimeout {
    ...
    [MKCMMQTTInterface cm_readDataReportTimeoutWithMacAddress:[MKCMDeviceModeManager shared].macAddress topic:[MKCMDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configDataReportTimeout {
    __block BOOL success = NO;
    [MKCMMQTTInterface cm_configDataReportTimeout:[self.timeout integerValue] macAddress:[MKCMDeviceModeManager shared].macAddress topic:[MKCMDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
