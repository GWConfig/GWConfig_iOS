...
@interface MKCMNTPServerController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCMNTPServerModel *dataModel;

@end

@implementation MKCMNTPServerController

- (void)dealloc {
    NSLog(@"MKCMNTPServerController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKCommureApp", @"MKCMNTPServerController", @"cm_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCMNTPServerModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCMNTPServerModel alloc] init];
    }
    return _dataModel;
}
