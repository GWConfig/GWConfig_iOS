//
//  MKCMMqttWifiSettingsController.m
...
@interface MKCMMqttWifiSettingsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCMMqttWifiSettingsModel *dataModel;

@end

@implementation MKCMMqttWifiSettingsController

- (void)dealloc {
    NSLog(@"MKCMMqttWifiSettingsController销毁");
}
            ...
            if (((self.dataModel.eapType == 0 || self.dataModel.eapType == 1) && self.dataModel.verifyServer) || self.dataModel.eapType == 2) {
                ...
                [[NSNotificationCenter defaultCenter] addObserver:self
                                                         selector:@selector(receiveUpdateEAPCerts:)
                                                             name:MKCMReceiveDeviceUpdateEapCertsResultNotification
                                                           object:nil];
                return;
            }
        }
...
- (NSInteger)numberOfRowsInSection:(NSInteger)section {
    ...
    if (section == 11) {
        //Country&Band
        return ([[MKCMDeviceModeManager shared].deviceType isEqualToString:@"20"] ? self.section11List.count : 0);
    }
    return 0;
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKCommureApp", @"MKCMMqttWifiSettingsController", @"cm_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCMMqttWifiSettingsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCMMqttWifiSettingsModel alloc] init];
    }
    return _dataModel;
}
