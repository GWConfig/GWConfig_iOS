...
@interface MKCMMqttServerSettingView ()
...
@implementation MKCMMqttServerSettingView
...
- (void)clearButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cm_mqtt_deviecSetting_fileButtonPressed:)]) {
        [self.delegate cm_mqtt_deviecSetting_fileButtonPressed:2];
    }
}

- (void)exportButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cm_mqtt_deviecSetting_fileButtonPressed:)]) {
        [self.delegate cm_mqtt_deviecSetting_fileButtonPressed:0];
    }
}

- (void)importButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cm_mqtt_deviecSetting_fileButtonPressed:)]) {
        [self.delegate cm_mqtt_deviecSetting_fileButtonPressed:1];
    }
}
