...
@interface MKCMMqttServerController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKCMMqttServerConfigFooterViewDelegate,
...
@property (nonatomic, strong)MKCMMqttServerModel *dataModel;

@property (nonatomic, strong)MKCMMqttServerConfigFooterView *sslParamsView;

@property (nonatomic, strong)MKCMMqttServerConfigFooterViewModel *sslParamsModel;

@property (nonatomic, strong)UIView *footerView;

@end

@implementation MKCMMqttServerController

- (void)dealloc {
    NSLog(@"MKCMMqttServerController销毁");
}
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveUpdateCerts:)
                                                 name:MKCMReceiveDeviceUpdateMqttCertsResultNotification
                                               object:nil];
}
...
- (void)cm_mqtt_serverForDevice_bottomButtonPressed:(NSInteger)index {
    if (index == 0) {
        //Export Demo File
...
- (void)cm_selectedServerParams:(NSString *)fileName {
    [MKCMExcelDataManager parseDeviceExcel:fileName
        ...
        MKCMMqttServerModel *model = [MKCMMqttServerModel mk_modelWithJSON:returnData];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKCommureApp", @"MKCMMqttServerController", @"cm_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCMMqttServerModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCMMqttServerModel alloc] init];
    }
    return _dataModel;
}
...
- (MKCMMqttServerConfigFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKCMMqttServerConfigFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 380.f)];
...
- (MKCMMqttServerConfigFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKCMMqttServerConfigFooterViewModel alloc] init];
    }
