//
//  MKCMMqttNetworkSettingForGWTController.m
...
@interface MKCMMqttNetworkSettingForGWTController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCMMqttNetworkSettingForGWTModel *dataModel;

@end

@implementation MKCMMqttNetworkSettingForGWTController

- (void)dealloc {
    NSLog(@"MKCMMqttNetworkSettingForGWTController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKCommureApp", @"MKCMMqttNetworkSettingForGWTController", @"cm_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCMMqttNetworkSettingForGWTModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCMMqttNetworkSettingForGWTModel alloc] init];
    }
    return _dataModel;
}
