//
//  MKCMMqttNetworkSettingsModel.m
...
@interface MKCMMqttNetworkSettingsModel ()
...
@implementation MKCMMqttNetworkSettingsModel
...
- (BOOL)configNetworkInfos {
    __block BOOL success = NO;
    [MKCMMQTTInterface cm_modifyWifiNetworkInfos:self macAddress:[MKCMDeviceModeManager shared].macAddress topic:[MKCMDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
