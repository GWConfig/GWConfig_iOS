...
@interface MKCMFilterByMacController ()<UITableViewDelegate,
UITableViewDataSource,
MKCMFilterEditSectionHeaderViewDelegate,
...
@property (nonatomic, strong)MKCMFilterByMacModel *dataModel;

@end

@implementation MKCMFilterByMacController

- (void)dealloc {
    NSLog(@"MKCMFilterByAdvNameController销毁");
}
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    if (section == 1) {
        MKCMFilterEditSectionHeaderViewModel *headerModel = [[MKCMFilterEditSectionHeaderViewModel alloc] init];
        headerModel.index = 0;
        headerModel.msg = @"Edit Mac Address";
        MKCMFilterEditSectionHeaderView *headerView = [MKCMFilterEditSectionHeaderView initHeaderViewWithTableView:tableView];
...
#pragma mark - MKCMFilterEditSectionHeaderViewDelegate
/// 加号点击事件
/// @param index 所在index
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKCommureApp", @"MKCMFilterByMacController", @"cm_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCMFilterByMacModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCMFilterByMacModel alloc] init];
    }
    return _dataModel;
}
