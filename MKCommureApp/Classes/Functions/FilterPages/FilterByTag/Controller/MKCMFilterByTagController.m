...
@interface MKCMFilterByTagController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCMFilterByTagModel *dataModel;

@end

@implementation MKCMFilterByTagController

- (void)dealloc {
    NSLog(@"MKCMFilterByTagController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKCommureApp", @"MKCMFilterByTagController", @"cm_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCMFilterByTagModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCMFilterByTagModel alloc] init];
    }
    return _dataModel;
}
