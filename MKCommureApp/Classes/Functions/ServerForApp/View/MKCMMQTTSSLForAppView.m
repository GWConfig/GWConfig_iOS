...
@implementation MKCMMQTTSSLForAppViewModel
@end

@interface MKCMMQTTSSLForAppView ()<MKMQTTSSLCertificateViewDelegate>
...
@implementation MKCMMQTTSSLForAppView
...
- (void)mk_fileSelectedButtonPressed:(NSInteger)index {
    if (index == 0) {
        //CA File
        if ([self.delegate respondsToSelector:@selector(cm_mqtt_sslParams_app_caFilePressed)]) {
            [self.delegate cm_mqtt_sslParams_app_caFilePressed];
        }
        return;
    }
    //P12证书
    if ([self.delegate respondsToSelector:@selector(cm_mqtt_sslParams_app_clientFilePressed)]) {
        [self.delegate cm_mqtt_sslParams_app_clientFilePressed];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(cm_mqtt_sslParams_app_sslStatusChanged:)]) {
        [self.delegate cm_mqtt_sslParams_app_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(cm_mqtt_sslParams_app_certificateChanged:)]) {
            [self.delegate cm_mqtt_sslParams_app_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKCMMQTTSSLForAppViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCMMQTTSSLForAppViewModel.class] || _dataModel.certificate < 0 || _dataModel.certificate > 2) {
        return;
    }
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"MKCommureApp", @"MKCMMQTTSSLForAppView", @"cm_switchSelectedIcon.png") : LOADICON(@"MKCommureApp", @"MKCMMQTTSSLForAppView", @"cm_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"MKCommureApp", @"MKCMMQTTSSLForAppView", @"cm_switchUnselectedIcon.png") forState:UIControlStateNormal];
