...
@implementation MKCMManageBleDevicesCellModel
@end

@interface MKCMManageBleDevicesCell ()
...
@implementation MKCMManageBleDevicesCell

+ (MKCMManageBleDevicesCell *)initCellWithTableView:(UITableView *)tableView {
    MKCMManageBleDevicesCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCMManageBleDevicesCellIdenty"];
    if (!cell) {
        cell = [[MKCMManageBleDevicesCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCMManageBleDevicesCellIdenty"];
    }
    return cell;
}
...
- (void)connectButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cm_manageBleDevicesCellConnectButtonPressed:typeCode:)]) {
        [self.delegate cm_manageBleDevicesCellConnectButtonPressed:SafeStr(self.dataModel.macAddress)
                                                          typeCode:self.dataModel.typeCode];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKCMManageBleDevicesCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCMManageBleDevicesCellModel.class]) {
        return;
    }
