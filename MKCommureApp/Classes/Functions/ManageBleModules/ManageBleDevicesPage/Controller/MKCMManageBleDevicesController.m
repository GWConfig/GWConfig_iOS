//
//  MKCMManageBleDevicesController.m
...
@interface MKCMManageBleDevicesController ()<UITableViewDelegate,
UITableViewDataSource,
MKCMManageBleDevicesSearchButtonDelegate,
MKCMManageBleDevicesCellDelegate>

@property (nonatomic, strong)MKBaseTableView *tableView;

@property (nonatomic, strong)MKCMManageBleDevicesSearchButtonModel *buttonModel;

@property (nonatomic, strong)MKCMManageBleDevicesSearchButton *searchButton;
...
@implementation MKCMManageBleDevicesController

- (void)dealloc {
    NSLog(@"MKCMManageBleDevicesController销毁");
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    MKCMManageBleDevicesCell *cell = [MKCMManageBleDevicesCell initCellWithTableView:tableView];
    cell.dataModel = self.dataList[indexPath.row];
    cell.delegate = self;
    return cell;
}

#pragma mark - MKCMManageBleDevicesSearchButtonDelegate
- (void)cm_scanSearchButtonMethod {
    [MKCMManageBleDeviceSearchView showSearchName:self.buttonModel.searchName
...
- (void)cm_scanSearchButtonClearMethod {
...
#pragma mark - MKCMManageBleDevicesCellDelegate
/// 用户点击了链接按钮
...
- (void)cm_manageBleDevicesCellConnectButtonPressed:(NSString *)macAddress typeCode:(NSInteger)typeCode; {
    ...
    [alertView showAlertWithTitle:@"Enter password" message:@"" notificationName:@"mk_cm_needDismissAlert"];
}
...
- (void)addNotifications {
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveDeviceDatas:)
                                                 name:MKCMReceiveDeviceDatasNotification
                                               object:nil];
}
...
- (void)updateCellModelWithMac:(NSString *)macAddress
    ...
    MKCMManageBleDevicesCellModel *cellModel = [[MKCMManageBleDevicesCellModel alloc] init];
...
- (void)connectBeaconWithPassword:(NSString *)password bleMac:(NSString *)bleMac {
    ...
    [MKCMMQTTInterface cm_connectBXPButtonWithPassword:password bleMac:bleMac macAddress:[MKCMDeviceModeManager shared].macAddress topic:[MKCMDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (MKCMManageBleDevicesSearchButton *)searchButton {
    if (!_searchButton) {
        _searchButton = [[MKCMManageBleDevicesSearchButton alloc] init];
        _searchButton.delegate = self;
    }
    return _searchButton;
}

- (MKCMManageBleDevicesSearchButtonModel *)buttonModel {
    if (!_buttonModel) {
        _buttonModel = [[MKCMManageBleDevicesSearchButtonModel alloc] init];
