...
@interface MKCMAccelerometerController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCMAccelerometerModel *dataModel;

@end

@implementation MKCMAccelerometerController

- (void)dealloc {
    NSLog(@"MKCMAccelerometerController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKCommureApp", @"MKCMAccelerometerController", @"cm_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCMAccelerometerModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCMAccelerometerModel alloc] init];
