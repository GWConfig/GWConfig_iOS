...
@interface MKCMBatchOtaTableHeader ()
...
@implementation MKCMBatchOtaTableHeader
...
- (MKTextField *)subTextField {
    if (!_subTextField) {
        ...
        _subTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(cm_topicValueChanged:type:)]) {
                [self.delegate cm_topicValueChanged:text type:0];
            }
        };
    }
...
- (MKTextField *)pubTextField {
    if (!_pubTextField) {
        ...
        _pubTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(cm_topicValueChanged:type:)]) {
                [self.delegate cm_topicValueChanged:text type:1];
            }
        };
    }
...
- (UIButton *)excelButton {
    if (!_excelButton) {
        ...
        [_excelButton setImage:LOADICON(@"MKCommureApp", @"MKCMBatchOtaTableHeader", @"cm_batchOtaListSelectIcon.png") forState:UIControlStateNormal];
...
- (UIButton *)scanButton {
    if (!_scanButton) {
        ...
        [_scanButton setImage:LOADICON(@"MKCommureApp", @"MKCMBatchOtaTableHeader", @"cm_batchOtaQRCodeIcon.png") forState:UIControlStateNormal];
