...
@implementation MKCMBleWifiSettingsCertCellModel
@end

@interface MKCMBleWifiSettingsCertCell ()
...
@implementation MKCMBleWifiSettingsCertCell

+ (MKCMBleWifiSettingsCertCell *)initCellWithTableView:(UITableView *)tableView {
    MKCMBleWifiSettingsCertCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCMBleWifiSettingsCertCellIdenty"];
    if (!cell) {
        cell = [[MKCMBleWifiSettingsCertCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCMBleWifiSettingsCertCellIdenty"];
    }
    return cell;
}
...
- (void)certificateButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cm_bleWifiSettingsCertPressed:)]) {
        [self.delegate cm_bleWifiSettingsCertPressed:self.dataModel.index];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKCMBleWifiSettingsCertCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCMBleWifiSettingsCertCellModel.class]) {
        return;
    }
...
- (UIButton *)certificateButton {
    if (!_certificateButton) {
        ...
        [_certificateButton setImage:LOADICON(@"MKCommureApp", @"MKCMBleWifiSettingsCertCell", @"cm_config_certAddIcon.png") forState:UIControlStateNormal];
