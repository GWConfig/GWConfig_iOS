//
//  MKCMBleNetworkSettingsController.m
...
@interface MKCMBleNetworkSettingsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCMBleNetworkSettingsModel *dataModel;

@end

@implementation MKCMBleNetworkSettingsController

- (void)dealloc {
    NSLog(@"MKCMBleNetworkSettingsController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKCommureApp", @"MKCMBleNetworkSettingsController", @"cm_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCMBleNetworkSettingsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCMBleNetworkSettingsModel alloc] init];
    }
    return _dataModel;
}
