...
- (BOOL)readDHCPStatus {
    ...
    [MKCMInterface cm_readDHCPStatusWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configDHCPStatus {
    __block BOOL success = NO;
    [MKCMInterface cm_configDHCPStatus:self.dhcp sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readIpAddress {
    __block BOOL success = NO;
    [MKCMInterface cm_readNetworkIpInfosWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configIpAddress {
    __block BOOL success = NO;
    [MKCMInterface cm_configIpAddress:self.ip
                                 mask:self.mask
