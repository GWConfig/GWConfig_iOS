...
@implementation MKCMMQTTSSLForDeviceViewModel
@end

@interface MKCMMQTTSSLForDeviceView ()<MKMQTTSSLCertificateViewDelegate>
...
@implementation MKCMMQTTSSLForDeviceView
...
- (void)mk_fileSelectedButtonPressed:(NSInteger)index {
    if (index == 0) {
        //CA File
        if ([self.delegate respondsToSelector:@selector(cm_mqtt_sslParams_device_caFilePressed)]) {
            [self.delegate cm_mqtt_sslParams_device_caFilePressed];
        }
        return;
    }
    if (index == 1) {
        //Client Key File
        if ([self.delegate respondsToSelector:@selector(cm_mqtt_sslParams_device_clientKeyPressed)]) {
            [self.delegate cm_mqtt_sslParams_device_clientKeyPressed];
        }
        return;
    }
    //Client Cert File
    if ([self.delegate respondsToSelector:@selector(cm_mqtt_sslParams_device_clientCertPressed)]) {
        [self.delegate cm_mqtt_sslParams_device_clientCertPressed];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(cm_mqtt_sslParams_device_sslStatusChanged:)]) {
        [self.delegate cm_mqtt_sslParams_device_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(cm_mqtt_sslParams_device_certificateChanged:)]) {
            [self.delegate cm_mqtt_sslParams_device_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKCMMQTTSSLForDeviceViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCMMQTTSSLForDeviceViewModel.class]) {
        return;
    }
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"MKCommureApp", @"MKCMMQTTSSLForDeviceView", @"cm_switchSelectedIcon.png") : LOADICON(@"MKCommureApp", @"MKCMMQTTSSLForDeviceView", @"cm_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"MKCommureApp", @"MKCMMQTTSSLForDeviceView", @"cm_switchUnselectedIcon.png") forState:UIControlStateNormal];
