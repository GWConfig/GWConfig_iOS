//
//  MKCMBleNTPTimezoneController.m
...
@interface MKCMBleNTPTimezoneController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCMBleNTPTimezoneModel *dataModel;

@property (nonatomic, strong)NSMutableArray *timezoneList;

@end

@implementation MKCMBleNTPTimezoneController

- (void)dealloc {
    NSLog(@"MKCMBleNTPTimezoneController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKCommureApp", @"MKCMBleNTPTimezoneController", @"cm_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCMBleNTPTimezoneModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCMBleNTPTimezoneModel alloc] init];
    }
    return _dataModel;
}
