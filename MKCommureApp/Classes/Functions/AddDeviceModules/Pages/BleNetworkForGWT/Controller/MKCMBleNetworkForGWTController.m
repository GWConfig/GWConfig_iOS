//
//  MKCMBleNetworkForGWTController.m
...
@interface MKCMBleNetworkForGWTController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCMBleNetworkForGWTModel *dataModel;

@end

@implementation MKCMBleNetworkForGWTController

- (void)dealloc {
    NSLog(@"MKCMBleNetworkForGWTController销毁");
}
...
- (void)mk_certSelectedMethod:(mk_certListPageType)certType certName:(NSString *)certName {
    ...
    if (certType == mk_clientKeySelPage) {
        ...
        MKCMBleWifiSettingsCertCellModel *cellModel = self.section11List[0];
        cellModel.fileName = certName;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKCommureApp", @"MKCMBleNetworkForGWTController", @"cm_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCMBleNetworkForGWTModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCMBleNetworkForGWTModel alloc] init];
    }
    return _dataModel;
}
