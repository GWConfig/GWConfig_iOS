...
@implementation MKCMAboutController

- (void)dealloc {
    NSLog(@"MKCMAboutController销毁");
}
...
- (void)feedButtonPressed {
    MKCMAppLogController *vc = [[MKCMAppLogController alloc] init];
...
- (UIImageView *)aboutIcon {
    if (!_aboutIcon) {
        ...
        _aboutIcon.image = LOADICON(@"MKCommureApp", @"MKCMAboutController", @"cm_deviceList_centerIcon.png");
    }
    return _aboutIcon;
}
