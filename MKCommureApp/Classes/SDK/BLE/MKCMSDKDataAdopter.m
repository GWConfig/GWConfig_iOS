...
@implementation MKCMSDKDataAdopter

+ (NSString *)fetchWifiSecurity:(mk_cm_wifiSecurity)security {
    switch (security) {
        case mk_cm_wifiSecurity_personal:
            return @"00";
        case mk_cm_wifiSecurity_enterprise:
            return @"01";
    }
}

+ (NSString *)fetchWifiEapType:(mk_cm_eapType)eapType {
    switch (eapType) {
        case mk_cm_eapType_peap_mschapv2:
            return @"00";
        case mk_cm_eapType_ttls_mschapv2:
            return @"01";
        case mk_cm_eapType_tls:
            return @"02";
    }
}

+ (NSString *)fetchConnectModeString:(mk_cm_connectMode)mode {
    switch (mode) {
        case mk_cm_connectMode_TCP:
            return @"00";
        case mk_cm_connectMode_CASignedServerCertificate:
            return @"01";
        case mk_cm_connectMode_CACertificate:
            return @"02";
        case mk_cm_connectMode_SelfSignedCertificates:
            return @"03";
    }
}

+ (NSString *)fetchMqttServerQosMode:(mk_cm_mqttServerQosMode)mode {
    switch (mode) {
        case mk_cm_mqttQosLevelAtMostOnce:
            return @"00";
        case mk_cm_mqttQosLevelAtLeastOnce:
            return @"01";
        case mk_cm_mqttQosLevelExactlyOnce:
            return @"02";
    }
}
