...
static NSString *const mk_cm_logName = @"mk_cm_bleLog";

NSString *const mk_cm_peripheralConnectStateChangedNotification = @"mk_cm_peripheralConnectStateChangedNotification";
NSString *const mk_cm_centralManagerStateChangedNotification = @"mk_cm_centralManagerStateChangedNotification";

NSString *const mk_cm_deviceDisconnectTypeNotification = @"mk_cm_deviceDisconnectTypeNotification";

static MKCMCentralManager *manager = nil;
static dispatch_once_t onceToken;

@interface MKCMCentralManager ()
...
@property (nonatomic, assign)mk_cm_centralConnectStatus connectStatus;

@end

@implementation MKCMCentralManager

- (void)dealloc {
    [self logToLocal:@"MKCMCentralManager销毁"];
    NSLog(@"MKCMCentralManager销毁");
}

- (instancetype)init {
    if (self = [super init]) {
        [self logToLocal:@"MKCMCentralManager初始化"];
        [[MKBLEBaseCentralManager shared] loadDataManager:self];
    }
    return self;
}

+ (MKCMCentralManager *)shared {
    dispatch_once(&onceToken, ^{
        if (!manager) {
            manager = [MKCMCentralManager new];
        }
    });
    return manager;
}
...
- (void)MKBLEBaseCentralManagerDiscoverPeripheral:(CBPeripheral *)peripheral
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if ([self.delegate respondsToSelector:@selector(mk_cm_receiveDevice:)]) {
                [self.delegate mk_cm_receiveDevice:dataModel];
            }
        });
    });
}
...
- (void)MKBLEBaseCentralManagerStartScan {
    ...
    if ([self.delegate respondsToSelector:@selector(mk_cm_startScan)]) {
        [self.delegate mk_cm_startScan];
    }
}

- (void)MKBLEBaseCentralManagerStopScan {
    [self logToLocal:@"停止扫描"];
    if ([self.delegate respondsToSelector:@selector(mk_cm_stopScan)]) {
        [self.delegate mk_cm_stopScan];
    }
}
...
- (void)MKBLEBaseCentralManagerStateChanged:(MKCentralManagerState)centralManagerState {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:mk_cm_centralManagerStateChangedNotification object:nil];
}
...
- (void)MKBLEBasePeripheralConnectStateChanged:(MKPeripheralConnectState)connectState {
    ...
    if (connectState == MKPeripheralConnectStateUnknow) {
        self.connectStatus = mk_cm_centralConnectStatusUnknow;
    }else if (connectState == MKPeripheralConnectStateConnecting) {
        self.connectStatus = mk_cm_centralConnectStatusConnecting;
    }else if (connectState == MKPeripheralConnectStateConnectedFailed) {
        self.connectStatus = mk_cm_centralConnectStatusConnectedFailed;
    }else if (connectState == MKPeripheralConnectStateDisconnect) {
        self.connectStatus = mk_cm_centralConnectStatusDisconnect;
    }
    NSLog(@"当前连接状态发生改变了:%@",@(connectState));
    NSString *string = [NSString stringWithFormat:@"连接状态发生改变:%@",@(connectState)];
    [self logToLocal:string];
    [[NSNotificationCenter defaultCenter] postNotificationName:mk_cm_peripheralConnectStateChangedNotification object:nil];
}
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
    ...
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"AA01"]]) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:mk_cm_deviceDisconnectTypeNotification
                                                            object:nil
...
- (mk_cm_centralManagerStatus )centralStatus {
    return ([MKBLEBaseCentralManager shared].centralStatus == MKCentralManagerStateEnable)
    ? mk_cm_centralManagerStatusEnable
    : mk_cm_centralManagerStatusUnable;
}
...
- (void)addTaskWithTaskID:(mk_cm_taskOperationID)operationID
           characteristic:(CBCharacteristic *)characteristic
              commandData:(NSString *)commandData
             successBlock:(void (^)(id returnData))successBlock
             failureBlock:(void (^)(NSError *error))failureBlock {
    MKCMOperation <MKBLEBaseOperationProtocol>*operation = [self generateOperationWithOperationID:operationID
...
- (void)addReadTaskWithTaskID:(mk_cm_taskOperationID)operationID
               characteristic:(CBCharacteristic *)characteristic
                 successBlock:(void (^)(id returnData))successBlock
                 failureBlock:(void (^)(NSError *error))failureBlock {
    MKCMOperation <MKBLEBaseOperationProtocol>*operation = [self generateReadOperationWithOperationID:operationID
                                                                                       characteristic:characteristic
                                                                                         successBlock:successBlock
                                                                                         failureBlock:failureBlock];
    if (!operation) {
...
- (void)addOperation:(MKCMOperation *)operation {
    if (!operation) {
        return;
    }
    [[MKBLEBaseCentralManager shared] addOperation:operation];
...
- (void)connectPeripheral:(CBPeripheral *)peripheral
    ...
    MKCMPeripheral *trackerPeripheral = [[MKCMPeripheral alloc] initWithPeripheral:peripheral];
...
- (void)sendPasswordToDevice {
    ...
    __weak typeof(self) weakSelf = self;
    MKCMOperation *operation = [[MKCMOperation alloc] initOperationWithID:mk_cm_connectPasswordOperation commandBlock:^{
        [[MKBLEBaseCentralManager shared] sendDataToPeripheral:commandData characteristic:[MKBLEBaseCentralManager shared].peripheral.cm_password type:CBCharacteristicWriteWithResponse];
        ...
        //密码正确
        MKBLEBase_main_safe(^{
            sself.connectStatus = mk_cm_centralConnectStatusConnected;
            [[NSNotificationCenter defaultCenter] postNotificationName:mk_cm_peripheralConnectStateChangedNotification object:nil];
            if (sself.sucBlock) {
...
- (MKCMOperation <MKBLEBaseOperationProtocol>*)generateOperationWithOperationID:(mk_cm_taskOperationID)operationID
                                                                 characteristic:(CBCharacteristic *)characteristic
                                                                    commandData:(NSString *)commandData
                                                                   successBlock:(void (^)(id returnData))successBlock
                                                                   failureBlock:(void (^)(NSError *error))failureBlock{
    ...
    __weak typeof(self) weakSelf = self;
    MKCMOperation <MKBLEBaseOperationProtocol>*operation = [[MKCMOperation alloc] initOperationWithID:operationID commandBlock:^{
        __strong typeof(self) sself = weakSelf;
...
- (MKCMOperation <MKBLEBaseOperationProtocol>*)generateReadOperationWithOperationID:(mk_cm_taskOperationID)operationID
                                                                     characteristic:(CBCharacteristic *)characteristic
                                                                       successBlock:(void (^)(id returnData))successBlock
                                                                       failureBlock:(void (^)(NSError *error))failureBlock{
    if (![[MKBLEBaseCentralManager shared] readyToCommunication]) {
...
- (void)logToLocal:(NSString *)string {
    ...
    [MKBLEBaseLogManager saveDataWithFileName:mk_cm_logName dataList:@[string]];
}
