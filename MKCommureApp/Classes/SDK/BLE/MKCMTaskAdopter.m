...
NSString *const mk_cm_totalNumKey = @"mk_cm_totalNumKey";
NSString *const mk_cm_totalIndexKey = @"mk_cm_totalIndexKey";
NSString *const mk_cm_contentKey = @"mk_cm_contentKey";

@implementation MKCMTaskAdopter
...
+ (NSDictionary *)parseReadDataWithCharacteristic:(CBCharacteristic *)characteristic {
    ...
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"2A24"]]) {
        ...
        return [self dataParserGetDataSuccess:@{@"modeID":tempString} operationID:mk_cm_taskReadDeviceModelOperation];
    }
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"2A26"]]) {
        //firmware
        NSString *tempString = [[NSString alloc] initWithData:readData encoding:NSUTF8StringEncoding];
        return [self dataParserGetDataSuccess:@{@"firmware":tempString} operationID:mk_cm_taskReadFirmwareOperation];
    }
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"2A27"]]) {
        //hardware
        NSString *tempString = [[NSString alloc] initWithData:readData encoding:NSUTF8StringEncoding];
        return [self dataParserGetDataSuccess:@{@"hardware":tempString} operationID:mk_cm_taskReadHardwareOperation];
    }
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"2A28"]]) {
        //soft ware
        NSString *tempString = [[NSString alloc] initWithData:readData encoding:NSUTF8StringEncoding];
        return [self dataParserGetDataSuccess:@{@"software":tempString} operationID:mk_cm_taskReadSoftwareOperation];
    }
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"2A29"]]) {
        //manufacturerKey
        NSString *tempString = [[NSString alloc] initWithData:readData encoding:NSUTF8StringEncoding];
        return [self dataParserGetDataSuccess:@{@"manufacturer":tempString} operationID:mk_cm_taskReadManufacturerOperation];
    }
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"AA00"]]) {
        ...
        return [self dataParserGetDataSuccess:@{@"state":state} operationID:mk_cm_connectPasswordOperation];
    }
...
+ (NSDictionary *)parseCustomReadData:(NSString *)content cmd:(NSString *)cmd data:(NSData *)data {
    mk_cm_taskOperationID operationID = mk_cm_defaultTaskOperationID;
    ...
    if ([cmd isEqualToString:@"02"]) {
        ...
        operationID = mk_cm_taskReadDeviceNameOperation;
        ...
        operationID = mk_cm_taskReadDeviceMacAddressOperation;
    }else if ([cmd isEqualToString:@"0a"]) {
        //读取WIFI STA MAC
        NSString *macAddress = [NSString stringWithFormat:@"%@:%@:%@:%@:%@:%@",[content substringWithRange:NSMakeRange(0, 2)],[content substringWithRange:NSMakeRange(2, 2)],[content substringWithRange:NSMakeRange(4, 2)],[content substringWithRange:NSMakeRange(6, 2)],[content substringWithRange:NSMakeRange(8, 2)],[content substringWithRange:NSMakeRange(10, 2)]];
        resultDic = @{@"macAddress":[macAddress uppercaseString]};
        operationID = mk_cm_taskReadDeviceWifiSTAMacAddressOperation;
        ...
        operationID = mk_cm_taskReadNTPServerHostOperation;
    }else if ([cmd isEqualToString:@"12"]) {
        //读取时区
        resultDic = @{
            @"timeZone":[MKBLEBaseSDKAdopter signedHexTurnString:content],
        };
        operationID = mk_cm_taskReadTimeZoneOperation;
        ...
        operationID = mk_cm_taskReadServerHostOperation;
        ...
        resultDic = @{@"port":port};
        operationID = mk_cm_taskReadServerPortOperation;
        ...
        operationID = mk_cm_taskReadServerKeepAliveOperation;
        ...
        resultDic = @{@"qos":qos};
        operationID = mk_cm_taskReadServerQosOperation;
        ...
        operationID = mk_cm_taskReadSubscibeTopicOperation;
        ...
        operationID = mk_cm_taskReadPublishTopicOperation;
        ...
        resultDic = @{@"mode":mode};
        operationID = mk_cm_taskReadConnectModeOperation;
        ...
        operationID = mk_cm_taskReadWIFISecurityOperation;
        ...
        operationID = mk_cm_taskReadWIFISSIDOperation;
        ...
        operationID = mk_cm_taskReadWIFIPasswordOperation;
        ...
        operationID = mk_cm_taskReadWIFIEAPTypeOperation;
        ...
        operationID = mk_cm_taskReadWIFIEAPUsernameOperation;
        ...
        operationID = mk_cm_taskReadWIFIEAPPasswordOperation;
        ...
        operationID = mk_cm_taskReadWIFIEAPDomainIDOperation;
    }else if ([cmd isEqualToString:@"47"]) {
        //读取是否校验服务器
        BOOL verify = ([content isEqualToString:@"01"]);
        resultDic = @{@"verify":@(verify)};
        operationID = mk_cm_taskReadWIFIVerifyServerStatusOperation;
    }else if ([cmd isEqualToString:@"4b"]) {
        //读取DHCP开关
        BOOL isOn = ([content isEqualToString:@"01"]);
        resultDic = @{@"isOn":@(isOn)};
        operationID = mk_cm_taskReadWIFIDHCPStatusOperation;
    }else if ([cmd isEqualToString:@"4c"]) {
        //读取IP信息
        NSString *ip = [MKCMSDKDataAdopter parseIpAddress:[content substringWithRange:NSMakeRange(0, 8)]];
        NSString *mask = [MKCMSDKDataAdopter parseIpAddress:[content substringWithRange:NSMakeRange(8, 8)]];
        NSString *gateway = [MKCMSDKDataAdopter parseIpAddress:[content substringWithRange:NSMakeRange(16, 8)]];
        NSString *dns = [MKCMSDKDataAdopter parseIpAddress:[content substringWithRange:NSMakeRange(24, 8)]];
        resultDic = @{
            @"ip":ip,
            @"mask":mask,
            @"gateway":gateway,
        ...
        operationID = mk_cm_taskReadWIFINetworkIpInfosOperation;
        ...
        operationID = mk_cm_taskReadNetworkTypeOperation;
    }else if ([cmd isEqualToString:@"4e"]) {
        //读取Ethernet DHCP开关
        BOOL isOn = ([content isEqualToString:@"01"]);
        resultDic = @{@"isOn":@(isOn)};
        operationID = mk_cm_taskReadEthernetDHCPStatusOperation;
        ...
        operationID = mk_cm_taskReadCountryBandOperation;
    }
...
+ (NSDictionary *)parseCustomConfigData:(NSString *)content cmd:(NSString *)cmd {
    ...
    if ([cmd isEqualToString:@"02"]) {
        //重启进入STA模式
        operationID = mk_cm_taskEnterSTAModeOperation;
    }else if ([cmd isEqualToString:@"11"]) {
        //配置NTP服务器域名
        operationID = mk_cm_taskConfigNTPServerHostOperation;
    }else if ([cmd isEqualToString:@"12"]) {
        //配置时区
        operationID = mk_cm_taskConfigTimeZoneOperation;
    }else if ([cmd isEqualToString:@"14"]) {
        //配置UTC时间戳
        operationID = mk_cm_taskConfigDeviceTimeOperation;
    }else if ([cmd isEqualToString:@"20"]) {
        //配置MQTT服务器域名
        operationID = mk_cm_taskConfigServerHostOperation;
    }else if ([cmd isEqualToString:@"21"]) {
        //配置MQTT服务器端口
        operationID = mk_cm_taskConfigServerPortOperation;
    }else if ([cmd isEqualToString:@"22"]) {
        //配置ClientID
        operationID = mk_cm_taskConfigClientIDOperation;
    }else if ([cmd isEqualToString:@"25"]) {
        //配置MQTT Clean Session
        operationID = mk_cm_taskConfigServerCleanSessionOperation;
    }else if ([cmd isEqualToString:@"26"]) {
        //配置MQTT KeepAlive
        operationID = mk_cm_taskConfigServerKeepAliveOperation;
    }else if ([cmd isEqualToString:@"27"]) {
        //配置MQTT Qos
        operationID = mk_cm_taskConfigServerQosOperation;
    }else if ([cmd isEqualToString:@"28"]) {
        //配置Subscribe topic
        operationID = mk_cm_taskConfigSubscibeTopicOperation;
    }else if ([cmd isEqualToString:@"29"]) {
        //配置Publish topic
        operationID = mk_cm_taskConfigPublishTopicOperation;
    }else if ([cmd isEqualToString:@"2f"]) {
        //配置MTQQ服务器通信加密方式
        operationID = mk_cm_taskConfigConnectModeOperation;
    }else if ([cmd isEqualToString:@"40"]) {
        //配置WIFI 加密模式
        operationID = mk_cm_taskConfigWIFISecurityOperation;
    }else if ([cmd isEqualToString:@"41"]) {
        //配置WIFI SSID
        operationID = mk_cm_taskConfigWIFISSIDOperation;
    }else if ([cmd isEqualToString:@"42"]) {
        //配置WIFI password
        operationID = mk_cm_taskConfigWIFIPasswordOperation;
    }else if ([cmd isEqualToString:@"43"]) {
        //配置WIFI EAP 类型
        operationID = mk_cm_taskConfigWIFIEAPTypeOperation;
    }else if ([cmd isEqualToString:@"44"]) {
        //配置WIFI EAP 用户名
        operationID = mk_cm_taskConfigWIFIEAPUsernameOperation;
    }else if ([cmd isEqualToString:@"45"]) {
        //配置WIFI EAP 密码
        operationID = mk_cm_taskConfigWIFIEAPPasswordOperation;
    }else if ([cmd isEqualToString:@"46"]) {
        //配置wifi的EAP域名ID
        operationID = mk_cm_taskConfigWIFIEAPDomainIDOperation;
    }else if ([cmd isEqualToString:@"47"]) {
        //配置wifi是否校验服务器
        operationID = mk_cm_taskConfigWIFIVerifyServerStatusOperation;
    }else if ([cmd isEqualToString:@"4b"]) {
        //配置Wifi DHCP状态
        operationID = mk_cm_taskConfigWIFIDHCPStatusOperation;
    }else if ([cmd isEqualToString:@"4c"]) {
        //配置Wifi IP地址相关信息
        operationID = mk_cm_taskConfigWIFIIpInfoOperation;
    }else if ([cmd isEqualToString:@"4d"]) {
        //配置网络接口类型
        operationID = mk_cm_taskConfigNetworkTypeOperation;
    }else if ([cmd isEqualToString:@"4e"]) {
        //配置Ethernet DHCP状态
        operationID = mk_cm_taskConfigEthernetDHCPStatusOperation;
    }else if ([cmd isEqualToString:@"4f"]) {
        //配置Ethernet IP地址相关信息
        operationID = mk_cm_taskConfigEthernetIpInfoOperation;
    }else if ([cmd isEqualToString:@"50"]) {
        //配置国家地区参数
        operationID = mk_cm_taskConfigCountryBandOperation;
    }
...
+ (NSDictionary *)parseMultiPackageReadData:(NSData *)readData cmd:(NSString *)cmd {
    ...
    NSDictionary *resultDic= @{
        mk_cm_totalNumKey:totalNum,
        mk_cm_totalIndexKey:index,
        mk_cm_contentKey:(subData ? subData : [NSData data]),
    };
    if ([cmd isEqualToString:@"23"]) {
        //读取服务器登录用户名
        operationID = mk_cm_taskReadServerUserNameOperation;
...
+ (NSDictionary *)parseMultiPackageData:(NSString *)content cmd:(NSString *)cmd {
    ...
    if ([cmd isEqualToString:@"23"]) {
        //配置服务器登录用户名
        operationID = mk_cm_taskConfigServerUserNameOperation;
    }else if ([cmd isEqualToString:@"24"]) {
        //配置服务器登录密码
        operationID = mk_cm_taskConfigServerPasswordOperation;
    }else if ([cmd isEqualToString:@"30"]) {
        //配置CA file
        operationID = mk_cm_taskConfigCAFileOperation;
    }else if ([cmd isEqualToString:@"31"]) {
        //配置Client certificate file
        operationID = mk_cm_taskConfigClientCertOperation;
    }else if ([cmd isEqualToString:@"32"]) {
        //配置Client key file
        operationID = mk_cm_taskConfigClientPrivateKeyOperation;
    }else if ([cmd isEqualToString:@"48"]) {
        //配置WIFI CA file
        operationID = mk_cm_taskConfigWIFICAFileOperation;
    }else if ([cmd isEqualToString:@"49"]) {
        //配置WIFI Client certificate file
        operationID = mk_cm_taskConfigWIFIClientCertOperation;
...
+ (NSDictionary *)dataParserGetDataSuccess:(NSDictionary *)returnData operationID:(mk_cm_taskOperationID)operationID{
    if (!returnData) {
        return @{};
    }
