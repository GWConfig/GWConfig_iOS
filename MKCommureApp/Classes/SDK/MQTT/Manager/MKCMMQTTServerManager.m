...
static NSString *const mqttLogName = @"CommureGatewayLogData";

static MKCMMQTTServerManager *manager = nil;
static dispatch_once_t onceToken;

@interface NSObject (MKCMMQTTServerManager)

@end

@implementation NSObject (MKCMMQTTServerManager)

//+ (void)load{
//    [MKCMMQTTServerManager shared];
//}

@end

@interface MKCMMQTTServerManager ()

@property (nonatomic, assign)MKCMMQTTSessionManagerState state;

@property (nonatomic, strong)MKCMServerParamsModel *paramsModel;

@property (nonatomic, strong)NSMutableArray <id <MKCMServerManagerProtocol>>*managerList;

@property (nonatomic, strong)NSMutableDictionary *subscriptions;

@end

@implementation MKCMMQTTServerManager
...
+ (MKCMMQTTServerManager *)shared {
    dispatch_once(&onceToken, ^{
        if (!manager) {
            manager = [MKCMMQTTServerManager new];
        }
    });
    return manager;
}
...
- (void)sessionManager:(MQTTSessionManager *)sessionManager
    ...
    @synchronized (self.managerList) {
        for (id <MKCMServerManagerProtocol>protocol in self.managerList) {
            if ([protocol respondsToSelector:@selector(cm_didReceiveMessage:onTopic:)]) {
                [protocol cm_didReceiveMessage:dataDic onTopic:topic];
            }
        }
    }
}
...
- (void)sessionManager:(MQTTSessionManager *)sessionManager didChangeState:(MKMQTTSessionManagerState)newState {
    ...
    @synchronized (self.managerList) {
        for (id <MKCMServerManagerProtocol>protocol in self.managerList) {
            if ([protocol respondsToSelector:@selector(cm_didChangeState:)]) {
                [protocol cm_didChangeState:self.state];
            }
        }
    }
}
...
- (void)networkStateChanged{
    ...
    if (![[MKNetworkManager sharedInstance] currentNetworkAvailable]) {
        ...
        [self sessionManager:manager didChangeState:MKCMMQTTSessionManagerStateStarting];
        return;
    }
...
- (void)loadDataManager:(nonnull id <MKCMServerManagerProtocol>)dataManager {
    @synchronized (self.managerList) {
        if (dataManager
            && [dataManager conformsToProtocol:@protocol(MKCMServerManagerProtocol)]
...
- (BOOL)removeDataManager:(nonnull id <MKCMServerManagerProtocol>)dataManager {
    @synchronized (self.managerList) {
        if (!dataManager ||
            ![dataManager conformsToProtocol:@protocol(MKCMServerManagerProtocol)] ||
            ![self.managerList containsObject:dataManager]) {
...
- (void)loadLogFile {
    ...
    NSString *recordDateString = [[NSUserDefaults standardUserDefaults] objectForKey:@"mk_cm_mqtt_recordDate"];
    if (!ValidStr(recordDateString)) {
        //如果第一次存，则把当天的日期写入
        [[NSUserDefaults standardUserDefaults] setObject:[dateFormatter stringFromDate:[NSDate date]] forKey:@"mk_cm_mqtt_recordDate"];
        return;
    }
    //写入当天日期，用来做比较
    [[NSUserDefaults standardUserDefaults] setObject:[dateFormatter stringFromDate:[NSDate date]] forKey:@"mk_cm_mqtt_recordDate"];
...
#pragma mark - getter
- (MKCMServerParamsModel *)paramsModel {
    if (!_paramsModel) {
        _paramsModel = [[MKCMServerParamsModel alloc] init];
    }
    return _paramsModel;
}

- (NSMutableArray<id<MKCMServerManagerProtocol>> *)managerList {
    if (!_managerList) {
