...
/**
 线程ID
 */
@property (nonatomic, assign)mk_cm_serverOperationID operationID;
...
@implementation MKCMMQTTOperation
@synthesize executing = _executing;
@synthesize finished = _finished;

- (void)dealloc{
    NSLog(@"MKCMMQTTOperation销毁");
}
...
#pragma mark - MKCMMQTTOperationProtocol
...
- (void)didReceiveMessage:(NSDictionary *)data onTopic:(NSString *)topic {
    ...
    [self dataParserReceivedData:[MKCMMQTTTaskAdopter parseDataWithJson:data topic:topic]];
}
...
- (void)dataParserReceivedData:(NSDictionary *)dataDic{
    ...
    mk_cm_serverOperationID operationID = [dataDic[@"operationID"] integerValue];
    if (operationID == mk_cm_defaultServerOperationID || operationID != self.operationID) {
        return;
    }
