...
@implementation MKCMBleWifiSettingsBandCellModel
@end

@interface MKCMBleWifiSettingsBandCell ()
...
@implementation MKCMBleWifiSettingsBandCell

+ (MKCMBleWifiSettingsBandCell *)initCellWithTableView:(UITableView *)tableView {
    MKCMBleWifiSettingsBandCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCMBleWifiSettingsBandCellIdenty"];
    if (!cell) {
        cell = [[MKCMBleWifiSettingsBandCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCMBleWifiSettingsBandCellIdenty"];
    }
    return cell;
}
...
- (void)selectedButtonPressed {
    ...
    [pickView showPickViewWithDataList:self.dataList selectedRow:row block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(cm_bleWifiSettingsBandCell_countryChanged:)]) {
            [self.delegate cm_bleWifiSettingsBandCell_countryChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKCMBleWifiSettingsBandCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCMBleWifiSettingsBandCellModel.class] || _dataModel.country >= self.dataList.count) {
        return;
    }
