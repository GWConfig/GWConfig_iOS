...
@implementation MKCMUserCredentialsViewModel
@end

@interface MKCMUserCredentialsView ()
...
@implementation MKCMUserCredentialsView
...
#pragma mark - setter
- (void)setDataModel:(MKCMUserCredentialsViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCMUserCredentialsViewModel.class]) {
        return;
    }
...
- (MKTextField *)userNameTextField {
    if (!_userNameTextField) {
        ...
        _userNameTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(cm_mqtt_userCredentials_userNameChanged:)]) {
                [self.delegate cm_mqtt_userCredentials_userNameChanged:text];
            }
        };
...
- (MKTextField *)passwordTextField {
    if (!_passwordTextField) {
        ...
        _passwordTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(cm_mqtt_userCredentials_passwordChanged:)]) {
                [self.delegate cm_mqtt_userCredentials_passwordChanged:text];
            }
        };
