...
NSString *const MKCFDeviceModelOfflineNotification = @"MKCFDeviceModelOfflineNotification";

@interface MKCFDeviceModel ()
...
@implementation MKCFDeviceModel

- (void)dealloc{
    NSLog(@"MKCFDeviceModel销毁");
}

#pragma mark - public method

- (void)startStateMonitoringTimer{
    if (self.onLineState == MKCFDeviceModelStateOffline) {
        return;
    }
    ...
    dispatch_source_set_event_handler(self.receiveTimer, ^{
        ...
        if (self.receiveTimerCount >= 62.f) {
            ...
            self.receiveTimerCount = 0;
            self.onLineState = MKCFDeviceModelStateOffline;
            dispatch_async(dispatch_get_main_queue(), ^{
                [[NSNotificationCenter defaultCenter] postNotificationName:MKCFDeviceModelOfflineNotification
                                                                    object:nil
                                                                  userInfo:@{@"macAddress":self.macAddress}];
                if ([self.delegate respondsToSelector:@selector(cf_deviceOfflineWithMacAddress:)]) {
                    [self.delegate cf_deviceOfflineWithMacAddress:self.macAddress];
                }
            });
            return ;
        }
