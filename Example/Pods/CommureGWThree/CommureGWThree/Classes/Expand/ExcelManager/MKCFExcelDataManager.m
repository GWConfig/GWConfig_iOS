...
@implementation MKCFExcelDataManager

+ (void)exportAppExcel:(id <MKCFExcelAppProtocol>)protocol
              sucBlock:(void(^)(void))sucBlock
           failedBlock:(void(^)(NSError *error))failedBlock {
    if (!protocol || ![protocol conformsToProtocol:@protocol(MKCFExcelAppProtocol)]) {
...
+ (void)exportDeviceExcel:(id <MKCFExcelDeviceProtocol>)protocol
                 sucBlock:(void(^)(void))sucBlock
              failedBlock:(void(^)(NSError *error))failedBlock {
    if (!protocol || ![protocol conformsToProtocol:@protocol(MKCFExcelDeviceProtocol)]) {
        [self operationFailedBlockWithMsg:@"Params Error" block:failedBlock];
        return;
    }
...
//    }
    //设置excel文件名和路径
    NSString *documentPath = [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) objectAtIndex:0];
    NSString *path = [documentPath stringByAppendingPathComponent:@"MKGW3 Settings for Device.xlsx"];
    //创建新xlsx文件
    lxw_workbook *workbook = workbook_new([path UTF8String]);
    //创建sheet
    lxw_sheet *worksheet = workbook_add_worksheet(workbook, NULL);
...
+ (void)parseDeviceAllParams:(NSURL *)filePath
    ...
    //Network Type
    MKExcelCell *networkTypeCell = [sheet getCellWithColumn:@"B" row:28 error:nil];
    NSString *networkTypeString = [SafeStr(networkTypeCell.stringValue) stringByReplacingOccurrencesOfString:defaultKeyValueString withString:@""];
    NSInteger networkType = [networkTypeString integerValue];
    [dic setObject:@(networkType) forKey:@"networkType"];
    
    //wifi DHCP
    MKExcelCell *wifiDHCPCell = [sheet getCellWithColumn:@"B" row:29 error:nil];
    NSString *wifiDHCPString = [SafeStr(wifiDHCPCell.stringValue) stringByReplacingOccurrencesOfString:defaultKeyValueString withString:@""];
    BOOL wifiDHCP = ([wifiDHCPString isEqualToString:@"1"]);
    [dic setObject:@(wifiDHCP) forKey:@"wifiDHCP"];
    
    //wifi IP
    MKExcelCell *wifiIPCell = [sheet getCellWithColumn:@"B" row:30 error:nil];
    NSString *wifiIP = [SafeStr(wifiIPCell.stringValue) stringByReplacingOccurrencesOfString:defaultKeyValueString withString:@""];
    [dic setObject:SafeStr(wifiIP) forKey:@"wifiIP"];
    
    //wifi Mask
    MKExcelCell *wifiMaskCell = [sheet getCellWithColumn:@"B" row:31 error:nil];
    NSString *wifiMask = [SafeStr(wifiMaskCell.stringValue) stringByReplacingOccurrencesOfString:defaultKeyValueString withString:@""];
    [dic setObject:SafeStr(wifiMask) forKey:@"wifiMask"];
    
    //wifi Gateway
    MKExcelCell *wifiGatewayCell = [sheet getCellWithColumn:@"B" row:32 error:nil];
    NSString *wifiGateway = [SafeStr(wifiGatewayCell.stringValue) stringByReplacingOccurrencesOfString:defaultKeyValueString withString:@""];
    [dic setObject:SafeStr(wifiGateway) forKey:@"wifiGateway"];
    
    //wifi DNS
    MKExcelCell *wifiDNSCell = [sheet getCellWithColumn:@"B" row:33 error:nil];
    NSString *wifiDNS = [SafeStr(wifiDNSCell.stringValue) stringByReplacingOccurrencesOfString:defaultKeyValueString withString:@""];
    [dic setObject:SafeStr(wifiDNS) forKey:@"wifiDNS"];
    
    //Ethernet DHCP
    MKExcelCell *ethernetDHCPCell = [sheet getCellWithColumn:@"B" row:34 error:nil];
    NSString *ethernetDHCPString = [SafeStr(ethernetDHCPCell.stringValue) stringByReplacingOccurrencesOfString:defaultKeyValueString withString:@""];
    BOOL ethernetDHCP = ([ethernetDHCPString isEqualToString:@"1"]);
    [dic setObject:@(ethernetDHCP) forKey:@"ethernetDHCP"];
    
    //Ethernet IP
    MKExcelCell *ethernetIPCell = [sheet getCellWithColumn:@"B" row:35 error:nil];
    NSString *ethernetIP = [SafeStr(ethernetIPCell.stringValue) stringByReplacingOccurrencesOfString:defaultKeyValueString withString:@""];
    [dic setObject:SafeStr(ethernetIP) forKey:@"ethernetIP"];
    
    //Ethernet Mask
    MKExcelCell *ethernetMaskCell = [sheet getCellWithColumn:@"B" row:36 error:nil];
    NSString *ethernetMask = [SafeStr(ethernetMaskCell.stringValue) stringByReplacingOccurrencesOfString:defaultKeyValueString withString:@""];
    [dic setObject:SafeStr(ethernetMask) forKey:@"ethernetMask"];
    
    //Ethernet Gateway
    MKExcelCell *ethernetGatewayCell = [sheet getCellWithColumn:@"B" row:37 error:nil];
    NSString *ethernetGateway = [SafeStr(ethernetGatewayCell.stringValue) stringByReplacingOccurrencesOfString:defaultKeyValueString withString:@""];
    [dic setObject:SafeStr(ethernetGateway) forKey:@"ethernetGateway"];
    
    //Ethernet DNS
    MKExcelCell *ethernetDNSCell = [sheet getCellWithColumn:@"B" row:38 error:nil];
    NSString *ethernetDNS = [SafeStr(ethernetDNSCell.stringValue) stringByReplacingOccurrencesOfString:defaultKeyValueString withString:@""];
    [dic setObject:SafeStr(ethernetDNS) forKey:@"ethernetDNS"];
    
    //NTP Server
    MKExcelCell *ntpServerCell = [sheet getCellWithColumn:@"B" row:39 error:nil];
    ...
    //Timezone
    MKExcelCell *timezoneCell = [sheet getCellWithColumn:@"B" row:40 error:nil];
...
+ (BOOL)checkExcelAppProtocol:(id <MKCFExcelAppProtocol>)protocol {
    if (!protocol || ![protocol conformsToProtocol:@protocol(MKCFExcelAppProtocol)]) {
        return NO;
    }
...
+ (BOOL)checkExcelDeviceProtocol:(id <MKCFExcelDeviceProtocol>)protocol {
    if (!protocol || ![protocol conformsToProtocol:@protocol(MKCFExcelDeviceProtocol)]) {
        return NO;
    }
    if (!ValidStr(protocol.host) || protocol.host.length > 64 || ![protocol.host isAsciiString]) {
