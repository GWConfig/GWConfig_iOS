...
@implementation MKCFUserCredentialsViewModel
@end

@interface MKCFUserCredentialsView ()
...
@implementation MKCFUserCredentialsView
...
#pragma mark - setter
- (void)setDataModel:(MKCFUserCredentialsViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCFUserCredentialsViewModel.class]) {
        return;
    }
...
- (MKTextField *)userNameTextField {
    if (!_userNameTextField) {
        ...
        _userNameTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(cf_mqtt_userCredentials_userNameChanged:)]) {
                [self.delegate cf_mqtt_userCredentials_userNameChanged:text];
            }
        };
...
- (MKTextField *)passwordTextField {
    if (!_passwordTextField) {
        ...
        _passwordTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(cf_mqtt_userCredentials_passwordChanged:)]) {
                [self.delegate cf_mqtt_userCredentials_passwordChanged:text];
            }
        };
