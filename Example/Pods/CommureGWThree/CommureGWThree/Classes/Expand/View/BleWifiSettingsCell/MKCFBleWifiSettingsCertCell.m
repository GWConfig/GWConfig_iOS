...
@implementation MKCFBleWifiSettingsCertCellModel
@end

@interface MKCFBleWifiSettingsCertCell ()
...
@implementation MKCFBleWifiSettingsCertCell

+ (MKCFBleWifiSettingsCertCell *)initCellWithTableView:(UITableView *)tableView {
    MKCFBleWifiSettingsCertCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCFBleWifiSettingsCertCellIdenty"];
    if (!cell) {
        cell = [[MKCFBleWifiSettingsCertCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCFBleWifiSettingsCertCellIdenty"];
    }
    return cell;
}
...
- (void)certificateButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cf_bleWifiSettingsCertPressed:)]) {
        [self.delegate cf_bleWifiSettingsCertPressed:self.dataModel.index];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKCFBleWifiSettingsCertCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCFBleWifiSettingsCertCellModel.class]) {
        return;
    }
...
- (UIButton *)certificateButton {
    if (!_certificateButton) {
        ...
        [_certificateButton setImage:LOADICON(@"CommureGWThree", @"MKCFBleWifiSettingsCertCell", @"cf_config_certAddIcon.png") forState:UIControlStateNormal];
