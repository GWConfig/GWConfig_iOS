...
@implementation MKCFFilterBeaconCellModel
@end

@interface MKCFFilterBeaconCell ()
...
@implementation MKCFFilterBeaconCell

+ (MKCFFilterBeaconCell *)initCellWithTableView:(UITableView *)tableView {
    MKCFFilterBeaconCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCFFilterBeaconCellIdenty"];
    if (!cell) {
        cell = [[MKCFFilterBeaconCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCFFilterBeaconCellIdenty"];
    }
    return cell;
}
...
#pragma mark - setter
- (void)setDataModel:(MKCFFilterBeaconCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCFFilterBeaconCellModel.class]) {
        return;
    }
...
- (MKTextField *)minTextField {
    if (!_minTextField) {
        ...
        _minTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(mk_cf_beaconMinValueChanged:index:)]) {
                [self.delegate mk_cf_beaconMinValueChanged:text index:self.dataModel.index];
            }
        };
    }
...
- (MKTextField *)maxTextField {
    if (!_maxTextField) {
        ...
        _maxTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(mk_cf_beaconMaxValueChanged:index:)]) {
                [self.delegate mk_cf_beaconMaxValueChanged:text index:self.dataModel.index];
            }
        };
    }
