...
@implementation MKCFBleWifiSettingsBandCellModel
@end

@interface MKCFBleWifiSettingsBandCell ()
...
@implementation MKCFBleWifiSettingsBandCell

+ (MKCFBleWifiSettingsBandCell *)initCellWithTableView:(UITableView *)tableView {
    MKCFBleWifiSettingsBandCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCFBleWifiSettingsBandCellIdenty"];
    if (!cell) {
        cell = [[MKCFBleWifiSettingsBandCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCFBleWifiSettingsBandCellIdenty"];
    }
    return cell;
}
...
- (void)selectedButtonPressed {
    ...
    [pickView showPickViewWithDataList:self.dataList selectedRow:row block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(cf_bleWifiSettingsBandCell_countryChanged:)]) {
            [self.delegate cf_bleWifiSettingsBandCell_countryChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKCFBleWifiSettingsBandCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCFBleWifiSettingsBandCellModel.class] || _dataModel.country >= self.dataList.count) {
        return;
    }
