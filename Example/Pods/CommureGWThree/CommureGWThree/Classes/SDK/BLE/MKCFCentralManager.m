...
static NSString *const mk_cf_logName = @"mk_cf_bleLog";

NSString *const mk_cf_peripheralConnectStateChangedNotification = @"mk_cf_peripheralConnectStateChangedNotification";
NSString *const mk_cf_centralManagerStateChangedNotification = @"mk_cf_centralManagerStateChangedNotification";

NSString *const mk_cf_deviceDisconnectTypeNotification = @"mk_cf_deviceDisconnectTypeNotification";

static MKCFCentralManager *manager = nil;
static dispatch_once_t onceToken;

@interface MKCFCentralManager ()
...
@property (nonatomic, assign)mk_cf_centralConnectStatus connectStatus;

@end

@implementation MKCFCentralManager

- (void)dealloc {
    [self logToLocal:@"MKCFCentralManager销毁"];
    NSLog(@"MKCFCentralManager销毁");
}

- (instancetype)init {
    if (self = [super init]) {
        [self logToLocal:@"MKCFCentralManager初始化"];
        [[MKBLEBaseCentralManager shared] loadDataManager:self];
    }
    return self;
}

+ (MKCFCentralManager *)shared {
    dispatch_once(&onceToken, ^{
        if (!manager) {
            manager = [MKCFCentralManager new];
        }
    });
    return manager;
}
...
- (void)MKBLEBaseCentralManagerDiscoverPeripheral:(CBPeripheral *)peripheral
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if ([self.delegate respondsToSelector:@selector(mk_cf_receiveDevice:)]) {
                [self.delegate mk_cf_receiveDevice:dataModel];
            }
        });
    });
}
...
- (void)MKBLEBaseCentralManagerStartScan {
    ...
    if ([self.delegate respondsToSelector:@selector(mk_cf_startScan)]) {
        [self.delegate mk_cf_startScan];
    }
}

- (void)MKBLEBaseCentralManagerStopScan {
    [self logToLocal:@"停止扫描"];
    if ([self.delegate respondsToSelector:@selector(mk_cf_stopScan)]) {
        [self.delegate mk_cf_stopScan];
    }
}
...
- (void)MKBLEBaseCentralManagerStateChanged:(MKCentralManagerState)centralManagerState {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:mk_cf_centralManagerStateChangedNotification object:nil];
}
...
- (void)MKBLEBasePeripheralConnectStateChanged:(MKPeripheralConnectState)connectState {
    ...
    if (connectState == MKPeripheralConnectStateUnknow) {
        self.connectStatus = mk_cf_centralConnectStatusUnknow;
    }else if (connectState == MKPeripheralConnectStateConnecting) {
        self.connectStatus = mk_cf_centralConnectStatusConnecting;
    }else if (connectState == MKPeripheralConnectStateConnectedFailed) {
        self.connectStatus = mk_cf_centralConnectStatusConnectedFailed;
    }else if (connectState == MKPeripheralConnectStateDisconnect) {
        self.connectStatus = mk_cf_centralConnectStatusDisconnect;
    }
    NSLog(@"当前连接状态发生改变了:%@",@(connectState));
    NSString *string = [NSString stringWithFormat:@"连接状态发生改变:%@",@(connectState)];
    [self logToLocal:string];
    [[NSNotificationCenter defaultCenter] postNotificationName:mk_cf_peripheralConnectStateChangedNotification object:nil];
}
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
    ...
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"AA01"]]) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:mk_cf_deviceDisconnectTypeNotification
                                                            object:nil
...
- (mk_cf_centralManagerStatus )centralStatus {
    return ([MKBLEBaseCentralManager shared].centralStatus == MKCentralManagerStateEnable)
    ? mk_cf_centralManagerStatusEnable
    : mk_cf_centralManagerStatusUnable;
}
...
- (void)addTaskWithTaskID:(mk_cf_taskOperationID)operationID
           characteristic:(CBCharacteristic *)characteristic
              commandData:(NSString *)commandData
             successBlock:(void (^)(id returnData))successBlock
             failureBlock:(void (^)(NSError *error))failureBlock {
    MKCFOperation <MKBLEBaseOperationProtocol>*operation = [self generateOperationWithOperationID:operationID
...
- (void)addReadTaskWithTaskID:(mk_cf_taskOperationID)operationID
               characteristic:(CBCharacteristic *)characteristic
                 successBlock:(void (^)(id returnData))successBlock
                 failureBlock:(void (^)(NSError *error))failureBlock {
    MKCFOperation <MKBLEBaseOperationProtocol>*operation = [self generateReadOperationWithOperationID:operationID
                                                                                       characteristic:characteristic
                                                                                         successBlock:successBlock
                                                                                         failureBlock:failureBlock];
    if (!operation) {
...
- (void)addOperation:(MKCFOperation *)operation {
    if (!operation) {
        return;
    }
    [[MKBLEBaseCentralManager shared] addOperation:operation];
...
- (void)connectPeripheral:(CBPeripheral *)peripheral
    ...
    MKCFPeripheral *trackerPeripheral = [[MKCFPeripheral alloc] initWithPeripheral:peripheral];
...
- (void)sendPasswordToDevice {
    ...
    __weak typeof(self) weakSelf = self;
    MKCFOperation *operation = [[MKCFOperation alloc] initOperationWithID:mk_cf_connectPasswordOperation commandBlock:^{
        [[MKBLEBaseCentralManager shared] sendDataToPeripheral:commandData characteristic:[MKBLEBaseCentralManager shared].peripheral.cf_password type:CBCharacteristicWriteWithResponse];
        ...
        //密码正确
        MKBLEBase_main_safe(^{
            sself.connectStatus = mk_cf_centralConnectStatusConnected;
            [[NSNotificationCenter defaultCenter] postNotificationName:mk_cf_peripheralConnectStateChangedNotification object:nil];
            if (sself.sucBlock) {
...
- (MKCFOperation <MKBLEBaseOperationProtocol>*)generateOperationWithOperationID:(mk_cf_taskOperationID)operationID
                                                                 characteristic:(CBCharacteristic *)characteristic
                                                                    commandData:(NSString *)commandData
                                                                   successBlock:(void (^)(id returnData))successBlock
                                                                   failureBlock:(void (^)(NSError *error))failureBlock{
    ...
    __weak typeof(self) weakSelf = self;
    MKCFOperation <MKBLEBaseOperationProtocol>*operation = [[MKCFOperation alloc] initOperationWithID:operationID commandBlock:^{
        __strong typeof(self) sself = weakSelf;
...
- (MKCFOperation <MKBLEBaseOperationProtocol>*)generateReadOperationWithOperationID:(mk_cf_taskOperationID)operationID
                                                                     characteristic:(CBCharacteristic *)characteristic
                                                                       successBlock:(void (^)(id returnData))successBlock
                                                                       failureBlock:(void (^)(NSError *error))failureBlock{
    if (![[MKBLEBaseCentralManager shared] readyToCommunication]) {
...
- (void)logToLocal:(NSString *)string {
    ...
    [MKBLEBaseLogManager saveDataWithFileName:mk_cf_logName dataList:@[string]];
}
