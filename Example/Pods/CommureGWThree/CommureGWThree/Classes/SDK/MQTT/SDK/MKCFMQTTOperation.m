...
/**
 线程ID
 */
@property (nonatomic, assign)mk_cf_serverOperationID operationID;
...
@implementation MKCFMQTTOperation
@synthesize executing = _executing;
@synthesize finished = _finished;

- (void)dealloc{
    NSLog(@"MKCFMQTTOperation销毁");
}
...
#pragma mark - MKCFMQTTOperationProtocol
...
- (void)didReceiveMessage:(NSDictionary *)data onTopic:(NSString *)topic {
    ...
    [self dataParserReceivedData:[MKCFMQTTTaskAdopter parseDataWithJson:data topic:topic]];
}
...
- (void)dataParserReceivedData:(NSDictionary *)dataDic{
    ...
    mk_cf_serverOperationID operationID = [dataDic[@"operationID"] integerValue];
    if (operationID == mk_cf_defaultServerOperationID || operationID != self.operationID) {
        return;
    }
