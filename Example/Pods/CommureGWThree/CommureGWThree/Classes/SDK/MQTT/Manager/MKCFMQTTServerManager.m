...
static NSString *const mqttLogName = @"CommureGWThreeLogData";

static MKCFMQTTServerManager *manager = nil;
static dispatch_once_t onceToken;

@interface NSObject (MKCFMQTTServerManager)

@end

@implementation NSObject (MKCFMQTTServerManager)
...
@interface MKCFMQTTServerManager ()

@property (nonatomic, assign)MKCFMQTTSessionManagerState state;

@property (nonatomic, strong)MKCFServerParamsModel *paramsModel;

@property (nonatomic, strong)NSMutableArray <id <MKCFServerManagerProtocol>>*managerList;

@property (nonatomic, strong)NSMutableDictionary *subscriptions;

@end

@implementation MKCFMQTTServerManager
...
+ (MKCFMQTTServerManager *)shared {
    dispatch_once(&onceToken, ^{
        if (!manager) {
            manager = [MKCFMQTTServerManager new];
        }
    });
    return manager;
}
...
- (void)sessionManager:(MQTTSessionManager *)sessionManager
    ...
    @synchronized (self.managerList) {
        for (id <MKCFServerManagerProtocol>protocol in self.managerList) {
            if ([protocol respondsToSelector:@selector(cf_didReceiveMessage:onTopic:)]) {
                [protocol cf_didReceiveMessage:dataDic onTopic:topic];
            }
        }
    }
}
...
- (void)sessionManager:(MQTTSessionManager *)sessionManager didChangeState:(MKMQTTSessionManagerState)newState {
    ...
    @synchronized (self.managerList) {
        for (id <MKCFServerManagerProtocol>protocol in self.managerList) {
            if ([protocol respondsToSelector:@selector(cf_didChangeState:)]) {
                [protocol cf_didChangeState:self.state];
            }
        }
    }
}
...
- (void)networkStateChanged{
    ...
    if (![[MKNetworkManager sharedInstance] currentNetworkAvailable]) {
        ...
        [self sessionManager:manager didChangeState:MKCFMQTTSessionManagerStateStarting];
        return;
    }
...
- (void)loadDataManager:(nonnull id <MKCFServerManagerProtocol>)dataManager {
    @synchronized (self.managerList) {
        if (dataManager
            && [dataManager conformsToProtocol:@protocol(MKCFServerManagerProtocol)]
...
- (void)loadLogFile {
    ...
    NSString *recordDateString = [[NSUserDefaults standardUserDefaults] objectForKey:@"mk_cf_mqtt_recordDate"];
    if (!ValidStr(recordDateString)) {
        //如果第一次存，则把当天的日期写入
        [[NSUserDefaults standardUserDefaults] setObject:[dateFormatter stringFromDate:[NSDate date]] forKey:@"mk_cf_mqtt_recordDate"];
        return;
    }
    //写入当天日期，用来做比较
    [[NSUserDefaults standardUserDefaults] setObject:[dateFormatter stringFromDate:[NSDate date]] forKey:@"mk_cf_mqtt_recordDate"];
...
#pragma mark - getter
- (MKCFServerParamsModel *)paramsModel {
    if (!_paramsModel) {
        _paramsModel = [[MKCFServerParamsModel alloc] init];
    }
    return _paramsModel;
}

- (NSMutableArray<id<MKCFServerManagerProtocol>> *)managerList {
    if (!_managerList) {
