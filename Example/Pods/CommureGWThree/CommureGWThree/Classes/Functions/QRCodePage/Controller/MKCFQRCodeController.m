...
@interface MKCFQRCodeController () <SGScanCodeDelegate, SGScanCodeSampleBufferDelegate, UINavigationControllerDelegate, UIImagePickerControllerDelegate>
{
    SGScanCode *scanCode;
}
@property (nonatomic, strong) SGScanView *scanView;
@property (nonatomic, strong) UIView *bottomView;
@property (nonatomic, strong) MKCFQRToolBar *toolBar;
@end

@implementation MKCFQRCodeController

- (void)dealloc {
    NSLog(@"MKCFQRCodeController销毁");
...
- (MKCFQRToolBar *)toolBar {
    if (!_toolBar) {
        _toolBar = [(MKCFQRToolBar *)[MKCFQRToolBar alloc] init];
        CGFloat h = 220;
