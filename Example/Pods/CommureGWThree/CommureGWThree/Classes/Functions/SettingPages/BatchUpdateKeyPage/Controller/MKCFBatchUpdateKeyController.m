//
//  MKCFBatchUpdateKeyController.m
...
@interface MKCFBatchUpdateKeyController ()<UITableViewDelegate,
UITableViewDataSource,
MKCFImportServerControllerDelegate,
MKCFBatchUpdateKeyHeaderViewDelegate,
...
@property (nonatomic, strong)MKCFBatchUpdateKeyModel *dataModel;

@property (nonatomic, strong)MKCFBatchUpdateKeyHeaderView *tableHeaderView;
...
@implementation MKCFBatchUpdateKeyController

- (void)dealloc {
    NSLog(@"MKCFBatchUpdateKeyController销毁");
...
#pragma mark - MKCFBatchUpdateKeyHeaderViewDelegate
...
- (void)addNotes {
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveUpdateKeyResult:)
                                                 name:MKCFReceiveBxpButtonUpdateKeyResultNotification
                                               object:nil];
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(receiveBatchUpdateKeyResult:)
                                                     name:MKCFReceiveBxpButtonBatchUpdateKeyResultNotification
                                                   object:nil];
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"CommureGWThree", @"MKCFBatchUpdateKeyController", @"cf_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCFBatchUpdateKeyModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCFBatchUpdateKeyModel alloc] init];
    }
    return _dataModel;
}

- (MKCFBatchUpdateKeyHeaderView *)tableHeaderView {
    if (!_tableHeaderView) {
        _tableHeaderView = [[MKCFBatchUpdateKeyHeaderView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 160.f)];
