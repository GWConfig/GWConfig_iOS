//
//  MKCFBatchUpdateKeyHeaderView.m
...
@interface MKCFBatchUpdateKeyHeaderView ()
...
@implementation MKCFBatchUpdateKeyHeaderView
...
- (MKTextField *)keyTextField {
    if (!_keyTextField) {
        ...
        _keyTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(cf_encryptionKeyChanged:)]) {
                [self.delegate cf_encryptionKeyChanged:text];
            }
        };
...
- (UIButton *)excelButton {
    if (!_excelButton) {
        ...
        [_excelButton setImage:LOADICON(@"CommureGWThree", @"MKCFBatchUpdateKeyHeaderView", @"cf_batchOtaListSelectIcon.png") forState:UIControlStateNormal];
...
- (UIButton *)scanButton {
    if (!_scanButton) {
        ...
        [_scanButton setImage:LOADICON(@"CommureGWThree", @"MKCFBatchUpdateKeyHeaderView", @"cf_batchOtaQRCodeIcon.png") forState:UIControlStateNormal];
