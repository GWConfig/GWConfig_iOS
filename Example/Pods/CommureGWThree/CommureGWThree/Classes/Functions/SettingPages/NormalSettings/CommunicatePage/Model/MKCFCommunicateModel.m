...
@interface MKCFCommunicateModel ()
...
@implementation MKCFCommunicateModel
...
- (BOOL)readCommunicateTimeout {
    ...
    [MKCFMQTTInterface cf_readCommunicateTimeoutWithMacAddress:[MKCFDeviceModeManager shared].macAddress topic:[MKCFDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configCommunicateTimeout {
    __block BOOL success = NO;
    [MKCFMQTTInterface cf_configCommunicationTimeout:[self.timeout integerValue] macAddress:[MKCFDeviceModeManager shared].macAddress topic:[MKCFDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
