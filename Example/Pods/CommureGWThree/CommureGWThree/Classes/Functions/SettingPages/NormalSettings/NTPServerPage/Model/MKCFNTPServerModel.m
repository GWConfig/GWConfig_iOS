...
@implementation MKCFNTPServerModel
...
- (BOOL)readNTPServer {
    ...
    [MKCFMQTTInterface cf_readNTPServerWithMacAddress:[MKCFDeviceModeManager shared].macAddress topic:[MKCFDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configNTPServer {
    __block BOOL success = NO;
    [MKCFMQTTInterface cf_configNTPServer:self.isOn host:self.host macAddress:[MKCFDeviceModeManager shared].macAddress topic:[MKCFDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
