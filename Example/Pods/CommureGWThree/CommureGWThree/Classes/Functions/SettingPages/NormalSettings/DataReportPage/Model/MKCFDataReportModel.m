...
@implementation MKCFDataReportModel
...
- (BOOL)readDataReportTimeout {
    ...
    [MKCFMQTTInterface cf_readDataReportTimeoutWithMacAddress:[MKCFDeviceModeManager shared].macAddress topic:[MKCFDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configDataReportTimeout {
    __block BOOL success = NO;
    [MKCFMQTTInterface cf_configDataReportTimeout:[self.timeout integerValue] macAddress:[MKCFDeviceModeManager shared].macAddress topic:[MKCFDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
