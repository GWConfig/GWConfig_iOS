...
@interface MKCFSystemTimeController ()<UITableViewDelegate,
UITableViewDataSource,
MKCFSystemTimeCellDelegate,
...
@implementation MKCFSystemTimeController

- (void)dealloc {
    NSLog(@"MKCFSystemTimeController销毁");
    if (self.readTimer) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0)  {
        //Sync Time From NTP
        MKCFNTPServerController *vc = [[MKCFNTPServerController alloc] init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKCFSystemTimeCell *cell = [MKCFSystemTimeCell initCellWithTableView:tableView];
...
#pragma mark - MKCFSystemTimeCellDelegate
...
- (void)readDataFromServer {
    ...
    [MKCFMQTTInterface cf_readDeviceUTCTimeWithMacAddress:[MKCFDeviceModeManager shared].macAddress topic:[MKCFDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)syncTimeZoneToDevice:(NSInteger)timeZone {
    ...
    [MKCFMQTTInterface cf_configDeviceTimeZone:timeZone - 24 timestamp:[[NSDate date] timeIntervalSince1970] macAddress:[MKCFDeviceModeManager shared].macAddress topic:[MKCFDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)loadSection1Datas {
    MKCFSystemTimeCellModel *cellModel = [[MKCFSystemTimeCellModel alloc] init];
    cellModel.index = 0;
