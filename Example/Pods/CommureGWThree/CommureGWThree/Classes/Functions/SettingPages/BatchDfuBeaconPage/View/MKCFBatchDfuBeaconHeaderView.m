//
//  MKCFBatchDfuBeaconHeaderView.m
...
@interface MKCFBatchDfuBeaconHeaderView ()
...
@implementation MKCFBatchDfuBeaconHeaderView
...
- (void)excelButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cf_beaconListButtonPressed)]) {
        [self.delegate cf_beaconListButtonPressed];
    }
}
...
- (MKTextField *)firmwareTextField {
    if (!_firmwareTextField) {
        ...
        _firmwareTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(cf_firmwareUrlChanged:)]) {
                [self.delegate cf_firmwareUrlChanged:text];
            }
        };
...
- (MKTextField *)dataTextField {
    if (!_dataTextField) {
        ...
        _dataTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(cf_dataFileUrlChanged:)]) {
                [self.delegate cf_dataFileUrlChanged:text];
            }
        };
...
- (MKTextField *)passwordTextField {
    if (!_passwordTextField) {
        ...
        _passwordTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(cf_dataPasswordChanged:)]) {
                [self.delegate cf_dataPasswordChanged:text];
            }
        };
...
- (UIButton *)excelButton {
    if (!_excelButton) {
        ...
        [_excelButton setImage:LOADICON(@"CommureGWThree", @"MKCFBatchDfuBeaconHeaderView", @"cf_batchOtaListSelectIcon.png") forState:UIControlStateNormal];
...
- (UIButton *)scanButton {
    if (!_scanButton) {
        ...
        [_scanButton setImage:LOADICON(@"CommureGWThree", @"MKCFBatchDfuBeaconHeaderView", @"cf_batchOtaQRCodeIcon.png") forState:UIControlStateNormal];
