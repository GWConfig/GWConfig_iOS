...
@interface MKCFMqttServerSettingView ()
...
@implementation MKCFMqttServerSettingView
...
- (void)clearButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cf_mqtt_deviecSetting_fileButtonPressed:)]) {
        [self.delegate cf_mqtt_deviecSetting_fileButtonPressed:2];
    }
}

- (void)exportButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cf_mqtt_deviecSetting_fileButtonPressed:)]) {
        [self.delegate cf_mqtt_deviecSetting_fileButtonPressed:0];
    }
}

- (void)importButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cf_mqtt_deviecSetting_fileButtonPressed:)]) {
        [self.delegate cf_mqtt_deviecSetting_fileButtonPressed:1];
    }
}
