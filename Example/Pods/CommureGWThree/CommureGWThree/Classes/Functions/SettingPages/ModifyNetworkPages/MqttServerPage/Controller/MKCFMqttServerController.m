...
@interface MKCFMqttServerController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKCFMqttServerConfigFooterViewDelegate,
...
@property (nonatomic, strong)MKCFMqttServerModel *dataModel;

@property (nonatomic, strong)MKCFMqttServerConfigFooterView *sslParamsView;

@property (nonatomic, strong)MKCFMqttServerConfigFooterViewModel *sslParamsModel;

@property (nonatomic, strong)UIView *footerView;

@end

@implementation MKCFMqttServerController

- (void)dealloc {
    NSLog(@"MKCFMqttServerController销毁");
}
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveUpdateCerts:)
                                                 name:MKCFReceiveDeviceUpdateMqttCertsResultNotification
                                               object:nil];
}
...
#pragma mark - MKCFMqttServerConfigFooterViewDelegate
/// 用户改变了开关状态
/// @param isOn isOn
...
- (void)cf_selectedServerParams:(NSString *)fileName {
    [MKCFExcelDataManager parseDeviceExcel:fileName
        ...
        MKCFMqttServerModel *model = [MKCFMqttServerModel mk_modelWithJSON:returnData];
...
- (void)clearAllParams {
    MKCFMqttServerModel *model = [[MKCFMqttServerModel alloc] init];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"CommureGWThree", @"MKCFMqttServerController", @"cf_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCFMqttServerModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCFMqttServerModel alloc] init];
    }
    return _dataModel;
}
...
- (MKCFMqttServerConfigFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKCFMqttServerConfigFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 380.f)];
...
- (MKCFMqttServerConfigFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKCFMqttServerConfigFooterViewModel alloc] init];
    }
