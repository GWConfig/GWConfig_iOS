//
//  MKCFMqttServerConfigFooterView.m
...
@implementation MKCFMqttServerConfigFooterViewModel
@end
...
@interface MKCFMqttServerConfigFooterView ()<UIScrollViewDelegate,
MKMQTTGeneralParamsViewDelegate,
MKMQTTUserCredentialsViewDelegate,
MKCFMqttServerSSLViewDelegate>
...
@property (nonatomic, strong)MKCFMqttServerSSLView *sslView;

@property (nonatomic, assign)NSInteger index;

@end

@implementation MKCFMqttServerConfigFooterView
...
#pragma mark - MKCFMqttServerSSLViewDelegate
- (void)cf_mqtt_sslParams_modifyDevice_sslStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(cf_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
...
- (void)cf_mqtt_sslParams_modifyDevice_certificateChanged:(NSInteger)certificate {
...
/// @param value value
- (void)cf_mqtt_sslParams_modifyDevice_textFieldValueChanged:(NSInteger)index value:(NSString *)value {
    ...
    if ([self.delegate respondsToSelector:@selector(cf_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate cf_mqtt_serverForDevice_textFieldValueChanged:value textID:tempIndex];
    }
}
...
#pragma mark - setter
- (void)setDataModel:(MKCFMqttServerConfigFooterViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCFMqttServerConfigFooterViewModel.class]) {
        return;
    }
    ...
    MKCFMqttServerSSLViewModel *sslModel = [[MKCFMqttServerSSLViewModel alloc] init];
...
- (MKCFMqttServerSSLView *)sslView {
    if (!_sslView) {
        _sslView = [[MKCFMqttServerSSLView alloc] init];
