...
@implementation MKCFMqttServerSSLViewModel
@end

@interface MKCFMqttServerSSLView ()<MKCFMqttServerSSLTextFieldDelegate>
...
@property (nonatomic, strong)MKCFMqttServerSSLTextField *caFileView;

@property (nonatomic, strong)MKCFMqttServerSSLTextField *clientKeyView;

@property (nonatomic, strong)MKCFMqttServerSSLTextField *clientCertView;

@end

@implementation MKCFMqttServerSSLView
...
#pragma mark - MKCFMqttServerSSLTextFieldDelegate
/// textField内容发送改变时的回调事件
/// @param index 当前cell所在的index
/// @param value 当前textField的值
- (void)cf_modifyServerSSLTextFieldValueChanged:(NSInteger)index textValue:(NSString *)value {
    if ([self.delegate respondsToSelector:@selector(cf_mqtt_sslParams_modifyDevice_textFieldValueChanged:value:)]) {
        [self.delegate cf_mqtt_sslParams_modifyDevice_textFieldValueChanged:index value:value];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(cf_mqtt_sslParams_modifyDevice_sslStatusChanged:)]) {
        [self.delegate cf_mqtt_sslParams_modifyDevice_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(cf_mqtt_sslParams_modifyDevice_certificateChanged:)]) {
            [self.delegate cf_mqtt_sslParams_modifyDevice_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKCFMqttServerSSLViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCFMqttServerSSLViewModel.class]) {
        return;
    }
    ...
    MKCFMqttServerSSLTextFieldModel *caModel = [[MKCFMqttServerSSLTextFieldModel alloc] init];
    caModel.index = 0;
    ...
    MKCFMqttServerSSLTextFieldModel *clientKeyModel = [[MKCFMqttServerSSLTextFieldModel alloc] init];
    ...
    MKCFMqttServerSSLTextFieldModel *clientModel = [[MKCFMqttServerSSLTextFieldModel alloc] init];
    clientModel.index = 2;
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"CommureGWThree", @"MKCFMqttServerSSLView", @"cf_switchSelectedIcon.png") : LOADICON(@"CommureGWThree", @"MKCFMqttServerSSLView", @"cf_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"CommureGWThree", @"MKCFMqttServerSSLView", @"cf_switchUnselectedIcon.png") forState:UIControlStateNormal];
...
- (MKCFMqttServerSSLTextField *)caFileView {
    if (!_caFileView) {
        _caFileView = [[MKCFMqttServerSSLTextField alloc] init];
        _caFileView.delegate = self;
    }
    return _caFileView;
}

- (MKCFMqttServerSSLTextField *)clientKeyView {
    if (!_clientKeyView) {
        _clientKeyView = [[MKCFMqttServerSSLTextField alloc] init];
        _clientKeyView.delegate = self;
    }
    return _clientKeyView;
}

- (MKCFMqttServerSSLTextField *)clientCertView {
    if (!_clientCertView) {
        _clientCertView = [[MKCFMqttServerSSLTextField alloc] init];
