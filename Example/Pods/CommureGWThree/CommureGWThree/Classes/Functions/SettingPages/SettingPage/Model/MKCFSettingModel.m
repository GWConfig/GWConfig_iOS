...
@implementation MKCFSettingModel
...
- (BOOL)readAdvStatus {
    ...
    [MKCFMQTTInterface cf_readBleAdvStatusWithMacAddress:[MKCFDeviceModeManager shared].macAddress topic:[MKCFDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configAdvStatus {
    __block BOOL success = NO;
    [MKCFMQTTInterface cf_configBleAdvStatus:self.advIsOn advTime:[self.advTime integerValue] macAddress:[MKCFDeviceModeManager shared].macAddress topic:[MKCFDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
