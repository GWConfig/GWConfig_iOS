...
@interface MKCFSettingController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCFSettingModel *dataModel;

@property (nonatomic, copy)NSString *localNameAsciiStr;

@end

@implementation MKCFSettingController

- (void)dealloc {
    NSLog(@"MKCFSettingController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0) {
        ...
        MKCFNetworkStatusController *vc = [[MKCFNetworkStatusController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 1) {
        //Communicate timeout
        MKCFCommunicateController *vc = [[MKCFCommunicateController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 2) {
        //Data report timout
        MKCFDataReportController *vc = [[MKCFDataReportController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 3) {
        //Connect Beacon timeout
        MKCFConnectBeaconTimeoutController *vc = [[MKCFConnectBeaconTimeoutController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 4) {
        //System time
        MKCFSystemTimeController *vc = [[MKCFSystemTimeController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 5) {
        //Reset device by button
        MKCFResetByButtonController *vc = [[MKCFResetByButtonController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    ...
    if (indexPath.section == 2 && indexPath.row == 0) {
        //OTA
        MKCFOTAController *vc = [[MKCFOTAController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 1) {
        //Modify network settings
        MKCFMqttConfigSelectController *vc = [[MKCFMqttConfigSelectController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    ...
    if (indexPath.section == 3 && indexPath.row == 0) {
        //Batch DFU Beacon
        MKCFBatchDfuBeaconController *vc = [[MKCFBatchDfuBeaconController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 3 && indexPath.row == 1) {
        //Batch update Beacon key
        MKCFBatchUpdateKeyController *vc = [[MKCFBatchUpdateKeyController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    ...
    if (indexPath.section == 4 && indexPath.row == 0) {
        //Device information
        MKCFDeviceInfoController *vc = [[MKCFDeviceInfoController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
}
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    if (section == 1) {
        return ([[MKCFDeviceModeManager shared].deviceType isEqualToString:@"10"] ? 1 : self.section1List.count);
    }
    if (section == 2) {
...
- (void)rootButtonPressed {
    ...
    [alertView showAlertWithTitle:@"Reboot Device" message:msg notificationName:@"mk_cf_needDismissAlert"];
}
...
- (void)resetButtonPressed {
    ...
    [alertView addAction:cancelAction];
    [alertView addAction:confirmAction];
    [alertView showAlertWithTitle:@"Reset Device" message:msg notificationName:@"mk_cf_needDismissAlert"];
}
...
- (void)configLocalName{
    ...
    self.localNameAsciiStr = SafeStr([MKCFDeviceModeManager shared].deviceName);
    MKAlertViewTextField *textField = [[MKAlertViewTextField alloc] initWithTextValue:SafeStr([MKCFDeviceModeManager shared].deviceName)
    ...
    [alertView showAlertWithTitle:@"Edit Local Name" message:msg notificationName:@"mk_cf_needDismissAlert"];
}
...
- (void)saveDeviceLocalName {
    ...
    [MKCFDeviceDatabaseManager updateLocalName:self.localNameAsciiStr
                                    macAddress:[MKCFDeviceModeManager shared].macAddress
                                      sucBlock:^{
        [[MKHudManager share] hide];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_cf_deviceNameChangedNotification"
                                                            object:nil
                                                          userInfo:@{
                                                              @"macAddress":[MKCFDeviceModeManager shared].macAddress,
...
- (void)resetDevice {
    ...
    [MKCFMQTTInterface cf_resetDeviceWithMacAddress:[MKCFDeviceModeManager shared].macAddress topic:[MKCFDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        [self removeDevice];
    } failedBlock:^(NSError * _Nonnull error) {
        [[MKHudManager share] hide];
...
- (void)removeDevice {
    ...
    [MKCFDeviceDatabaseManager deleteDeviceWithMacAddress:[MKCFDeviceModeManager shared].macAddress sucBlock:^{
        [[MKHudManager share] hide];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_cf_deleteDeviceNotification"
                                                            object:nil
                                                          userInfo:@{@"macAddress":[MKCFDeviceModeManager shared].macAddress}];
...
- (void)showAdvTimeAlert {
    ...
    [alertView addAction:confirmAction];
    [alertView addTextField:textField];
    [alertView showAlertWithTitle:@"Advertising time" message:@"" notificationName:@"mk_cf_needDismissAlert"];
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"CommureGWThree", @"MKCFSettingController", @"cf_editIcon.png") forState:UIControlStateNormal];
...
- (MKCFSettingModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCFSettingModel alloc] init];
    }
    return _dataModel;
}
