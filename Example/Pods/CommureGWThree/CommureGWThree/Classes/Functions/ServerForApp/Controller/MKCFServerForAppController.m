...
@interface MKCFServerForAppController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKCFServerConfigAppFooterViewDelegate,
...
@property (nonatomic, strong)MKCFServerForAppModel *dataModel;

@property (nonatomic, strong)MKCFServerConfigAppFooterView *sslParamsView;

@property (nonatomic, strong)MKCFServerConfigAppFooterViewModel *sslParamsModel;
...
@implementation MKCFServerForAppController

- (void)dealloc {
    NSLog(@"MKCFServerForAppController销毁");
...
- (void)rightButtonMethod {
    ...
    [[MKCFMQTTDataManager shared] saveServerParams:self.dataModel];
    ...
    [[MKCFMQTTDataManager shared] connect];
}
...
#pragma mark - MKCFServerConfigAppFooterViewDelegate
/// 用户改变了开关状态
/// @param isOn isOn
/// @param statusID 0:cleanSession   1:ssl
- (void)cf_mqtt_serverForApp_switchStatusChanged:(BOOL)isOn statusID:(NSInteger)statusID {
    if (statusID == 0) {
        //cleanSession
...
- (void)cf_mqtt_serverForApp_qosChanged:(NSInteger)qos {
...
- (void)cf_mqtt_serverForApp_textFieldValueChanged:(NSString *)text textID:(NSInteger)textID {
    if (textID == 0) {
        //keep alive
...
- (void)cf_mqtt_serverForApp_certificateChanged:(NSInteger)certificate {
...
- (void)cf_mqtt_serverForApp_fileButtonPressed:(NSInteger)fileType {
    if (fileType == 0) {
        //caFaile
...
- (void)cf_selectedServerParams:(NSString *)fileName {
    [MKCFExcelDataManager parseAppExcel:fileName
                                sucBlock:^(NSDictionary * _Nonnull returnData) {
        MKCFServerForAppModel *model = [MKCFServerForAppModel mk_modelWithJSON:returnData];
...
- (void)serverManagerStateChanged {
    ...
    if ([MKCFMQTTDataManager shared].state == MKCFMQTTSessionManagerStateConnected) {
        ...
        [[NSNotificationCenter defaultCenter] removeObserver:self
                                                        name:MKCFMQTTSessionManagerStateChangedNotification
                                                      object:nil];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_cf_needReloadTopicsNotification"
                                                            object:nil];
...
//    }
    ...
    [MKCFExcelDataManager exportAppExcel:self.dataModel
                                 sucBlock:^{
        [[MKHudManager share] hide];
        [self sharedExcel];
    }
...
- (void)connectFailed {
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self
                                                    name:MKCFMQTTSessionManagerStateChangedNotification
                                                  object:nil];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"MKCFMQTTServerConnectFailedNotification" object:nil];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"CommureGWThree", @"MKCFServerForAppController", @"cf_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCFServerForAppModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCFServerForAppModel alloc] init];
    }
    return _dataModel;
}
...
- (MKCFServerConfigAppFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKCFServerConfigAppFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 230.f)];
...
- (MKCFServerConfigAppFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKCFServerConfigAppFooterViewModel alloc] init];
    }
