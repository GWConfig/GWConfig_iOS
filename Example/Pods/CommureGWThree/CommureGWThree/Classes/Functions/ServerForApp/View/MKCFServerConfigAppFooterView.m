//
//  MKCFServerConfigAppFooterView.m
...
@implementation MKCFServerConfigAppFooterViewModel
@end
...
@interface MKCFServerConfigAppFooterView ()<UIScrollViewDelegate,
MKMQTTGeneralParamsViewDelegate,
MKMQTTUserCredentialsViewDelegate,
MKCFMQTTSSLForAppViewDelegate>
...
@property (nonatomic, strong)MKCFMQTTSSLForAppView *sslView;
...
@implementation MKCFServerConfigAppFooterView
...
- (void)mk_mqtt_generalParams_cleanSessionStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(cf_mqtt_serverForApp_switchStatusChanged:statusID:)]) {
        [self.delegate cf_mqtt_serverForApp_switchStatusChanged:isOn statusID:0];
    }
}

- (void)mk_mqtt_generalParams_qosChanged:(NSInteger)qos {
    if ([self.delegate respondsToSelector:@selector(cf_mqtt_serverForApp_qosChanged:)]) {
        [self.delegate cf_mqtt_serverForApp_qosChanged:qos];
    }
}

- (void)mk_mqtt_generalParams_KeepAliveChanged:(NSString *)keepAlive {
    if ([self.delegate respondsToSelector:@selector(cf_mqtt_serverForApp_textFieldValueChanged:textID:)]) {
        [self.delegate cf_mqtt_serverForApp_textFieldValueChanged:keepAlive textID:0];
    }
}
...
- (void)mk_mqtt_userCredentials_userNameChanged:(NSString *)userName {
    if ([self.delegate respondsToSelector:@selector(cf_mqtt_serverForApp_textFieldValueChanged:textID:)]) {
        [self.delegate cf_mqtt_serverForApp_textFieldValueChanged:userName textID:1];
    }
}

- (void)mk_mqtt_userCredentials_passwordChanged:(NSString *)password {
    if ([self.delegate respondsToSelector:@selector(cf_mqtt_serverForApp_textFieldValueChanged:textID:)]) {
        [self.delegate cf_mqtt_serverForApp_textFieldValueChanged:password textID:2];
    }
}

#pragma mark - MKCFMQTTSSLForAppViewDelegate
- (void)cf_mqtt_sslParams_app_sslStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(cf_mqtt_serverForApp_switchStatusChanged:statusID:)]) {
        [self.delegate cf_mqtt_serverForApp_switchStatusChanged:isOn statusID:1];
    }
}
...
- (void)cf_mqtt_sslParams_app_certificateChanged:(NSInteger)certificate {
    if ([self.delegate respondsToSelector:@selector(cf_mqtt_serverForApp_certificateChanged:)]) {
        [self.delegate cf_mqtt_serverForApp_certificateChanged:certificate];
    }
}

/// 用户点击选择了caFaile按钮
- (void)cf_mqtt_sslParams_app_caFilePressed {
    if ([self.delegate respondsToSelector:@selector(cf_mqtt_serverForApp_fileButtonPressed:)]) {
        [self.delegate cf_mqtt_serverForApp_fileButtonPressed:0];
    }
}
...
- (void)cf_mqtt_sslParams_app_clientFilePressed {
    if ([self.delegate respondsToSelector:@selector(cf_mqtt_serverForApp_fileButtonPressed:)]) {
        [self.delegate cf_mqtt_serverForApp_fileButtonPressed:1];
    }
}
...
#pragma mark - setter
- (void)setDataModel:(MKCFServerConfigAppFooterViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCFServerConfigAppFooterViewModel.class]) {
        return;
    }
    ...
    MKCFMQTTSSLForAppViewModel *sslModel = [[MKCFMQTTSSLForAppViewModel alloc] init];
...
- (MKCFMQTTSSLForAppView *)sslView {
    if (!_sslView) {
        _sslView = [[MKCFMQTTSSLForAppView alloc] init];
