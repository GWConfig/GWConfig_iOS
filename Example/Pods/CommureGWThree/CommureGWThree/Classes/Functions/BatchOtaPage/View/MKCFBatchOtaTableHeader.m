...
@interface MKCFBatchOtaTableHeader ()
...
@implementation MKCFBatchOtaTableHeader
...
- (MKTextField *)urlTextField {
    if (!_urlTextField) {
        ...
        _urlTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(cf_urlValueChanged:)]) {
                [self.delegate cf_urlValueChanged:text];
            }
        };
    }
...
- (UIButton *)excelButton {
    if (!_excelButton) {
        ...
        [_excelButton setImage:LOADICON(@"CommureGWThree", @"MKCFBatchOtaTableHeader", @"cf_batchOtaListSelectIcon.png") forState:UIControlStateNormal];
...
- (UIButton *)scanButton {
    if (!_scanButton) {
        ...
        [_scanButton setImage:LOADICON(@"CommureGWThree", @"MKCFBatchOtaTableHeader", @"cf_batchOtaQRCodeIcon.png") forState:UIControlStateNormal];
