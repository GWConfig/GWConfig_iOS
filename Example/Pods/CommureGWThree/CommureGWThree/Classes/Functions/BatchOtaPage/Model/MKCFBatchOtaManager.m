...
@implementation MKCFBatchOtaManager

- (void)dealloc {
    NSLog(@"MKCFBatchOtaManager销毁");
...
- (void)startBatchOta:(NSArray <NSString *>*)macList
   statusChangedBlock:(void (^)(NSString *macAddress,cf_batchOtaStatus status))statusBlock
    ...
    for (NSInteger i = 0; i < macList.count; i ++) {
        ...
        MKCFBatchOtaModel *otaModel = [[MKCFBatchOtaModel alloc] init];
...
- (void)processOtaModel {
    ...
    MKCFBatchOtaModel *otaModel = [self.otaList firstObject];
    @weakify(self);
    [otaModel otaWithResultBlock:^(NSString * _Nonnull macAddress, cf_batchOtaStatus status) {
        @strongify(self);
        self.updateCount = 0;
        if (self.otaBlock) {
            self.otaBlock(macAddress, status);
        }
        if (status != cf_batchOtaStatus_upgrading) {
            //升级结果
