...
@implementation MKCFAboutController

- (void)dealloc {
    NSLog(@"MKCFAboutController销毁");
}
...
- (void)feedButtonPressed {
    MKCFAppLogController *vc = [[MKCFAppLogController alloc] init];
...
- (UIImageView *)aboutIcon {
    if (!_aboutIcon) {
        ...
        _aboutIcon.image = LOADICON(@"CommureGWThree", @"MKCFAboutController", @"cf_deviceList_centerIcon.png");
    }
    return _aboutIcon;
}
