//
//  MKCFServerForDeviceController.m
...
@interface MKCFServerForDeviceController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKCFServerConfigDeviceFooterViewDelegate,
...
@property (nonatomic, strong)MKCFServerForDeviceModel *dataModel;

@property (nonatomic, strong)MKCFServerConfigDeviceFooterView *sslParamsView;

@property (nonatomic, strong)MKCFServerConfigDeviceFooterViewModel *sslParamsModel;

@property (nonatomic, strong)UIView *footerView;

@end

@implementation MKCFServerForDeviceController

- (void)dealloc {
    NSLog(@"MKCFServerForDeviceController销毁");
}
...
#pragma mark - MKCFServerConfigDeviceFooterViewDelegate
/// 用户改变了开关状态
/// @param isOn isOn
...
- (void)cf_mqtt_serverForDevice_fileButtonPressed:(NSInteger)fileType {
    if (fileType == 0) {
        //caFile
...
- (void)cf_selectedServerParams:(NSString *)fileName {
    [MKCFExcelDataManager parseDeviceExcel:fileName
                                   sucBlock:^(NSDictionary * _Nonnull returnData) {
        MKCFServerForDeviceModel *model = [MKCFServerForDeviceModel mk_modelWithJSON:returnData];
...
- (void)saveDataToDevice {
    ...
    [self.dataModel configDataWithSucBlock:^{
        ...
        [MKCFDeviceMQTTParamsModel shared].mqttConfig = YES;
        [MKCFDeviceMQTTParamsModel shared].deviceModel.clientID = self.dataModel.clientID;
        [MKCFDeviceMQTTParamsModel shared].deviceModel.deviceName = self.dataModel.deviceName;
        [MKCFDeviceMQTTParamsModel shared].deviceModel.subscribedTopic = self.dataModel.subscribeTopic;
        [MKCFDeviceMQTTParamsModel shared].deviceModel.publishedTopic = self.dataModel.publishTopic;
        [MKCFDeviceMQTTParamsModel shared].deviceModel.macAddress = self.dataModel.macAddress;
    } failedBlock:^(NSError * _Nonnull error) {
        @strongify(self);
        [[MKHudManager share] hide];
        [self.view showCentralToast:error.userInfo[@"errorInfo"]];
        [MKCFDeviceMQTTParamsModel shared].deviceModel.clientID = @"";
        [MKCFDeviceMQTTParamsModel shared].deviceModel.deviceName = @"";
        [MKCFDeviceMQTTParamsModel shared].deviceModel.subscribedTopic = @"";
        [MKCFDeviceMQTTParamsModel shared].deviceModel.publishedTopic = @"";
        [MKCFDeviceMQTTParamsModel shared].deviceModel.macAddress = @"";
    }];
}
...
//    }
    ...
    [MKCFExcelDataManager exportDeviceExcel:self.dataModel
                                    sucBlock:^{
        [[MKHudManager share] hide];
        [self sharedExcel];
    }
...
- (void)sharedExcel {
    ...
    [mailComposer addAttachmentData:data
                           ...
                           fileName:@"MKGW3 Settings for Device.xlsx"];
...
- (void)clearAllParams {
    MKCFServerForDeviceModel *emptyModel = [[MKCFServerForDeviceModel alloc] init];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"CommureGWThree", @"MKCFServerForDeviceController", @"cf_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCFServerForDeviceModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCFServerForDeviceModel alloc] init];
    }
    return _dataModel;
}
...
- (MKCFServerConfigDeviceFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKCFServerConfigDeviceFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 380.f)];
...
- (MKCFServerConfigDeviceFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKCFServerConfigDeviceFooterViewModel alloc] init];
    }
