//
//  MKCFBleNTPTimezoneController.m
...
@interface MKCFBleNTPTimezoneController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCFBleNTPTimezoneModel *dataModel;

@property (nonatomic, strong)NSMutableArray *timezoneList;

@end

@implementation MKCFBleNTPTimezoneController

- (void)dealloc {
    NSLog(@"MKCFBleNTPTimezoneController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"CommureGWThree", @"MKCFBleNTPTimezoneController", @"cf_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCFBleNTPTimezoneModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCFBleNTPTimezoneModel alloc] init];
    }
    return _dataModel;
}
