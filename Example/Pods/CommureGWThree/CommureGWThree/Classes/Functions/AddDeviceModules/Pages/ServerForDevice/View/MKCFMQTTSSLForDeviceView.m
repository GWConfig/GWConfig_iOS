...
@implementation MKCFMQTTSSLForDeviceViewModel
@end

@interface MKCFMQTTSSLForDeviceView ()<MKMQTTSSLCertificateViewDelegate>
...
@implementation MKCFMQTTSSLForDeviceView
...
- (void)mk_fileSelectedButtonPressed:(NSInteger)index {
    if (index == 0) {
        //CA File
        if ([self.delegate respondsToSelector:@selector(cf_mqtt_sslParams_device_caFilePressed)]) {
            [self.delegate cf_mqtt_sslParams_device_caFilePressed];
        }
        return;
    }
    if (index == 1) {
        //Client Key File
        if ([self.delegate respondsToSelector:@selector(cf_mqtt_sslParams_device_clientKeyPressed)]) {
            [self.delegate cf_mqtt_sslParams_device_clientKeyPressed];
        }
        return;
    }
    //Client Cert File
    if ([self.delegate respondsToSelector:@selector(cf_mqtt_sslParams_device_clientCertPressed)]) {
        [self.delegate cf_mqtt_sslParams_device_clientCertPressed];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(cf_mqtt_sslParams_device_sslStatusChanged:)]) {
        [self.delegate cf_mqtt_sslParams_device_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(cf_mqtt_sslParams_device_certificateChanged:)]) {
            [self.delegate cf_mqtt_sslParams_device_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKCFMQTTSSLForDeviceViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCFMQTTSSLForDeviceViewModel.class]) {
        return;
    }
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"CommureGWThree", @"MKCFMQTTSSLForDeviceView", @"cf_switchSelectedIcon.png") : LOADICON(@"CommureGWThree", @"MKCFMQTTSSLForDeviceView", @"cf_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"CommureGWThree", @"MKCFMQTTSSLForDeviceView", @"cf_switchUnselectedIcon.png") forState:UIControlStateNormal];
