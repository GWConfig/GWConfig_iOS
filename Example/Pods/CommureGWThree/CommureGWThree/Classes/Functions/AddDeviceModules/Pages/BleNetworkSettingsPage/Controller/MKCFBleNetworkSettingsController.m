//
//  MKCFBleNetworkSettingsController.m
...
@interface MKCFBleNetworkSettingsController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextButtonCellDelegate,
MKTextFieldCellDelegate,
mk_textSwitchCellDelegate,
MKCFBleWifiSettingsCertCellDelegate,
...
@property (nonatomic, strong)MKCFBleNetworkSettingsModel *dataModel;

@end

@implementation MKCFBleNetworkSettingsController

- (void)dealloc {
    NSLog(@"MKCFBleNetworkSettingsController销毁");
}
...
#pragma mark - MKCFBleWifiSettingsCertCellDelegate
...
- (void)mk_certSelectedMethod:(mk_certListPageType)certType certName:(NSString *)certName {
    if (certType == mk_caCertSelPage) {
        ...
        MKCFBleWifiSettingsCertCellModel *cellModel = self.section9List[0];
    ...
    if (certType == mk_clientCertSelPage) {
        ...
        MKCFBleWifiSettingsCertCellModel *cellModel = self.section10List[0];
        cellModel.fileName = certName;
    ...
    if (certType == mk_clientKeySelPage) {
        ...
        MKCFBleWifiSettingsCertCellModel *cellModel = self.section11List[0];
        cellModel.fileName = certName;
...
- (void)saveDataToDevice {
    ...
    [self.dataModel configDataWithSucBlock:^{
        ...
        [MKCFDeviceMQTTParamsModel shared].wifiConfig = YES;
    } failedBlock:^(NSError * _Nonnull error) {
        @strongify(self);
        [[MKHudManager share] hide];
        [self.view showCentralToast:error.userInfo[@"errorInfo"]];
        [MKCFDeviceMQTTParamsModel shared].wifiConfig = NO;
    }];
}
...
- (UITableViewCell *)cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 9) {
        //CA certificate.
        MKCFBleWifiSettingsCertCell *cell = [MKCFBleWifiSettingsCertCell initCellWithTableView:self.tableView];
...
- (void)loadSection9Datas {
    MKCFBleWifiSettingsCertCellModel *cellModel = [[MKCFBleWifiSettingsCertCellModel alloc] init];
    cellModel.index = 0;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"CommureGWThree", @"MKCFBleNetworkSettingsController", @"cf_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCFBleNetworkSettingsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCFBleNetworkSettingsModel alloc] init];
    }
    return _dataModel;
}
