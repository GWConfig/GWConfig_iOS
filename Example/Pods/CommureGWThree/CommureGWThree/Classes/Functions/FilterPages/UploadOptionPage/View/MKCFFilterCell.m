...
@implementation MKCFFilterCellModel
@end

@interface MKCFFilterCell ()
...
@implementation MKCFFilterCell

+ (MKCFFilterCell *)initCellWithTableView:(UITableView *)tableView {
    MKCFFilterCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCFFilterCellIdenty"];
    if (!cell) {
        cell = [[MKCFFilterCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCFFilterCellIdenty"];
    }
    return cell;
}
...
- (void)rightButtonPressed {
    ...
    [pickView showPickViewWithDataList:self.dataModel.dataList selectedRow:row block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(cf_filterValueChanged:index:)]) {
            [self.delegate cf_filterValueChanged:currentRow index:self.dataModel.index];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKCFFilterCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCFFilterCellModel.class]) {
        return;
    }
