//
//  MKCFAdvParamsConfigController.m
...
@interface MKCFAdvParamsConfigController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCFAdvParamsConfigModel *dataModel;

@end

@implementation MKCFAdvParamsConfigController

- (void)dealloc {
    NSLog(@"MKCFAdvParamsConfigController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"CommureGWThree", @"MKCFAdvParamsConfigController", @"cf_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCFAdvParamsConfigModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCFAdvParamsConfigModel alloc] init];
