...
@interface MKCFAdvParamsConfigModel ()
...
@implementation MKCFAdvParamsConfigModel
...
- (BOOL)readConfigParams {
    ...
    [MKCFMQTTInterface cf_readAdvParamsWithType:0 bleMacAddress:self.bleMac macAddress:[MKCFDeviceModeManager shared].macAddress topic:[MKCFDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (BOOL)configConfigParams {
    __block BOOL success = NO;
    [MKCFMQTTInterface cf_configAdvParamsWithType:0 bleMacAddress:self.bleMac interval:([self.configInterval integerValue] * 100) duration:[self.configDuration integerValue] txPower:self.configTxPower macAddress:[MKCFDeviceModeManager shared].macAddress topic:[MKCFDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (BOOL)readNormalParams {
    __block BOOL success = NO;
    [MKCFMQTTInterface cf_readAdvParamsWithType:1 bleMacAddress:self.bleMac macAddress:[MKCFDeviceModeManager shared].macAddress topic:[MKCFDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        if ([returnData[@"data"][@"result_code"] integerValue] == 0) {
            //成功
            success = YES;
            NSInteger value = [returnData[@"data"][@"adv_interval"] integerValue] / 100;
...
- (BOOL)configNormalParams {
    __block BOOL success = NO;
    [MKCFMQTTInterface cf_configAdvParamsWithType:1 bleMacAddress:self.bleMac interval:([self.normalInterval integerValue] * 100) duration:[self.normalDuration integerValue] txPower:self.normalTxPower macAddress:[MKCFDeviceModeManager shared].macAddress topic:[MKCFDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = [returnData[@"data"][@"result_code"] integerValue] == 0;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readTriggerParams {
    __block BOOL success = NO;
    [MKCFMQTTInterface cf_readAdvParamsWithType:2 bleMacAddress:self.bleMac macAddress:[MKCFDeviceModeManager shared].macAddress topic:[MKCFDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        if ([returnData[@"data"][@"result_code"] integerValue] == 0) {
            //成功
            success = YES;
            NSInteger value = [returnData[@"data"][@"adv_interval"] integerValue] / 100;
...
- (BOOL)configTriggerParams {
    __block BOOL success = NO;
    [MKCFMQTTInterface cf_configAdvParamsWithType:2 bleMacAddress:self.bleMac interval:([self.triggerInterval integerValue] * 100) duration:[self.triggerDuration integerValue] txPower:self.triggerTxPower macAddress:[MKCFDeviceModeManager shared].macAddress topic:[MKCFDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = [returnData[@"data"][@"result_code"] integerValue] == 0;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
