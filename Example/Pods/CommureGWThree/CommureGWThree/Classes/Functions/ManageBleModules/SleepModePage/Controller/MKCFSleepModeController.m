...
@interface MKCFSleepModeController ()

@property (nonatomic, strong)MKTextField *textField;

@end

@implementation MKCFSleepModeController

- (void)dealloc {
    NSLog(@"MKCFSleepModeController销毁");
}
...
- (void)readDataFromDevice {
    ...
    [MKCFMQTTInterface cf_readSleepModeParametersWithBleMacAddress:self.bleMac macAddress:[MKCFDeviceModeManager shared].macAddress topic:[MKCFDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)configDatas {
    ...
    [MKCFMQTTInterface cf_configSleepModeParametersWithBleMacAddress:self.bleMac time:value macAddress:[MKCFDeviceModeManager shared].macAddress topic:[MKCFDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
            [self.view showCentralToast:@"Read Failed"];
            return;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"CommureGWThree", @"MKCFSleepModeController", @"cf_saveIcon.png") forState:UIControlStateNormal];
