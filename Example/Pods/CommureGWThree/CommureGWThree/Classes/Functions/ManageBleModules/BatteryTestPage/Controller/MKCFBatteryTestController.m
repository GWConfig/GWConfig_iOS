...
@interface MKCFBatteryTestController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCFBatteryTestModel *dataModel;

@end

@implementation MKCFBatteryTestController

- (void)dealloc {
    NSLog(@"MKCFBatteryTestController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"CommureGWThree", @"MKCFBatteryTestController", @"cf_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCFBatteryTestModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCFBatteryTestModel alloc] init];
