...
@interface MKCFButtonLogController ()<MFMailComposeViewControllerDelegate,
MKCFButtonLogHeaderDelegate>

@property (nonatomic, strong)MKCFButtonLogHeader *headerView;
...
@implementation MKCFButtonLogController

- (void)dealloc {
    NSLog(@"MKCFButtonLogController销毁");
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveButtonLog:)
                                                 name:MKCFReceiveButtonLogNotification
                                               object:nil];
}
...
#pragma mark - MKCFButtonLogHeaderDelegate
- (void)cf_syncButtonPressed {
    self.textView.text = @"";
    [[MKHudManager share] showHUDWithTitle:@"Reading..." inView:self.view isPenetration:NO];
    [MKCFMQTTInterface cf_readButtonLogWithBleMacAddress:self.bleMac macAddress:[MKCFDeviceModeManager shared].macAddress topic:[MKCFDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)cf_deleteButtonPressed {
    ...
    [alertView showAlertWithTitle:@"Warning!" message:msg notificationName:@"mk_cf_needDismissAlert"];
}
...
- (void)sendClearCommand {
    [[MKHudManager share] showHUDWithTitle:@"Reading..." inView:self.view isPenetration:NO];
    [MKCFMQTTInterface cf_clearButtonLogWithBleMacAddress:self.bleMac macAddress:[MKCFDeviceModeManager shared].macAddress topic:[MKCFDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        self.textView.text = @"";
        [[MKHudManager share] hide];
        [self.view showCentralToast:@"Setup succeed!"];
    } failedBlock:^(NSError * _Nonnull error) {
...
#pragma mark - getter
- (MKCFButtonLogHeader *)headerView {
    if (!_headerView) {
        _headerView = [[MKCFButtonLogHeader alloc] init];
