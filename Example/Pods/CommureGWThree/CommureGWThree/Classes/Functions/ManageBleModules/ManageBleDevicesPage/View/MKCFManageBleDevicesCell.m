...
@implementation MKCFManageBleDevicesCellModel
@end

@interface MKCFManageBleDevicesCell ()
...
@implementation MKCFManageBleDevicesCell

+ (MKCFManageBleDevicesCell *)initCellWithTableView:(UITableView *)tableView {
    MKCFManageBleDevicesCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCFManageBleDevicesCellIdenty"];
    if (!cell) {
        cell = [[MKCFManageBleDevicesCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCFManageBleDevicesCellIdenty"];
    }
    return cell;
}
...
- (void)connectButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cf_manageBleDevicesCellConnectButtonPressed:typeCode:)]) {
        [self.delegate cf_manageBleDevicesCellConnectButtonPressed:SafeStr(self.dataModel.macAddress)
                                                          typeCode:self.dataModel.typeCode];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKCFManageBleDevicesCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCFManageBleDevicesCellModel.class]) {
        return;
    }
