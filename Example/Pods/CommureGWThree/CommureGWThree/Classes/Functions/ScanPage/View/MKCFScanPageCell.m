...
@implementation MKCFScanPageCell

+ (MKCFScanPageCell *)initCellWithTableView:(UITableView *)tableView {
    MKCFScanPageCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCFScanPageCellIdenty"];
    if (!cell) {
        cell = [[MKCFScanPageCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCFScanPageCellIdenty"];
    }
    return cell;
}
...
#pragma mark - setter
- (void)setDataModel:(MKCFScanPageModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCFScanPageModel.class]) {
        return;
    }
