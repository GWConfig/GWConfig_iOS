...
static NSString *const localPasswordKey = @"mk_cf_passwordKey";

static NSTimeInterval const kRefreshInterval = 0.5f;

@interface MKCFScanPageController ()<UITableViewDelegate,
UITableViewDataSource,
mk_cf_centralManagerScanDelegate>
...
@implementation MKCFScanPageController

- (void)dealloc {
    NSLog(@"MKCFScanPageController销毁");
    //移除runloop的监听
    CFRunLoopRemoveObserver(CFRunLoopGetCurrent(), self.observerRef, kCFRunLoopCommonModes);
    [[MKCFCentralManager shared] stopScan];
    [MKCFCentralManager sharedDealloc];
}
...
- (void)viewDidLoad {
    ...
    [MKCFCentralManager shared].delegate = self;
}

#pragma mark - super method
- (void)rightButtonMethod {
    if ([MKCFCentralManager shared].centralStatus != mk_cf_centralManagerStatusEnable) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    MKCFScanPageCell *cell = [MKCFScanPageCell initCellWithTableView:tableView];
    cell.dataModel = self.dataList[indexPath.row];
    return cell;
}

#pragma mark - mk_cf_centralManagerScanDelegate

- (void)mk_cf_receiveDevice:(NSDictionary *)deviceModel {
    MKCFScanPageModel *dataModel = [MKCFScanPageModel mk_modelWithJSON:deviceModel];
...
- (void)addConfiguredDevice {
    MKCFConfiguredGatewayController *vc = [[MKCFConfiguredGatewayController alloc] init];
...
- (void)scanTimerRun{
    ...
    [[MKCFCentralManager shared] startScan];
...
- (void)updateDataWithDeviceModel:(MKCFScanPageModel *)deviceModel{
...
#pragma mark - 连接部分
- (void)connectDeviceWithModel:(MKCFScanPageModel *)deviceModel {
    //停止扫描
    [self.refreshIcon.layer removeAnimationForKey:@"mk_refreshAnimationKey"];
    [[MKCFCentralManager shared] stopScan];
    if (self.scanTimer) {
...
- (void)connectDevice:(MKCFScanPageModel *)deviecModel {
    ...
    [[MKCFCentralManager shared] connectPeripheral:deviecModel.peripheral password:password sucBlock:^(CBPeripheral * _Nonnull peripheral) {
...
- (void)pushMQTTForDevicePage:(NSString *)deviceType {
    ...
    //MK110
    MKCFBleMqttConfigSelectController *vc = [[MKCFBleMqttConfigSelectController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}
...
- (UIImageView *)refreshIcon {
    if (!_refreshIcon) {
        ...
        _refreshIcon.image = LOADICON(@"CommureGWThree", @"MKCFScanPageController", @"cf_scanRefresh.png");
    }
    return _refreshIcon;
}
