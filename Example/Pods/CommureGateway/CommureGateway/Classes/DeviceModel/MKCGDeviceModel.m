...
NSString *const MKCGDeviceModelOfflineNotification = @"MKCGDeviceModelOfflineNotification";

@interface MKCGDeviceModel ()
...
@implementation MKCGDeviceModel

- (void)dealloc{
    NSLog(@"MKCGDeviceModel销毁");
}

#pragma mark - public method

- (void)startStateMonitoringTimer{
    if (self.onLineState == MKCGDeviceModelStateOffline) {
        return;
    }
    ...
    dispatch_source_set_event_handler(self.receiveTimer, ^{
        ...
        if (self.receiveTimerCount >= 62.f) {
            ...
            self.receiveTimerCount = 0;
            self.onLineState = MKCGDeviceModelStateOffline;
            dispatch_async(dispatch_get_main_queue(), ^{
                [[NSNotificationCenter defaultCenter] postNotificationName:MKCGDeviceModelOfflineNotification
                                                                    object:nil
                                                                  userInfo:@{@"macAddress":self.macAddress}];
                if ([self.delegate respondsToSelector:@selector(cg_deviceOfflineWithMacAddress:)]) {
                    [self.delegate cg_deviceOfflineWithMacAddress:self.macAddress];
                }
            });
            return ;
        }
