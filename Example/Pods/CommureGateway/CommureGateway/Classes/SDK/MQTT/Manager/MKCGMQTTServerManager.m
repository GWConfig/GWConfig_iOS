...
static MKCGMQTTServerManager *manager = nil;
static dispatch_once_t onceToken;

@interface NSObject (MKCGMQTTServerManager)

@end

@implementation NSObject (MKCGMQTTServerManager)
...
@interface MKCGMQTTServerManager ()

@property (nonatomic, assign)MKCGMQTTSessionManagerState state;

@property (nonatomic, strong)MKCGServerParamsModel *paramsModel;

@property (nonatomic, strong)NSMutableArray <id <MKCGServerManagerProtocol>>*managerList;

@property (nonatomic, strong)NSMutableDictionary *subscriptions;

@end

@implementation MKCGMQTTServerManager
...
+ (MKCGMQTTServerManager *)shared {
    dispatch_once(&onceToken, ^{
        if (!manager) {
            manager = [MKCGMQTTServerManager new];
        }
    });
    return manager;
}
...
- (void)sessionManager:(MQTTSessionManager *)sessionManager
    ...
    @synchronized (self.managerList) {
        for (id <MKCGServerManagerProtocol>protocol in self.managerList) {
            if ([protocol respondsToSelector:@selector(cg_didReceiveMessage:onTopic:)]) {
                [protocol cg_didReceiveMessage:dataDic onTopic:topic];
            }
        }
    }
}
...
- (void)sessionManager:(MQTTSessionManager *)sessionManager didChangeState:(MKMQTTSessionManagerState)newState {
    ...
    @synchronized (self.managerList) {
        for (id <MKCGServerManagerProtocol>protocol in self.managerList) {
            if ([protocol respondsToSelector:@selector(cg_didChangeState:)]) {
                [protocol cg_didChangeState:self.state];
            }
        }
    }
}
...
- (void)networkStateChanged{
    ...
    if (![[MKNetworkManager sharedInstance] currentNetworkAvailable]) {
        ...
        [self sessionManager:manager didChangeState:MKCGMQTTSessionManagerStateStarting];
        return;
    }
...
- (void)loadDataManager:(nonnull id <MKCGServerManagerProtocol>)dataManager {
    @synchronized (self.managerList) {
        if (dataManager
            && [dataManager conformsToProtocol:@protocol(MKCGServerManagerProtocol)]
...
- (void)loadLogFile {
    ...
    NSString *recordDateString = [[NSUserDefaults standardUserDefaults] objectForKey:@"mk_cg_mqtt_recordDate"];
    if (!ValidStr(recordDateString)) {
        //如果第一次存，则把当天的日期写入
        [[NSUserDefaults standardUserDefaults] setObject:[dateFormatter stringFromDate:[NSDate date]] forKey:@"mk_cg_mqtt_recordDate"];
        return;
    }
    //写入当天日期，用来做比较
    [[NSUserDefaults standardUserDefaults] setObject:[dateFormatter stringFromDate:[NSDate date]] forKey:@"mk_cg_mqtt_recordDate"];
...
#pragma mark - getter
- (MKCGServerParamsModel *)paramsModel {
    if (!_paramsModel) {
        _paramsModel = [[MKCGServerParamsModel alloc] init];
    }
    return _paramsModel;
}

- (NSMutableArray<id<MKCGServerManagerProtocol>> *)managerList {
    if (!_managerList) {
