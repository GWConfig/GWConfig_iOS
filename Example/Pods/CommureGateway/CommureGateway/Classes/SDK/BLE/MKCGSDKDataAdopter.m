...
@implementation MKCGSDKDataAdopter

+ (NSString *)fetchWifiSecurity:(mk_cg_wifiSecurity)security {
    switch (security) {
        case mk_cg_wifiSecurity_personal:
            return @"00";
        case mk_cg_wifiSecurity_enterprise:
            return @"01";
    }
}
...
+ (NSString *)fetchWifiEapType:(mk_cg_eapType)eapType {
    switch (eapType) {
        case mk_cg_eapType_peap_mschapv2:
            return @"00";
        case mk_cg_eapType_ttls_mschapv2:
            return @"01";
        case mk_cg_eapType_tls:
            return @"02";
    }
}
...
+ (NSString *)fetchConnectModeString:(mk_cg_connectMode)mode {
    switch (mode) {
        case mk_cg_connectMode_TCP:
            return @"00";
        case mk_cg_connectMode_CASignedServerCertificate:
            return @"01";
        case mk_cg_connectMode_CACertificate:
            return @"02";
        case mk_cg_connectMode_SelfSignedCertificates:
            return @"03";
    }
}
...
+ (NSString *)fetchMqttServerQosMode:(mk_cg_mqttServerQosMode)mode {
    switch (mode) {
        case mk_cg_mqttQosLevelAtMostOnce:
            return @"00";
        case mk_cg_mqttQosLevelAtLeastOnce:
            return @"01";
        case mk_cg_mqttQosLevelExactlyOnce:
            return @"02";
    }
}
