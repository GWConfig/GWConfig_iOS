...
@implementation MKCGDeviceListCell

+ (MKCGDeviceListCell *)initCellWithTableView:(UITableView *)tableView {
    MKCGDeviceListCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCGDeviceListCellIdenty"];
    if (!cell) {
        cell = [[MKCGDeviceListCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCGDeviceListCellIdenty"];
    }
    return cell;
}
...
- (void)longPressEventAction:(UILongPressGestureRecognizer *)longPress {
    if (longPress.state == UIGestureRecognizerStateBegan && [self.delegate respondsToSelector:@selector(cg_cellDeleteButtonPressed:)]) {
        [self.delegate cg_cellDeleteButtonPressed:self.indexPath.row];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKCGDeviceListModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCGDeviceModel.class]) {
        return;
    }
    self.deviceNameLabel.text = SafeStr(_dataModel.deviceName);
    self.stateLabel.text = (_dataModel.onLineState == MKCGDeviceModelStateOnline ? @"Online" : @"Offline");
    self.stateLabel.textColor = (_dataModel.onLineState == MKCGDeviceModelStateOnline ? NAVBAR_COLOR_MACROS : UIColorFromRGB(0xcccccc));
    self.macLabel.text = SafeStr(_dataModel.macAddress);
    if (dataModel.onLineState == MKCGDeviceModelStateOffline) {
        //设备离线
        self.wifiIcon.image = LOADICON(@"CommureGateway", @"MKCGDeviceListCell", @"cg_offline_wifi.png");
        return;
    }
    //设备在线
    if (_dataModel.wifiLevel >= -50 && _dataModel.wifiLevel <= 0) {
        //Good
        self.wifiIcon.image = LOADICON(@"CommureGateway", @"MKCGDeviceListCell", @"cg_good_wifi.png");
        return;
    }
    if (_dataModel.wifiLevel >= -65 && _dataModel.wifiLevel < 50) {
        //Medium
        self.wifiIcon.image = LOADICON(@"CommureGateway", @"MKCGDeviceListCell", @"cg_medium_wifi.png");
        return;
    }
    if (_dataModel.wifiLevel < -65) {
        //Poor
        self.wifiIcon.image = LOADICON(@"CommureGateway", @"MKCGDeviceListCell", @"cg_poor_wifi.png");
        return;
    }
}
...
- (UIImageView *)wifiIcon {
    if (!_wifiIcon) {
        ...
        _wifiIcon.image = LOADICON(@"CommureGateway", @"MKCGDeviceListCell", @"cg_offline_wifi.png");
    }
    return _wifiIcon;
}
...
- (UIImageView *)rightIcon {
    if (!_rightIcon) {
        ...
        _rightIcon.image = LOADICON(@"CommureGateway", @"MKCGDeviceListCell", @"cg_goNextButton.png");
    }
    return _rightIcon;
}
