...
@implementation MKCGMQTTSSLForDeviceViewModel
@end

@interface MKCGMQTTSSLForDeviceView ()<MKMQTTSSLCertificateViewDelegate>
...
@implementation MKCGMQTTSSLForDeviceView
...
- (void)mk_fileSelectedButtonPressed:(NSInteger)index {
    if (index == 0) {
        //CA File
        if ([self.delegate respondsToSelector:@selector(cg_mqtt_sslParams_device_caFilePressed)]) {
            [self.delegate cg_mqtt_sslParams_device_caFilePressed];
        }
        return;
    }
    if (index == 1) {
        //Client Key File
        if ([self.delegate respondsToSelector:@selector(cg_mqtt_sslParams_device_clientKeyPressed)]) {
            [self.delegate cg_mqtt_sslParams_device_clientKeyPressed];
        }
        return;
    }
    //Client Cert File
    if ([self.delegate respondsToSelector:@selector(cg_mqtt_sslParams_device_clientCertPressed)]) {
        [self.delegate cg_mqtt_sslParams_device_clientCertPressed];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(cg_mqtt_sslParams_device_sslStatusChanged:)]) {
        [self.delegate cg_mqtt_sslParams_device_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(cg_mqtt_sslParams_device_certificateChanged:)]) {
            [self.delegate cg_mqtt_sslParams_device_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKCGMQTTSSLForDeviceViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCGMQTTSSLForDeviceViewModel.class]) {
        return;
    }
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"CommureGateway", @"MKCGMQTTSSLForDeviceView", @"cg_switchSelectedIcon.png") : LOADICON(@"CommureGateway", @"MKCGMQTTSSLForDeviceView", @"cg_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"CommureGateway", @"MKCGMQTTSSLForDeviceView", @"cg_switchUnselectedIcon.png") forState:UIControlStateNormal];
