//
//  MKCGServerForDeviceController.m
...
@interface MKCGServerForDeviceController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKCGServerConfigDeviceFooterViewDelegate,
...
@property (nonatomic, strong)MKCGServerForDeviceModel *dataModel;

@property (nonatomic, strong)MKCGServerConfigDeviceFooterView *sslParamsView;

@property (nonatomic, strong)MKCGServerConfigDeviceFooterViewModel *sslParamsModel;

@property (nonatomic, strong)UIView *footerView;

@end

@implementation MKCGServerForDeviceController

- (void)dealloc {
    NSLog(@"MKCGServerForDeviceController销毁");
}
...
#pragma mark - MKCGServerConfigDeviceFooterViewDelegate
/// 用户改变了开关状态
/// @param isOn isOn
/// @param statusID 0:cleanSession   1:ssl
- (void)cg_mqtt_serverForDevice_switchStatusChanged:(BOOL)isOn statusID:(NSInteger)statusID {
    if (statusID == 0) {
        //cleanSession
...
- (void)cg_mqtt_serverForDevice_textFieldValueChanged:(NSString *)text textID:(NSInteger)textID {
    if (textID == 0) {
        //keepAlive
...
- (void)cg_mqtt_serverForDevice_qosChanged:(NSInteger)qos qosID:(NSInteger)qosID{
    if (qosID == 0) {
        //qos
...
- (void)cg_mqtt_serverForDevice_certificateChanged:(NSInteger)certificate {
...
- (void)cg_mqtt_serverForDevice_fileButtonPressed:(NSInteger)fileType {
    if (fileType == 0) {
        //caFile
...
- (void)cg_selectedServerParams:(NSString *)fileName {
    [MKCGExcelDataManager parseDeviceExcel:fileName
                                   sucBlock:^(NSDictionary * _Nonnull returnData) {
        MKCGServerForDeviceModel *model = [MKCGServerForDeviceModel mk_modelWithJSON:returnData];
...
- (void)saveDataToDevice {
    ...
    [self.dataModel configDataWithSucBlock:^{
        ...
        [MKCGDeviceMQTTParamsModel shared].mqttConfig = YES;
        [MKCGDeviceMQTTParamsModel shared].deviceModel.clientID = self.dataModel.clientID;
        [MKCGDeviceMQTTParamsModel shared].deviceModel.deviceName = self.dataModel.deviceName;
        [MKCGDeviceMQTTParamsModel shared].deviceModel.subscribedTopic = self.dataModel.subscribeTopic;
        [MKCGDeviceMQTTParamsModel shared].deviceModel.publishedTopic = self.dataModel.publishTopic;
        [MKCGDeviceMQTTParamsModel shared].deviceModel.macAddress = self.dataModel.macAddress;
    } failedBlock:^(NSError * _Nonnull error) {
        @strongify(self);
        [[MKHudManager share] hide];
        [self.view showCentralToast:error.userInfo[@"errorInfo"]];
        [MKCGDeviceMQTTParamsModel shared].deviceModel.clientID = @"";
        [MKCGDeviceMQTTParamsModel shared].deviceModel.deviceName = @"";
        [MKCGDeviceMQTTParamsModel shared].deviceModel.subscribedTopic = @"";
        [MKCGDeviceMQTTParamsModel shared].deviceModel.publishedTopic = @"";
        [MKCGDeviceMQTTParamsModel shared].deviceModel.macAddress = @"";
    }];
}
...
//    }
    ...
    [MKCGExcelDataManager exportDeviceExcel:self.dataModel
                                    sucBlock:^{
        [[MKHudManager share] hide];
        [self sharedExcel];
    }
...
- (void)clearAllParams {
    MKCGServerForDeviceModel *emptyModel = [[MKCGServerForDeviceModel alloc] init];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"CommureGateway", @"MKCGServerForDeviceController", @"cg_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCGServerForDeviceModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCGServerForDeviceModel alloc] init];
    }
    return _dataModel;
}
...
- (MKCGServerConfigDeviceFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKCGServerConfigDeviceFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 380.f)];
...
- (MKCGServerConfigDeviceFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKCGServerConfigDeviceFooterViewModel alloc] init];
    }
