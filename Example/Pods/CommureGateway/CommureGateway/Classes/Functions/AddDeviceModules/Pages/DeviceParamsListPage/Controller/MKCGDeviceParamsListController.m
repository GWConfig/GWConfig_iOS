//
//  MKCGDeviceParamsListController.m
...
@interface MKCGDeviceParamsListController ()<UITableViewDelegate, UITableViewDataSource>
...
@property (nonatomic, strong)MKCGDeviceParamListModel *dataModel;

@end

@implementation MKCGDeviceParamsListController

- (void)dealloc {
    NSLog(@"MKCGDeviceParamsListController销毁");
...
- (void)viewDidLoad {
    ...
    [MKCGDeviceMQTTParamsModel shared].deviceModel.deviceType = @"00";
...
- (void)leftButtonMethod {
    ...
    [[MKCGCentralManager shared] disconnect];
    [MKCGDeviceMQTTParamsModel sharedDealloc];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0) {
        //WIFI Settings
        MKCGBleWifiSettingsController *vc = [[MKCGBleWifiSettingsController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 1) {
        //MQTT settings
        MKCGServerForDeviceController *vc = [[MKCGServerForDeviceController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 1 && indexPath.row == 0) {
        //Network Settings
        MKCGBleNetworkSettingsController *vc = [[MKCGBleNetworkSettingsController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 1 && indexPath.row == 1) {
        //NTP & Timezone
        MKCGBleNTPTimezoneController *vc = [[MKCGBleNTPTimezoneController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 1 && indexPath.row == 2) {
        //Device Information
        MKCGBleDeviceInfoController *vc = [[MKCGBleDeviceInfoController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
}
...
- (void)receiveDeviceOnline:(NSNotification *)note {
    ...
    if (!ValidDict(user) || !ValidStr(user[@"macAddress"]) || ![[MKCGDeviceMQTTParamsModel shared].deviceModel.macAddress isEqualToString:user[@"macAddress"]]) {
        return;
    }
...
- (void)connectButtonPressed {
    ...
    if (![MKCGDeviceMQTTParamsModel shared].wifiConfig || ![MKCGDeviceMQTTParamsModel shared].mqttConfig) {
...
- (void)startMqttProcess {
    [[NSNotificationCenter defaultCenter] removeObserver:self
                                                    name:mk_cg_peripheralConnectStateChangedNotification
                                                  object:nil];
    NSString *topic = [MKCGDeviceMQTTParamsModel shared].deviceModel.publishedTopic;
    [self.progressView show];
    [[MKCGMQTTDataManager shared] subscriptions:@[topic]];
    [[NSNotificationCenter defaultCenter] addObserver:self
...
- (void)connectSuccess {
    ...
    deviceModel.clientID = [MKCGDeviceMQTTParamsModel shared].deviceModel.clientID;
    deviceModel.subscribedTopic = [MKCGDeviceMQTTParamsModel shared].deviceModel.subscribedTopic;
    deviceModel.publishedTopic = [MKCGDeviceMQTTParamsModel shared].deviceModel.publishedTopic;
    deviceModel.macAddress = [MKCGDeviceMQTTParamsModel shared].deviceModel.macAddress;
    deviceModel.deviceName = [MKCGDeviceMQTTParamsModel shared].deviceModel.deviceName;
    
        
    MKCGConnectSuccessController *vc = [[MKCGConnectSuccessController alloc] init];
...
- (UIView *)footerView {
    if (!_footerView) {
        ...
        retainButton.hidden = ![MKCGDeviceMQTTParamsModel shared].cloud;
...
- (MKCGDeviceParamListModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCGDeviceParamListModel alloc] init];
    }
    return _dataModel;
}
