//
//  MKCGBleNetworkSettingsController.m
...
@interface MKCGBleNetworkSettingsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCGBleNetworkSettingsModel *dataModel;

@end

@implementation MKCGBleNetworkSettingsController

- (void)dealloc {
    NSLog(@"MKCGBleNetworkSettingsController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"CommureGateway", @"MKCGBleNetworkSettingsController", @"cg_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCGBleNetworkSettingsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCGBleNetworkSettingsModel alloc] init];
    }
    return _dataModel;
}
