//
//  MKCGServerConfigDeviceFooterView.m
...
@property (nonatomic, strong)MKCGMQTTSSLForDeviceView *sslView;

@property (nonatomic, assign)NSInteger index;

@end

@implementation MKCGServerConfigDeviceFooterView
...
- (void)mk_mqtt_generalParams_cleanSessionStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(cg_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate cg_mqtt_serverForDevice_switchStatusChanged:isOn statusID:0];
    }
}

- (void)mk_mqtt_generalParams_qosChanged:(NSInteger)qos {
    if ([self.delegate respondsToSelector:@selector(cg_mqtt_serverForDevice_qosChanged:qosID:)]) {
        [self.delegate cg_mqtt_serverForDevice_qosChanged:qos qosID:0];
    }
}

- (void)mk_mqtt_generalParams_KeepAliveChanged:(NSString *)keepAlive {
    if ([self.delegate respondsToSelector:@selector(cg_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate cg_mqtt_serverForDevice_textFieldValueChanged:keepAlive textID:0];
    }
}
...
- (void)mk_mqtt_userCredentials_userNameChanged:(NSString *)userName {
    if ([self.delegate respondsToSelector:@selector(cg_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate cg_mqtt_serverForDevice_textFieldValueChanged:userName textID:1];
    }
}

- (void)mk_mqtt_userCredentials_passwordChanged:(NSString *)password {
    if ([self.delegate respondsToSelector:@selector(cg_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate cg_mqtt_serverForDevice_textFieldValueChanged:password textID:2];
    }
}

#pragma mark - MKCGMQTTSSLForDeviceViewDelegate
- (void)cg_mqtt_sslParams_device_sslStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(cg_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate cg_mqtt_serverForDevice_switchStatusChanged:isOn statusID:1];
    }
}

/// 用户选择了加密方式
/// @param certificate 0:CA certificate     1:Self signed certificates
- (void)cg_mqtt_sslParams_device_certificateChanged:(NSInteger)certificate {
    if ([self.delegate respondsToSelector:@selector(cg_mqtt_serverForDevice_certificateChanged:)]) {
        [self.delegate cg_mqtt_serverForDevice_certificateChanged:certificate];
    }
}

/// 用户点击选择了caFaile按钮
- (void)cg_mqtt_sslParams_device_caFilePressed {
    if ([self.delegate respondsToSelector:@selector(cg_mqtt_serverForDevice_fileButtonPressed:)]) {
        [self.delegate cg_mqtt_serverForDevice_fileButtonPressed:0];
    }
}

/// 用户点击选择了Client Key按钮
- (void)cg_mqtt_sslParams_device_clientKeyPressed {
    if ([self.delegate respondsToSelector:@selector(cg_mqtt_serverForDevice_fileButtonPressed:)]) {
        [self.delegate cg_mqtt_serverForDevice_fileButtonPressed:1];
    }
}

/// 用户点击了Client Cert File按钮
- (void)cg_mqtt_sslParams_device_clientCertPressed {
    if ([self.delegate respondsToSelector:@selector(cg_mqtt_serverForDevice_fileButtonPressed:)]) {
        [self.delegate cg_mqtt_serverForDevice_fileButtonPressed:2];
    }
}
...
- (CGFloat)fetchHeightWithSSLStatus:(BOOL)isOn
    ...
    if (certificate == 1) {
        //CA 证书
        return defaultScrollViewHeight + caSize.height;
    }
    if (certificate == 2) {
        ...
        CGSize clientCertSize = [NSString sizeWithText:clientCertName
                                               andFont:MKFont(13.f)
                                            andMaxSize:CGSizeMake(self.frame.size.width - 2 * 15.f - 120.f -  2 * 10.f - 40.f, MAXFLOAT)];
        return defaultScrollViewHeight + caSize.height + clientKeySize.height + clientCertSize.height;
    }
    return defaultScrollViewHeight;
}

#pragma mark - setter
- (void)setDataModel:(MKCGServerConfigDeviceFooterViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCGServerConfigDeviceFooterViewModel.class]) {
        return;
    }
    ...
    MKCGMQTTSSLForDeviceViewModel *sslModel = [[MKCGMQTTSSLForDeviceViewModel alloc] init];
...
- (MKCGMQTTSSLForDeviceView *)sslView {
    if (!_sslView) {
        _sslView = [[MKCGMQTTSSLForDeviceView alloc] init];
