//
//  MKCGBleWifiSettingsController.m
...
@interface MKCGBleWifiSettingsController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextButtonCellDelegate,
MKTextFieldCellDelegate,
mk_textSwitchCellDelegate,
MKCGBleWifiSettingsCertCellDelegate,
MKCAFileSelectControllerDelegate,
MKCGBleWifiSettingsBandCellDelegate>
...
@property (nonatomic, strong)MKCGBleWifiSettingsModel *dataModel;

@end

@implementation MKCGBleWifiSettingsController

- (void)dealloc {
    NSLog(@"MKCGBleWifiSettingsController销毁");
}
...
#pragma mark - MKCGBleWifiSettingsCertCellDelegate
...
- (void)mk_certSelectedMethod:(mk_certListPageType)certType certName:(NSString *)certName {
    if (certType == mk_caCertSelPage) {
        ...
        MKCGBleWifiSettingsCertCellModel *cellModel = self.section8List[0];
    ...
    if (certType == mk_clientCertSelPage) {
        ...
        MKCGBleWifiSettingsCertCellModel *cellModel = self.section9List[0];
        cellModel.fileName = certName;
    ...
    if (certType == mk_clientKeySelPage) {
        ...
        MKCGBleWifiSettingsCertCellModel *cellModel = self.section10List[0];
        cellModel.fileName = certName;
...
#pragma mark - MKCGBleWifiSettingsBandCellDelegate
- (void)cg_bleWifiSettingsBandCell_countryChanged:(NSInteger)country {
    MKCGBleWifiSettingsBandCellModel *cellModel = self.section11List[0];
...
- (void)saveDataToDevice {
    ...
    [self.dataModel configDataWithSucBlock:^{
        ...
        [MKCGDeviceMQTTParamsModel shared].wifiConfig = YES;
    } failedBlock:^(NSError * _Nonnull error) {
        @strongify(self);
        [[MKHudManager share] hide];
        [self.view showCentralToast:error.userInfo[@"errorInfo"]];
        [MKCGDeviceMQTTParamsModel shared].wifiConfig = NO;
    }];
}
...
- (UITableViewCell *)cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 8) {
        //CA certificate.
        MKCGBleWifiSettingsCertCell *cell = [MKCGBleWifiSettingsCertCell initCellWithTableView:self.tableView];
    ...
    //Country&Band
    MKCGBleWifiSettingsBandCell *cell = [MKCGBleWifiSettingsBandCell initCellWithTableView:self.tableView];
...
- (void)loadSection8Datas {
    MKCGBleWifiSettingsCertCellModel *cellModel = [[MKCGBleWifiSettingsCertCellModel alloc] init];
    cellModel.index = 0;
...
- (void)loadSection11Datas {
    MKCGBleWifiSettingsBandCellModel *cellModel = [[MKCGBleWifiSettingsBandCellModel alloc] init];
    cellModel.country = 0;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"CommureGateway", @"MKCGBleWifiSettingsController", @"cg_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCGBleWifiSettingsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCGBleWifiSettingsModel alloc] init];
    }
    return _dataModel;
}
