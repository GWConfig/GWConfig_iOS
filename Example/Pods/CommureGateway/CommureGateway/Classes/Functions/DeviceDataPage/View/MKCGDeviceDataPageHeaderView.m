//
//  MKCGDeviceDataPageHeaderView.m
...
@implementation MKCGDeviceDataPageHeaderViewModel
@end

@interface MKCGDeviceDataPageManageBleButton : UIControl
...
@implementation MKCGDeviceDataPageManageBleButton
...
- (UIImageView *)rightIcon {
    if (!_rightIcon) {
        ...
        _rightIcon.image = LOADICON(@"CommureGateway", @"MKCGDeviceDataPageManageBleButton", @"cg_goNextButton.png");
    }
    return _rightIcon;
}
...
@interface MKCGDeviceDataPageHeaderView ()
...
@property (nonatomic, strong)MKCGDeviceDataPageManageBleButton *manageBleButton;

@end

@implementation MKCGDeviceDataPageHeaderView
...
- (void)uploadButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cg_updateLoadButtonAction)]) {
        [self.delegate cg_updateLoadButtonAction];
    }
}

- (void)scannerButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cg_scannerStatusChanged:)]) {
        [self.delegate cg_scannerStatusChanged:!self.scannerButton.selected];
    }
}

- (void)manageBleButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cg_manageBleDeviceAction)]) {
        [self.delegate cg_manageBleDeviceAction];
    }
}

- (void)filterTestButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cg_filterTestButtonAction)]) {
        [self.delegate cg_filterTestButtonAction];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKCGDeviceDataPageHeaderViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCGDeviceDataPageHeaderViewModel.class]) {
        return;
    }
    self.scannerButton.selected = _dataModel.isOn;
    UIImage *image = (self.scannerButton.selected ? LOADICON(@"CommureGateway", @"MKCGDeviceDataPageHeaderView", @"cg_switchSelectedIcon.png") : LOADICON(@"CommureGateway", @"MKCGDeviceDataPageHeaderView", @"cg_switchUnselectedIcon.png"));
...
- (MKCGDeviceDataPageManageBleButton *)manageBleButton {
    if (!_manageBleButton) {
        _manageBleButton = [[MKCGDeviceDataPageManageBleButton alloc] init];
