...
@implementation MKCGBatchOtaManager

- (void)dealloc {
    NSLog(@"MKCGBatchOtaManager销毁");
...
- (void)startBatchOta:(NSArray <NSString *>*)macList
   statusChangedBlock:(void (^)(NSString *macAddress,cg_batchOtaStatus status))statusBlock
    ...
    for (NSInteger i = 0; i < macList.count; i ++) {
        ...
        MKCGBatchOtaModel *otaModel = [[MKCGBatchOtaModel alloc] init];
...
- (void)processOtaModel {
    ...
    MKCGBatchOtaModel *otaModel = [self.otaList firstObject];
    @weakify(self);
    [otaModel otaWithResultBlock:^(NSString * _Nonnull macAddress, cg_batchOtaStatus status) {
        @strongify(self);
        self.updateCount = 0;
        if (self.otaBlock) {
            self.otaBlock(macAddress, status);
        }
        if (status != cg_batchOtaStatus_upgrading) {
            //升级结果
