...
@interface MKCGFilterByTagModel ()
...
@implementation MKCGFilterByTagModel
...
- (BOOL)readFilterByTag {
    ...
    [MKCGMQTTInterface cg_readFilterBXPTagWithMacAddress:[MKCGDeviceModeManager shared].macAddress topic:[MKCGDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterByTag:(NSArray <NSString *>*)tagList {
    __block BOOL success = NO;
    [MKCGMQTTInterface cg_configFilterByTag:self.precise reverseFilter:self.reverse tagIDList:tagList macAddress:[MKCGDeviceModeManager shared].macAddress topic:[MKCGDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
