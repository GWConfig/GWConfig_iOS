...
@interface MKCGFilterByMacModel ()
...
@implementation MKCGFilterByMacModel
...
- (BOOL)readFilterByMac {
    ...
    [MKCGMQTTInterface cg_readFilterByMacWithMacAddress:[MKCGDeviceModeManager shared].macAddress topic:[MKCGDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterByMac:(NSArray <NSString *>*)macList {
    __block BOOL success = NO;
    [MKCGMQTTInterface cg_configFilterByMacAddress:macList preciseMatch:self.preciseMatch reverseFilter:self.reverseFilter macAddress:[MKCGDeviceModeManager shared].macAddress topic:[MKCGDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
