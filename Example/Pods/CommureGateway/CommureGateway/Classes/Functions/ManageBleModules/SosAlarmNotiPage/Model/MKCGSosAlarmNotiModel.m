...
#import "MKCGSosAlarmNotiModel.h"
...
@interface MKCGSosAlarmNotiModel ()
...
@implementation MKCGSosAlarmNotiModel
...
- (BOOL)readSosParams {
    ...
    [MKCGMQTTInterface cg_readSosAlarmNotiWithBleMacAddress:self.bleMac macAddress:[MKCGDeviceModeManager shared].macAddress topic:[MKCGDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (BOOL)readDismissSosParams {
    __block BOOL success = NO;
    [MKCGMQTTInterface cg_readDismissSosAlarmNotiWithBleMacAddress:self.bleMac macAddress:[MKCGDeviceModeManager shared].macAddress topic:[MKCGDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        if ([returnData[@"data"][@"result_code"] integerValue] == 0) {
            //成功
            success = YES;
            NSString *ledColor = SafeStr(returnData[@"data"][@"led_color"]);
...
- (BOOL)configDismissSosParams {
    __block BOOL success = NO;
    [MKCGMQTTInterface cg_configDismissSosAlarmNotiWithColor:self.color ledBlinkingInterval:[self.ledInterval integerValue] ledBlinkingDuration:[self.ledDuration integerValue] buzzerBeepingInterval:[self.beepingInterval integerValue] buzzerBeepingDuration:[self.beepingDuration integerValue] bleMacAddress:self.bleMac macAddress:[MKCGDeviceModeManager shared].macAddress topic:[MKCGDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = [returnData[@"data"][@"result_code"] integerValue] == 0;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
