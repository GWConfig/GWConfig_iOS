//
//  MKCGSelfTestTriggerController.m
...
@interface MKCGSelfTestTriggerController ()

@property (nonatomic, strong)MKTextField *textField;

@end

@implementation MKCGSelfTestTriggerController

- (void)dealloc {
    NSLog(@"MKCGSelfTestTriggerController销毁");
}
...
- (void)readDataFromDevice {
    ...
    [MKCGMQTTInterface cg_readSelfTestTriggeredByButtonWithBleMacAddress:self.bleMac macAddress:[MKCGDeviceModeManager shared].macAddress topic:[MKCGDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)configDatas {
    ...
    [MKCGMQTTInterface cg_configSelfTestTriggeredByButton:value bleMac:self.bleMac macAddress:[MKCGDeviceModeManager shared].macAddress topic:[MKCGDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
            [self.view showCentralToast:@"Read Failed"];
            return;
