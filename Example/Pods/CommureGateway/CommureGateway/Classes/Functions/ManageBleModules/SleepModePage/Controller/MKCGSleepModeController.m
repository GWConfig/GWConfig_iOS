...
@interface MKCGSleepModeController ()

@property (nonatomic, strong)MKTextField *textField;

@end

@implementation MKCGSleepModeController

- (void)dealloc {
    NSLog(@"MKCGSleepModeController销毁");
}
...
- (void)readDataFromDevice {
    ...
    [MKCGMQTTInterface cg_readSleepModeParametersWithBleMacAddress:self.bleMac macAddress:[MKCGDeviceModeManager shared].macAddress topic:[MKCGDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)configDatas {
    ...
    [MKCGMQTTInterface cg_configSleepModeParametersWithBleMacAddress:self.bleMac time:value macAddress:[MKCGDeviceModeManager shared].macAddress topic:[MKCGDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
            [self.view showCentralToast:@"Read Failed"];
            return;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"CommureGateway", @"MKCGSleepModeController", @"cg_saveIcon.png") forState:UIControlStateNormal];
