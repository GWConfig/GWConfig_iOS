...
@interface MKCGAccelerometerController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCGAccelerometerModel *dataModel;

@end

@implementation MKCGAccelerometerController

- (void)dealloc {
    NSLog(@"MKCGAccelerometerController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"CommureGateway", @"MKCGAccelerometerController", @"cg_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCGAccelerometerModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCGAccelerometerModel alloc] init];
