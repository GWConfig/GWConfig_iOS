//
//  MKCGButtonPressEffIntervalController.m
...
@interface MKCGButtonPressEffIntervalController ()

@property (nonatomic, strong)MKTextField *textField;

@end

@implementation MKCGButtonPressEffIntervalController

- (void)dealloc {
    NSLog(@"MKCGButtonPressEffIntervalController销毁");
}
...
- (void)readDataFromDevice {
    ...
    [MKCGMQTTInterface cg_readButtonPressEffectiveIntervalWithBleMacAddress:self.bleMac macAddress:[MKCGDeviceModeManager shared].macAddress topic:[MKCGDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)configDatas {
    ...
    [MKCGMQTTInterface cg_configButtonPressEffectiveIntervalWithBleMacAddress:self.bleMac interval:value macAddress:[MKCGDeviceModeManager shared].macAddress topic:[MKCGDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
            [self.view showCentralToast:@"Read Failed"];
            return;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"CommureGateway", @"MKCGButtonPressEffIntervalController", @"cg_saveIcon.png") forState:UIControlStateNormal];
