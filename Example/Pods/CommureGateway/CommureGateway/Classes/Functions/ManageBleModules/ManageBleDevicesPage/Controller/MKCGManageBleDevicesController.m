//
//  MKCGManageBleDevicesController.m
...
@interface MKCGManageBleDevicesController ()<UITableViewDelegate,
UITableViewDataSource,
MKCGManageBleDevicesSearchButtonDelegate,
MKCGManageBleDevicesCellDelegate>

@property (nonatomic, strong)MKBaseTableView *tableView;

@property (nonatomic, strong)MKCGManageBleDevicesSearchButtonModel *buttonModel;

@property (nonatomic, strong)MKCGManageBleDevicesSearchButton *searchButton;
...
@implementation MKCGManageBleDevicesController

- (void)dealloc {
    NSLog(@"MKCGManageBleDevicesController销毁");
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    MKCGManageBleDevicesCell *cell = [MKCGManageBleDevicesCell initCellWithTableView:tableView];
    cell.dataModel = self.dataList[indexPath.row];
    cell.delegate = self;
    return cell;
}

#pragma mark - MKCGManageBleDevicesSearchButtonDelegate
- (void)cg_scanSearchButtonMethod {
    [MKCGManageBleDeviceSearchView showSearchName:self.buttonModel.searchName
...
- (void)cg_scanSearchButtonClearMethod {
...
#pragma mark - MKCGManageBleDevicesCellDelegate
/// 用户点击了链接按钮
/// @param macAddress 目标设备的mac地址
/// @param typeCode 目标设备的设备类型

- (void)cg_manageBleDevicesCellConnectButtonPressed:(NSString *)macAddress typeCode:(NSInteger)typeCode; {
    @weakify(self);
    ...
    [alertView showAlertWithTitle:@"Enter password" message:@"" notificationName:@"mk_cg_needDismissAlert"];
}
...
- (void)addNotes {
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveDeviceDatas:)
                                                 name:MKCGReceiveDeviceDatasNotification
                                               object:nil];
}
...
- (void)updateCellModelWithMac:(NSString *)macAddress
    ...
    MKCGManageBleDevicesCellModel *cellModel = [[MKCGManageBleDevicesCellModel alloc] init];
...
- (void)connectBeaconWithPassword:(NSString *)password bleMac:(NSString *)bleMac {
    ...
    [MKCGMQTTInterface cg_connectBXPButtonWithPassword:password bleMac:bleMac macAddress:[MKCGDeviceModeManager shared].macAddress topic:[MKCGDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (MKCGManageBleDevicesSearchButton *)searchButton {
    if (!_searchButton) {
        _searchButton = [[MKCGManageBleDevicesSearchButton alloc] init];
        _searchButton.delegate = self;
    }
    return _searchButton;
}

- (MKCGManageBleDevicesSearchButtonModel *)buttonModel {
    if (!_buttonModel) {
        _buttonModel = [[MKCGManageBleDevicesSearchButtonModel alloc] init];
