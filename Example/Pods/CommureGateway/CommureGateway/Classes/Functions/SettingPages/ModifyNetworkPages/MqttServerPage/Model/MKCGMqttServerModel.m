...
@implementation MKCGMqttServerModel
...
- (void)updateValue:(MKCGMqttServerModel *)model {
    if (!model || ![model isKindOfClass:MKCGMqttServerModel.class]) {
        return;
    }
...
- (BOOL)configMqttInfos {
    ...
    if ([self.publishTopic isEqualToString:defaultPubTopic]) {
        //用户使用默认的topic
        self.publishTopic = [NSString stringWithFormat:@"%@/%@/%@",[MKCGDeviceModeManager shared].deviceName,self.clientID,@"device_to_app"];
    }
    if ([self.subscribeTopic isEqualToString:defaultSubTopic]) {
        //用户使用默认的topic
        self.subscribeTopic = [NSString stringWithFormat:@"%@/%@/%@",[MKCGDeviceModeManager shared].deviceName,self.clientID,@"app_to_device"];
    }
    if (!self.sslIsOn) {
        self.connectMode = 0;
    }else {
    ...
    [MKCGMQTTInterface cg_modifyMqttInfos:self macAddress:[MKCGDeviceModeManager shared].macAddress topic:[MKCGDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)configMqttCerts {
    __block BOOL success = NO;
    [MKCGMQTTInterface cg_modifyMqttCerts:self macAddress:[MKCGDeviceModeManager shared].macAddress topic:[MKCGDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
