//
//  MKCGMqttServerConfigFooterView.m
...
@implementation MKCGMqttServerConfigFooterViewModel
@end
...
@interface MKCGMqttServerConfigFooterView ()<UIScrollViewDelegate,
MKMQTTGeneralParamsViewDelegate,
MKMQTTUserCredentialsViewDelegate,
MKCGMqttServerSSLViewDelegate>
...
@property (nonatomic, strong)MKCGMqttServerSSLView *sslView;

@property (nonatomic, assign)NSInteger index;

@end

@implementation MKCGMqttServerConfigFooterView
...
#pragma mark - MKCGMqttServerSSLViewDelegate
- (void)cg_mqtt_sslParams_modifyDevice_sslStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(cg_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
...
- (void)cg_mqtt_sslParams_modifyDevice_certificateChanged:(NSInteger)certificate {
...
/// @param value value
- (void)cg_mqtt_sslParams_modifyDevice_textFieldValueChanged:(NSInteger)index value:(NSString *)value {
    ...
    if ([self.delegate respondsToSelector:@selector(cg_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate cg_mqtt_serverForDevice_textFieldValueChanged:value textID:tempIndex];
    }
}
...
#pragma mark - setter
- (void)setDataModel:(MKCGMqttServerConfigFooterViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCGMqttServerConfigFooterViewModel.class]) {
        return;
    }
    ...
    MKCGMqttServerSSLViewModel *sslModel = [[MKCGMqttServerSSLViewModel alloc] init];
...
- (MKCGMqttServerSSLView *)sslView {
    if (!_sslView) {
        _sslView = [[MKCGMqttServerSSLView alloc] init];
