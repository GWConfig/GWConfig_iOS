...
@implementation MKCGMqttServerSSLViewModel
@end

@interface MKCGMqttServerSSLView ()<MKCGMqttServerSSLTextFieldDelegate>
...
@property (nonatomic, strong)MKCGMqttServerSSLTextField *caFileView;

@property (nonatomic, strong)MKCGMqttServerSSLTextField *clientKeyView;

@property (nonatomic, strong)MKCGMqttServerSSLTextField *clientCertView;

@end

@implementation MKCGMqttServerSSLView
...
#pragma mark - MKCGMqttServerSSLTextFieldDelegate
/// textField内容发送改变时的回调事件
/// @param index 当前cell所在的index
/// @param value 当前textField的值
- (void)cg_modifyServerSSLTextFieldValueChanged:(NSInteger)index textValue:(NSString *)value {
    if ([self.delegate respondsToSelector:@selector(cg_mqtt_sslParams_modifyDevice_textFieldValueChanged:value:)]) {
        [self.delegate cg_mqtt_sslParams_modifyDevice_textFieldValueChanged:index value:value];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(cg_mqtt_sslParams_modifyDevice_sslStatusChanged:)]) {
        [self.delegate cg_mqtt_sslParams_modifyDevice_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(cg_mqtt_sslParams_modifyDevice_certificateChanged:)]) {
            [self.delegate cg_mqtt_sslParams_modifyDevice_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKCGMqttServerSSLViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCGMqttServerSSLViewModel.class]) {
        return;
    }
    ...
    MKCGMqttServerSSLTextFieldModel *caModel = [[MKCGMqttServerSSLTextFieldModel alloc] init];
    caModel.index = 0;
    ...
    MKCGMqttServerSSLTextFieldModel *clientKeyModel = [[MKCGMqttServerSSLTextFieldModel alloc] init];
    ...
    MKCGMqttServerSSLTextFieldModel *clientModel = [[MKCGMqttServerSSLTextFieldModel alloc] init];
    clientModel.index = 2;
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"CommureGateway", @"MKCGMqttServerSSLView", @"cg_switchSelectedIcon.png") : LOADICON(@"CommureGateway", @"MKCGMqttServerSSLView", @"cg_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"CommureGateway", @"MKCGMqttServerSSLView", @"cg_switchUnselectedIcon.png") forState:UIControlStateNormal];
...
- (MKCGMqttServerSSLTextField *)caFileView {
    if (!_caFileView) {
        _caFileView = [[MKCGMqttServerSSLTextField alloc] init];
        _caFileView.delegate = self;
    }
    return _caFileView;
}

- (MKCGMqttServerSSLTextField *)clientKeyView {
    if (!_clientKeyView) {
        _clientKeyView = [[MKCGMqttServerSSLTextField alloc] init];
        _clientKeyView.delegate = self;
    }
    return _clientKeyView;
}

- (MKCGMqttServerSSLTextField *)clientCertView {
    if (!_clientCertView) {
        _clientCertView = [[MKCGMqttServerSSLTextField alloc] init];
