...
@interface MKCGMqttServerController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKCGMqttServerConfigFooterViewDelegate,
...
@property (nonatomic, strong)MKCGMqttServerModel *dataModel;

@property (nonatomic, strong)MKCGMqttServerConfigFooterView *sslParamsView;

@property (nonatomic, strong)MKCGMqttServerConfigFooterViewModel *sslParamsModel;

@property (nonatomic, strong)UIView *footerView;

@end

@implementation MKCGMqttServerController

- (void)dealloc {
    NSLog(@"MKCGMqttServerController销毁");
}
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveUpdateCerts:)
                                                 name:MKCGReceiveDeviceUpdateMqttCertsResultNotification
                                               object:nil];
}
...
#pragma mark - MKCGMqttServerConfigFooterViewDelegate
/// 用户改变了开关状态
/// @param isOn isOn
...
- (void)cg_mqtt_serverForDevice_bottomButtonPressed:(NSInteger)index {
    if (index == 0) {
        //Export Demo File
...
- (void)cg_selectedServerParams:(NSString *)fileName {
    [MKCGExcelDataManager parseDeviceExcel:fileName
        ...
        MKCGMqttServerModel *model = [MKCGMqttServerModel mk_modelWithJSON:returnData];
...
- (void)clearAllParams {
    MKCGMqttServerModel *model = [[MKCGMqttServerModel alloc] init];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"CommureGateway", @"MKCGMqttServerController", @"cg_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCGMqttServerModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCGMqttServerModel alloc] init];
    }
    return _dataModel;
}
...
- (MKCGMqttServerConfigFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKCGMqttServerConfigFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 380.f)];
...
- (MKCGMqttServerConfigFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKCGMqttServerConfigFooterViewModel alloc] init];
    }
