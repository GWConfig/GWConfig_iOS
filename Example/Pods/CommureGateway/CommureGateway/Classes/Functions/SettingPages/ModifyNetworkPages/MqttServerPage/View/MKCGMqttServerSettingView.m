...
@interface MKCGMqttServerSettingView ()
...
@implementation MKCGMqttServerSettingView
...
- (void)clearButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cg_mqtt_deviecSetting_fileButtonPressed:)]) {
        [self.delegate cg_mqtt_deviecSetting_fileButtonPressed:2];
    }
}

- (void)exportButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cg_mqtt_deviecSetting_fileButtonPressed:)]) {
        [self.delegate cg_mqtt_deviecSetting_fileButtonPressed:0];
    }
}

- (void)importButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cg_mqtt_deviecSetting_fileButtonPressed:)]) {
        [self.delegate cg_mqtt_deviecSetting_fileButtonPressed:1];
    }
}
