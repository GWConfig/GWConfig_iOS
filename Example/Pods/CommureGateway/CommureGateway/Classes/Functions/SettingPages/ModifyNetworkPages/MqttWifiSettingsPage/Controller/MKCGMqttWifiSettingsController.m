//
//  MKCGMqttWifiSettingsController.m
...
@interface MKCGMqttWifiSettingsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCGMqttWifiSettingsModel *dataModel;

@end

@implementation MKCGMqttWifiSettingsController

- (void)dealloc {
    NSLog(@"MKCGMqttWifiSettingsController销毁");
}
            ...
            if (((self.dataModel.eapType == 0 || self.dataModel.eapType == 1) && self.dataModel.verifyServer) || self.dataModel.eapType == 2) {
                ...
                [[NSNotificationCenter defaultCenter] addObserver:self
                                                         selector:@selector(receiveUpdateEAPCerts:)
                                                             name:MKCGReceiveDeviceUpdateEapCertsResultNotification
                                                           object:nil];
                return;
            }
        }
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"CommureGateway", @"MKCGMqttWifiSettingsController", @"cg_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCGMqttWifiSettingsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCGMqttWifiSettingsModel alloc] init];
    }
    return _dataModel;
}
