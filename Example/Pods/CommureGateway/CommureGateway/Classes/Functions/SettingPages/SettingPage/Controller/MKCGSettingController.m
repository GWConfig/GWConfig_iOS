...
@interface MKCGSettingController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCGSettingModel *dataModel;

@property (nonatomic, copy)NSString *localNameAsciiStr;

@end

@implementation MKCGSettingController

- (void)dealloc {
    NSLog(@"MKCGSettingController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0) {
        ...
        MKCGNetworkStatusController *vc = [[MKCGNetworkStatusController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 1) {
        //Communicate timeout
        MKCGCommunicateController *vc = [[MKCGCommunicateController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 2) {
        //Data report timout
        MKCGDataReportController *vc = [[MKCGDataReportController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 3) {
        //Connect Beacon timeout
        MKCGConnectBeaconTimeoutController *vc = [[MKCGConnectBeaconTimeoutController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 4) {
        //System time
        MKCGSystemTimeController *vc = [[MKCGSystemTimeController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 5) {
        //Reset device by button
        MKCGResetByButtonController *vc = [[MKCGResetByButtonController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    ...
    if (indexPath.section == 2 && indexPath.row == 0) {
        //OTA
        MKCGOTAController *vc = [[MKCGOTAController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 1) {
        //Modify network settings
        MKCGMqttConfigSelectController *vc = [[MKCGMqttConfigSelectController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    ...
    if (indexPath.section == 3 && indexPath.row == 0) {
        //Batch DFU Beacon
        MKCGBatchDfuBeaconController *vc = [[MKCGBatchDfuBeaconController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 3 && indexPath.row == 1) {
        //Batch update Beacon key
        MKCGBatchUpdateKeyController *vc = [[MKCGBatchUpdateKeyController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    ...
    if (indexPath.section == 4 && indexPath.row == 0) {
        //Device information
        MKCGDeviceInfoController *vc = [[MKCGDeviceInfoController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
}
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    if (section == 1) {
        return ([[MKCGDeviceModeManager shared].deviceType isEqualToString:@"10"] ? 1 : self.section1List.count);
    }
    if (section == 2) {
...
- (void)rootButtonPressed {
    ...
    [alertView showAlertWithTitle:@"Reboot Device" message:msg notificationName:@"mk_cg_needDismissAlert"];
}
...
- (void)resetButtonPressed {
    ...
    [alertView addAction:cancelAction];
    [alertView addAction:confirmAction];
    [alertView showAlertWithTitle:@"Reset Device" message:msg notificationName:@"mk_cg_needDismissAlert"];
}
...
- (void)configLocalName{
    ...
    self.localNameAsciiStr = SafeStr([MKCGDeviceModeManager shared].deviceName);
    MKAlertViewTextField *textField = [[MKAlertViewTextField alloc] initWithTextValue:SafeStr([MKCGDeviceModeManager shared].deviceName)
    ...
    [alertView showAlertWithTitle:@"Edit Local Name" message:msg notificationName:@"mk_cg_needDismissAlert"];
}
...
- (void)saveDeviceLocalName {
    ...
    [MKCGDeviceDatabaseManager updateLocalName:self.localNameAsciiStr
                                    macAddress:[MKCGDeviceModeManager shared].macAddress
                                      sucBlock:^{
        [[MKHudManager share] hide];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_cg_deviceNameChangedNotification"
                                                            object:nil
                                                          userInfo:@{
                                                              @"macAddress":[MKCGDeviceModeManager shared].macAddress,
...
- (void)resetDevice {
    ...
    [MKCGMQTTInterface cg_resetDeviceWithMacAddress:[MKCGDeviceModeManager shared].macAddress topic:[MKCGDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        [self removeDevice];
    } failedBlock:^(NSError * _Nonnull error) {
        [[MKHudManager share] hide];
...
- (void)removeDevice {
    ...
    [MKCGDeviceDatabaseManager deleteDeviceWithMacAddress:[MKCGDeviceModeManager shared].macAddress sucBlock:^{
        [[MKHudManager share] hide];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_cg_deleteDeviceNotification"
                                                            object:nil
                                                          userInfo:@{@"macAddress":[MKCGDeviceModeManager shared].macAddress}];
...
- (void)showAdvTimeAlert {
    ...
    [alertView addAction:confirmAction];
    [alertView addTextField:textField];
    [alertView showAlertWithTitle:@"Advertising time" message:@"" notificationName:@"mk_cg_needDismissAlert"];
}
...
- (void)configPowerSwitchStatus:(BOOL)isOn {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKCGMQTTInterface cg_configPowerSwitchStatus:isOn macAddress:[MKCGDeviceModeManager shared].macAddress topic:[MKCGDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"CommureGateway", @"MKCGSettingController", @"cg_editIcon.png") forState:UIControlStateNormal];
...
- (MKCGSettingModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCGSettingModel alloc] init];
    }
    return _dataModel;
}
