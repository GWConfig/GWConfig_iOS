...
@implementation MKCGSettingModel
...
- (BOOL)readAdvStatus {
    ...
    [MKCGMQTTInterface cg_readBleAdvStatusWithMacAddress:[MKCGDeviceModeManager shared].macAddress topic:[MKCGDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configAdvStatus {
    __block BOOL success = NO;
    [MKCGMQTTInterface cg_configBleAdvStatus:self.advIsOn advTime:[self.advTime integerValue] macAddress:[MKCGDeviceModeManager shared].macAddress topic:[MKCGDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readPowerSwitchStatus {
    __block BOOL success = NO;
    [MKCGMQTTInterface cg_readPowerSwitchStatusWithMacAddress:[MKCGDeviceModeManager shared].macAddress topic:[MKCGDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
