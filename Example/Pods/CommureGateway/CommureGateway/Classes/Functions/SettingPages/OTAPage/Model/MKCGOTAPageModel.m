...
@implementation MKCGOTAPageModel
...
- (BOOL)startOTA {
    ...
    [MKCGMQTTInterface cg_configOTAWithFilePath:self.filePath macAddress:[MKCGDeviceModeManager shared].macAddress topic:[MKCGDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
