...
@interface MKCGSystemTimeController ()<UITableViewDelegate,
UITableViewDataSource,
MKCGSystemTimeCellDelegate,
...
@implementation MKCGSystemTimeController

- (void)dealloc {
    NSLog(@"MKCGSystemTimeController销毁");
    if (self.readTimer) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0)  {
        //Sync Time From NTP
        MKCGNTPServerController *vc = [[MKCGNTPServerController alloc] init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKCGSystemTimeCell *cell = [MKCGSystemTimeCell initCellWithTableView:tableView];
...
#pragma mark - MKCGSystemTimeCellDelegate
...
- (void)readDataFromServer {
    ...
    [MKCGMQTTInterface cg_readDeviceUTCTimeWithMacAddress:[MKCGDeviceModeManager shared].macAddress topic:[MKCGDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)syncTimeZoneToDevice:(NSInteger)timeZone {
    ...
    [MKCGMQTTInterface cg_configDeviceTimeZone:timeZone - 24 timestamp:[[NSDate date] timeIntervalSince1970] macAddress:[MKCGDeviceModeManager shared].macAddress topic:[MKCGDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)loadSection1Datas {
    MKCGSystemTimeCellModel *cellModel = [[MKCGSystemTimeCellModel alloc] init];
    cellModel.index = 0;
