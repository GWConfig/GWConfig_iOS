...
@implementation MKCGDataReportModel
...
- (BOOL)readDataReportTimeout {
    ...
    [MKCGMQTTInterface cg_readDataReportTimeoutWithMacAddress:[MKCGDeviceModeManager shared].macAddress topic:[MKCGDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configDataReportTimeout {
    __block BOOL success = NO;
    [MKCGMQTTInterface cg_configDataReportTimeout:[self.timeout integerValue] macAddress:[MKCGDeviceModeManager shared].macAddress topic:[MKCGDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
