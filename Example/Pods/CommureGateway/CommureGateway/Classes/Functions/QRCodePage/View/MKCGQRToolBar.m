...
- (UIButton *)torchBtn {
    if (!_torchBtn) {
        ...
        [_torchBtn setBackgroundImage:LOADICON(@"CommureGateway", @"MKCGQRToolBar", @"cg_scan_torch.png") forState:(UIControlStateNormal)];
...
- (UILabel *)torchLab {
    if (!_torchLab) {
        ...
        _torchLab.text = @"Tap to turn light on";
...
- (void)torchBtn_action:(UIButton *)btn {
    if (btn.selected) {
        btn.selected = NO;
        [btn setBackgroundImage:LOADICON(@"CommureGateway", @"MKCGQRToolBar", @"cg_scan_torch.png") forState:(UIControlStateNormal)];
        [SGTorch turnOffTorch];
        self.torchLab.text = @"Tap to turn light on";
    } else {
        btn.selected = YES;
        [btn setBackgroundImage:LOADICON(@"CommureGateway", @"MKCGQRToolBar", @"cg_scan_torch_hl.png") forState:(UIControlStateNormal)];
        self.torchLab.text = @"Tap to turn light off";
        [SGTorch turnOnTorch];
    }
}
...
- (UILabel *)tipsLab {
    if (!_tipsLab) {
        ...
        _tipsLab.text = @"Recognize QR code.";
...
- (UIImageView *)qrcodeImgView {
    if (!_qrcodeImgView) {
        ...
        _qrcodeImgView.image = [UIImage imageNamed:@"cg_scan_mine_qrcode"];
    }
...
- (UIImageView *)albumImgView {
    if (!_albumImgView) {
        ...
        _albumImgView.image = [UIImage imageNamed:@"cg_scan_album"];
    }
