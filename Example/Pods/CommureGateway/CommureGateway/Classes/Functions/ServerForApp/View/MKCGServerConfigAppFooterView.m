//
//  MKCGServerConfigAppFooterView.m
...
@implementation MKCGServerConfigAppFooterViewModel
@end
...
@interface MKCGServerConfigAppFooterView ()<UIScrollViewDelegate,
MKMQTTGeneralParamsViewDelegate,
MKMQTTUserCredentialsViewDelegate,
MKCGMQTTSSLForAppViewDelegate>
...
@property (nonatomic, strong)MKCGMQTTSSLForAppView *sslView;
...
@implementation MKCGServerConfigAppFooterView
...
- (void)mk_mqtt_generalParams_cleanSessionStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(cg_mqtt_serverForApp_switchStatusChanged:statusID:)]) {
        [self.delegate cg_mqtt_serverForApp_switchStatusChanged:isOn statusID:0];
    }
}

- (void)mk_mqtt_generalParams_qosChanged:(NSInteger)qos {
    if ([self.delegate respondsToSelector:@selector(cg_mqtt_serverForApp_qosChanged:)]) {
        [self.delegate cg_mqtt_serverForApp_qosChanged:qos];
    }
}

- (void)mk_mqtt_generalParams_KeepAliveChanged:(NSString *)keepAlive {
    if ([self.delegate respondsToSelector:@selector(cg_mqtt_serverForApp_textFieldValueChanged:textID:)]) {
        [self.delegate cg_mqtt_serverForApp_textFieldValueChanged:keepAlive textID:0];
    }
}
...
- (void)mk_mqtt_userCredentials_userNameChanged:(NSString *)userName {
    if ([self.delegate respondsToSelector:@selector(cg_mqtt_serverForApp_textFieldValueChanged:textID:)]) {
        [self.delegate cg_mqtt_serverForApp_textFieldValueChanged:userName textID:1];
    }
}

- (void)mk_mqtt_userCredentials_passwordChanged:(NSString *)password {
    if ([self.delegate respondsToSelector:@selector(cg_mqtt_serverForApp_textFieldValueChanged:textID:)]) {
        [self.delegate cg_mqtt_serverForApp_textFieldValueChanged:password textID:2];
    }
}

#pragma mark - MKCGMQTTSSLForAppViewDelegate
- (void)cg_mqtt_sslParams_app_sslStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(cg_mqtt_serverForApp_switchStatusChanged:statusID:)]) {
        [self.delegate cg_mqtt_serverForApp_switchStatusChanged:isOn statusID:1];
    }
}
...
- (void)cg_mqtt_sslParams_app_certificateChanged:(NSInteger)certificate {
    if ([self.delegate respondsToSelector:@selector(cg_mqtt_serverForApp_certificateChanged:)]) {
        [self.delegate cg_mqtt_serverForApp_certificateChanged:certificate];
    }
}

/// 用户点击选择了caFaile按钮
- (void)cg_mqtt_sslParams_app_caFilePressed {
    if ([self.delegate respondsToSelector:@selector(cg_mqtt_serverForApp_fileButtonPressed:)]) {
        [self.delegate cg_mqtt_serverForApp_fileButtonPressed:0];
    }
}
...
- (void)cg_mqtt_sslParams_app_clientFilePressed {
    if ([self.delegate respondsToSelector:@selector(cg_mqtt_serverForApp_fileButtonPressed:)]) {
        [self.delegate cg_mqtt_serverForApp_fileButtonPressed:1];
    }
}
...
#pragma mark - setter
- (void)setDataModel:(MKCGServerConfigAppFooterViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCGServerConfigAppFooterViewModel.class]) {
        return;
    }
    ...
    MKCGMQTTSSLForAppViewModel *sslModel = [[MKCGMQTTSSLForAppViewModel alloc] init];
...
- (MKCGMQTTSSLForAppView *)sslView {
    if (!_sslView) {
        _sslView = [[MKCGMQTTSSLForAppView alloc] init];
