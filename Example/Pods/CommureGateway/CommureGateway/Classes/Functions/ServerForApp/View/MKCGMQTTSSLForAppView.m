...
@implementation MKCGMQTTSSLForAppViewModel
@end

@interface MKCGMQTTSSLForAppView ()<MKMQTTSSLCertificateViewDelegate>
...
@implementation MKCGMQTTSSLForAppView
...
- (void)mk_fileSelectedButtonPressed:(NSInteger)index {
    if (index == 0) {
        //CA File
        if ([self.delegate respondsToSelector:@selector(cg_mqtt_sslParams_app_caFilePressed)]) {
            [self.delegate cg_mqtt_sslParams_app_caFilePressed];
        }
        return;
    }
    //P12证书
    if ([self.delegate respondsToSelector:@selector(cg_mqtt_sslParams_app_clientFilePressed)]) {
        [self.delegate cg_mqtt_sslParams_app_clientFilePressed];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(cg_mqtt_sslParams_app_sslStatusChanged:)]) {
        [self.delegate cg_mqtt_sslParams_app_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(cg_mqtt_sslParams_app_certificateChanged:)]) {
            [self.delegate cg_mqtt_sslParams_app_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKCGMQTTSSLForAppViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCGMQTTSSLForAppViewModel.class] || _dataModel.certificate < 0 || _dataModel.certificate > 2) {
        return;
    }
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"CommureGateway", @"MKCGMQTTSSLForAppView", @"cg_switchSelectedIcon.png") : LOADICON(@"CommureGateway", @"MKCGMQTTSSLForAppView", @"cg_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"CommureGateway", @"MKCGMQTTSSLForAppView", @"cg_switchUnselectedIcon.png") forState:UIControlStateNormal];
