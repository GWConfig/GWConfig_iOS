...
@implementation MKCGDeviceDatabaseManager
...
+ (void)insertDeviceList:(NSArray <MKCGDeviceModel *>*)deviceList
    ...
    [[FMDatabaseQueue databaseQueueWithPath:kFilePath(@"CGDeviceDB")] inDatabase:^(FMDatabase *db) {
        ...
        for (MKCGDeviceModel *device in deviceList) {
            BOOL exist = NO;
...
+ (void)readLocalDeviceWithSucBlock:(void (^)(NSArray <MKCGDeviceModel *> *deviceList))sucBlock
                        failedBlock:(void (^)(NSError *error))failedBlock {
    FMDatabase* db = [FMDatabase databaseWithPath:kFilePath(@"CGDeviceDB")];
    if (![db open]) {
    ...
    [[FMDatabaseQueue databaseQueueWithPath:kFilePath(@"CGDeviceDB")] inDatabase:^(FMDatabase *db) {
        ...
        while ([result next]) {
            MKCGDeviceModel *dataModel = [[MKCGDeviceModel alloc] init];
