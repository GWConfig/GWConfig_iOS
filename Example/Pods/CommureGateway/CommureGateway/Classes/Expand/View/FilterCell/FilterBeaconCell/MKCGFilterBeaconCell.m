...
@implementation MKCGFilterBeaconCellModel
@end

@interface MKCGFilterBeaconCell ()
...
@implementation MKCGFilterBeaconCell

+ (MKCGFilterBeaconCell *)initCellWithTableView:(UITableView *)tableView {
    MKCGFilterBeaconCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCGFilterBeaconCellIdenty"];
    if (!cell) {
        cell = [[MKCGFilterBeaconCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCGFilterBeaconCellIdenty"];
    }
    return cell;
}
...
#pragma mark - setter
- (void)setDataModel:(MKCGFilterBeaconCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCGFilterBeaconCellModel.class]) {
        return;
    }
...
- (MKTextField *)minTextField {
    if (!_minTextField) {
        ...
        _minTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(mk_cg_beaconMinValueChanged:index:)]) {
                [self.delegate mk_cg_beaconMinValueChanged:text index:self.dataModel.index];
            }
        };
    }
...
- (MKTextField *)maxTextField {
    if (!_maxTextField) {
        ...
        _maxTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(mk_cg_beaconMaxValueChanged:index:)]) {
                [self.delegate mk_cg_beaconMaxValueChanged:text index:self.dataModel.index];
            }
        };
    }
