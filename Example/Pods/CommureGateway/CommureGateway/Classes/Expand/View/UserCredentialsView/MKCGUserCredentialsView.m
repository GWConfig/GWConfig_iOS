...
@implementation MKCGUserCredentialsViewModel
@end

@interface MKCGUserCredentialsView ()
...
@implementation MKCGUserCredentialsView
...
#pragma mark - setter
- (void)setDataModel:(MKCGUserCredentialsViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCGUserCredentialsViewModel.class]) {
        return;
    }
...
- (MKTextField *)userNameTextField {
    if (!_userNameTextField) {
        ...
        _userNameTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(cg_mqtt_userCredentials_userNameChanged:)]) {
                [self.delegate cg_mqtt_userCredentials_userNameChanged:text];
            }
        };
...
- (MKTextField *)passwordTextField {
    if (!_passwordTextField) {
        ...
        _passwordTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(cg_mqtt_userCredentials_passwordChanged:)]) {
                [self.delegate cg_mqtt_userCredentials_passwordChanged:text];
            }
        };
