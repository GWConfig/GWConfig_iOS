...
@implementation MKCGBleWifiSettingsBandCellModel
@end

@interface MKCGBleWifiSettingsBandCell ()
...
@implementation MKCGBleWifiSettingsBandCell

+ (MKCGBleWifiSettingsBandCell *)initCellWithTableView:(UITableView *)tableView {
    MKCGBleWifiSettingsBandCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCGBleWifiSettingsBandCellIdenty"];
    if (!cell) {
        cell = [[MKCGBleWifiSettingsBandCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCGBleWifiSettingsBandCellIdenty"];
    }
    return cell;
}
...
- (void)selectedButtonPressed {
    ...
    [pickView showPickViewWithDataList:self.dataList selectedRow:row block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(cg_bleWifiSettingsBandCell_countryChanged:)]) {
            [self.delegate cg_bleWifiSettingsBandCell_countryChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKCGBleWifiSettingsBandCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCGBleWifiSettingsBandCellModel.class] || _dataModel.country >= self.dataList.count) {
        return;
    }
