...
NSString *const mk_ch_totalNumKey = @"mk_ch_totalNumKey";
NSString *const mk_ch_totalIndexKey = @"mk_ch_totalIndexKey";
NSString *const mk_ch_contentKey = @"mk_ch_contentKey";

@implementation MKCHTaskAdopter
...
+ (NSDictionary *)parseReadDataWithCharacteristic:(CBCharacteristic *)characteristic {
    ...
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"2A24"]]) {
        ...
        return [self dataParserGetDataSuccess:@{@"modeID":tempString} operationID:mk_ch_taskReadDeviceModelOperation];
    }
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"2A26"]]) {
        //firmware
        NSString *tempString = [[NSString alloc] initWithData:readData encoding:NSUTF8StringEncoding];
        return [self dataParserGetDataSuccess:@{@"firmware":tempString} operationID:mk_ch_taskReadFirmwareOperation];
    }
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"2A27"]]) {
        //hardware
        NSString *tempString = [[NSString alloc] initWithData:readData encoding:NSUTF8StringEncoding];
        return [self dataParserGetDataSuccess:@{@"hardware":tempString} operationID:mk_ch_taskReadHardwareOperation];
    }
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"2A28"]]) {
        //soft ware
        NSString *tempString = [[NSString alloc] initWithData:readData encoding:NSUTF8StringEncoding];
        return [self dataParserGetDataSuccess:@{@"software":tempString} operationID:mk_ch_taskReadSoftwareOperation];
    }
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"2A29"]]) {
        //manufacturerKey
        NSString *tempString = [[NSString alloc] initWithData:readData encoding:NSUTF8StringEncoding];
        return [self dataParserGetDataSuccess:@{@"manufacturer":tempString} operationID:mk_ch_taskReadManufacturerOperation];
    }
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"AA00"]]) {
        ...
        return [self dataParserGetDataSuccess:@{@"state":state} operationID:mk_ch_connectPasswordOperation];
    }
...
+ (NSDictionary *)parseCustomReadData:(NSString *)content cmd:(NSString *)cmd data:(NSData *)data {
    mk_ch_taskOperationID operationID = mk_ch_defaultTaskOperationID;
    ...
    if ([cmd isEqualToString:@"02"]) {
        ...
        operationID = mk_ch_taskReadDeviceNameOperation;
        ...
        operationID = mk_ch_taskReadDeviceMacAddressOperation;
    }else if ([cmd isEqualToString:@"0a"]) {
        //读取WIFI STA MAC
        NSString *macAddress = [NSString stringWithFormat:@"%@:%@:%@:%@:%@:%@",[content substringWithRange:NSMakeRange(0, 2)],[content substringWithRange:NSMakeRange(2, 2)],[content substringWithRange:NSMakeRange(4, 2)],[content substringWithRange:NSMakeRange(6, 2)],[content substringWithRange:NSMakeRange(8, 2)],[content substringWithRange:NSMakeRange(10, 2)]];
        resultDic = @{@"macAddress":[macAddress uppercaseString]};
        operationID = mk_ch_taskReadDeviceWifiSTAMacAddressOperation;
        ...
        operationID = mk_ch_taskReadNTPServerHostOperation;
    }else if ([cmd isEqualToString:@"12"]) {
        //读取时区
        resultDic = @{
            @"timeZone":[MKBLEBaseSDKAdopter signedHexTurnString:content],
        };
        operationID = mk_ch_taskReadTimeZoneOperation;
        ...
        operationID = mk_ch_taskReadBleFirmwareOperation;
        ...
        operationID = mk_ch_taskReadServerHostOperation;
        ...
        resultDic = @{@"port":port};
        operationID = mk_ch_taskReadServerPortOperation;
        ...
        operationID = mk_ch_taskReadServerKeepAliveOperation;
        ...
        resultDic = @{@"qos":qos};
        operationID = mk_ch_taskReadServerQosOperation;
        ...
        operationID = mk_ch_taskReadSubscibeTopicOperation;
        ...
        operationID = mk_ch_taskReadPublishTopicOperation;
        ...
        resultDic = @{@"mode":mode};
        operationID = mk_ch_taskReadConnectModeOperation;
        ...
        operationID = mk_ch_taskReadWIFISecurityOperation;
        ...
        operationID = mk_ch_taskReadWIFISSIDOperation;
        ...
        operationID = mk_ch_taskReadWIFIPasswordOperation;
        ...
        operationID = mk_ch_taskReadWIFIEAPTypeOperation;
        ...
        operationID = mk_ch_taskReadWIFIEAPUsernameOperation;
        ...
        operationID = mk_ch_taskReadWIFIEAPPasswordOperation;
        ...
        operationID = mk_ch_taskReadWIFIEAPDomainIDOperation;
    }else if ([cmd isEqualToString:@"47"]) {
        //读取是否校验服务器
        BOOL verify = ([content isEqualToString:@"01"]);
        resultDic = @{@"verify":@(verify)};
        operationID = mk_ch_taskReadWIFIVerifyServerStatusOperation;
    }else if ([cmd isEqualToString:@"4b"]) {
        //读取DHCP开关
        BOOL isOn = ([content isEqualToString:@"01"]);
        resultDic = @{@"isOn":@(isOn)};
        operationID = mk_ch_taskReadWIFIDHCPStatusOperation;
    }else if ([cmd isEqualToString:@"4c"]) {
        //读取IP信息
        NSString *ip = [MKCHSDKDataAdopter parseIpAddress:[content substringWithRange:NSMakeRange(0, 8)]];
        NSString *mask = [MKCHSDKDataAdopter parseIpAddress:[content substringWithRange:NSMakeRange(8, 8)]];
        NSString *gateway = [MKCHSDKDataAdopter parseIpAddress:[content substringWithRange:NSMakeRange(16, 8)]];
        NSString *dns = [MKCHSDKDataAdopter parseIpAddress:[content substringWithRange:NSMakeRange(24, 8)]];
        resultDic = @{
            @"ip":ip,
            @"mask":mask,
            @"gateway":gateway,
        ...
        operationID = mk_ch_taskReadWIFINetworkIpInfosOperation;
        ...
        operationID = mk_ch_taskReadCountryBandOperation;
    }
...
+ (NSDictionary *)parseCustomConfigData:(NSString *)content cmd:(NSString *)cmd {
    ...
    if ([cmd isEqualToString:@"02"]) {
        //重启进入STA模式
        operationID = mk_ch_taskEnterSTAModeOperation;
    }else if ([cmd isEqualToString:@"11"]) {
        //配置NTP服务器域名
        operationID = mk_ch_taskConfigNTPServerHostOperation;
    }else if ([cmd isEqualToString:@"12"]) {
        //配置时区
        operationID = mk_ch_taskConfigTimeZoneOperation;
    }else if ([cmd isEqualToString:@"20"]) {
        //配置MQTT服务器域名
        operationID = mk_ch_taskConfigServerHostOperation;
    }else if ([cmd isEqualToString:@"21"]) {
        //配置MQTT服务器端口
        operationID = mk_ch_taskConfigServerPortOperation;
    }else if ([cmd isEqualToString:@"22"]) {
        //配置ClientID
        operationID = mk_ch_taskConfigClientIDOperation;
    }else if ([cmd isEqualToString:@"25"]) {
        //配置MQTT Clean Session
        operationID = mk_ch_taskConfigServerCleanSessionOperation;
    }else if ([cmd isEqualToString:@"26"]) {
        //配置MQTT KeepAlive
        operationID = mk_ch_taskConfigServerKeepAliveOperation;
    }else if ([cmd isEqualToString:@"27"]) {
        //配置MQTT Qos
        operationID = mk_ch_taskConfigServerQosOperation;
    }else if ([cmd isEqualToString:@"28"]) {
        //配置Subscribe topic
        operationID = mk_ch_taskConfigSubscibeTopicOperation;
    }else if ([cmd isEqualToString:@"29"]) {
        //配置Publish topic
        operationID = mk_ch_taskConfigPublishTopicOperation;
    }else if ([cmd isEqualToString:@"2f"]) {
        //配置MTQQ服务器通信加密方式
        operationID = mk_ch_taskConfigConnectModeOperation;
    }else if ([cmd isEqualToString:@"40"]) {
        //配置WIFI 加密模式
        operationID = mk_ch_taskConfigWIFISecurityOperation;
    }else if ([cmd isEqualToString:@"41"]) {
        //配置WIFI SSID
        operationID = mk_ch_taskConfigWIFISSIDOperation;
    }else if ([cmd isEqualToString:@"42"]) {
        //配置WIFI password
        operationID = mk_ch_taskConfigWIFIPasswordOperation;
    }else if ([cmd isEqualToString:@"43"]) {
        //配置WIFI EAP 类型
        operationID = mk_ch_taskConfigWIFIEAPTypeOperation;
    }else if ([cmd isEqualToString:@"44"]) {
        //配置WIFI EAP 用户名
        operationID = mk_ch_taskConfigWIFIEAPUsernameOperation;
    }else if ([cmd isEqualToString:@"45"]) {
        //配置WIFI EAP 密码
        operationID = mk_ch_taskConfigWIFIEAPPasswordOperation;
    }else if ([cmd isEqualToString:@"46"]) {
        //配置wifi的EAP域名ID
        operationID = mk_ch_taskConfigWIFIEAPDomainIDOperation;
    }else if ([cmd isEqualToString:@"47"]) {
        //配置wifi是否校验服务器
        operationID = mk_ch_taskConfigWIFIVerifyServerStatusOperation;
    }else if ([cmd isEqualToString:@"4b"]) {
        //配置Wifi DHCP状态
        operationID = mk_ch_taskConfigWIFIDHCPStatusOperation;
    }else if ([cmd isEqualToString:@"4c"]) {
        //配置Wifi IP地址相关信息
        operationID = mk_ch_taskConfigWIFIIpInfoOperation;
    }else if ([cmd isEqualToString:@"50"]) {
        //配置国家地区参数
        operationID = mk_ch_taskConfigCountryBandOperation;
    }
...
+ (NSDictionary *)parseMultiPackageReadData:(NSData *)readData cmd:(NSString *)cmd {
    ...
    NSDictionary *resultDic= @{
        mk_ch_totalNumKey:totalNum,
        mk_ch_totalIndexKey:index,
        mk_ch_contentKey:(subData ? subData : [NSData data]),
    };
    if ([cmd isEqualToString:@"23"]) {
        //读取服务器登录用户名
        operationID = mk_ch_taskReadServerUserNameOperation;
    }else if ([cmd isEqualToString:@"24"]) {
        //读取服务器登录密码
        operationID = mk_ch_taskReadServerPasswordOperation;
    }
    return [self dataParserGetDataSuccess:resultDic operationID:operationID];
}
...
+ (NSDictionary *)parseMultiPackageData:(NSString *)content cmd:(NSString *)cmd {
    ...
    if ([cmd isEqualToString:@"23"]) {
        //配置服务器登录用户名
        operationID = mk_ch_taskConfigServerUserNameOperation;
    }else if ([cmd isEqualToString:@"24"]) {
        //配置服务器登录密码
        operationID = mk_ch_taskConfigServerPasswordOperation;
    }else if ([cmd isEqualToString:@"30"]) {
        //配置CA file
        operationID = mk_ch_taskConfigCAFileOperation;
    }else if ([cmd isEqualToString:@"31"]) {
        //配置Client certificate file
        operationID = mk_ch_taskConfigClientCertOperation;
    }else if ([cmd isEqualToString:@"32"]) {
        //配置Client key file
        operationID = mk_ch_taskConfigClientPrivateKeyOperation;
    }else if ([cmd isEqualToString:@"48"]) {
        //配置WIFI CA file
        operationID = mk_ch_taskConfigWIFICAFileOperation;
    }else if ([cmd isEqualToString:@"49"]) {
        //配置WIFI Client certificate file
        operationID = mk_ch_taskConfigWIFIClientCertOperation;
    }else if ([cmd isEqualToString:@"4a"]) {
        //配置WIFI Client key file
        operationID = mk_ch_taskConfigWIFIClientPrivateKeyOperation;
    }
    return [self dataParserGetDataSuccess:@{@"success":@(success)} operationID:operationID];
}

+ (NSDictionary *)dataParserGetDataSuccess:(NSDictionary *)returnData operationID:(mk_ch_taskOperationID)operationID{
    if (!returnData) {
        return @{};
    }
