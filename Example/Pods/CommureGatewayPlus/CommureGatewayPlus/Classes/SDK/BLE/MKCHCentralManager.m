...
static NSString *const mk_ch_logName = @"mk_ch_bleLog";

NSString *const mk_ch_peripheralConnectStateChangedNotification = @"mk_ch_peripheralConnectStateChangedNotification";
NSString *const mk_ch_centralManagerStateChangedNotification = @"mk_ch_centralManagerStateChangedNotification";

NSString *const mk_ch_deviceDisconnectTypeNotification = @"mk_ch_deviceDisconnectTypeNotification";

static MKCHCentralManager *manager = nil;
static dispatch_once_t onceToken;

@interface MKCHCentralManager ()
...
@property (nonatomic, assign)mk_ch_centralConnectStatus connectStatus;

@end

@implementation MKCHCentralManager

- (void)dealloc {
    [self logToLocal:@"MKCHCentralManager销毁"];
    NSLog(@"MKCHCentralManager销毁");
}

- (instancetype)init {
    if (self = [super init]) {
        [self logToLocal:@"MKCHCentralManager初始化"];
        [[MKBLEBaseCentralManager shared] loadDataManager:self];
    }
    return self;
}

+ (MKCHCentralManager *)shared {
    dispatch_once(&onceToken, ^{
        if (!manager) {
            manager = [MKCHCentralManager new];
        }
    });
    return manager;
}
...
- (void)MKBLEBaseCentralManagerDiscoverPeripheral:(CBPeripheral *)peripheral
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if ([self.delegate respondsToSelector:@selector(mk_ch_receiveDevice:)]) {
                [self.delegate mk_ch_receiveDevice:dataModel];
            }
        });
    });
}
...
- (void)MKBLEBaseCentralManagerStartScan {
    ...
    if ([self.delegate respondsToSelector:@selector(mk_ch_startScan)]) {
        [self.delegate mk_ch_startScan];
    }
}

- (void)MKBLEBaseCentralManagerStopScan {
    [self logToLocal:@"停止扫描"];
    if ([self.delegate respondsToSelector:@selector(mk_ch_stopScan)]) {
        [self.delegate mk_ch_stopScan];
    }
}
...
- (void)MKBLEBaseCentralManagerStateChanged:(MKCentralManagerState)centralManagerState {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:mk_ch_centralManagerStateChangedNotification object:nil];
}
...
- (void)MKBLEBasePeripheralConnectStateChanged:(MKPeripheralConnectState)connectState {
    ...
    if (connectState == MKPeripheralConnectStateUnknow) {
        self.connectStatus = mk_ch_centralConnectStatusUnknow;
    }else if (connectState == MKPeripheralConnectStateConnecting) {
        self.connectStatus = mk_ch_centralConnectStatusConnecting;
    }else if (connectState == MKPeripheralConnectStateConnectedFailed) {
        self.connectStatus = mk_ch_centralConnectStatusConnectedFailed;
    }else if (connectState == MKPeripheralConnectStateDisconnect) {
        self.connectStatus = mk_ch_centralConnectStatusDisconnect;
    }
    NSLog(@"当前连接状态发生改变了:%@",@(connectState));
    NSString *string = [NSString stringWithFormat:@"连接状态发生改变:%@",@(connectState)];
    [self logToLocal:string];
    [[NSNotificationCenter defaultCenter] postNotificationName:mk_ch_peripheralConnectStateChangedNotification object:nil];
}
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
    ...
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"AA01"]]) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:mk_ch_deviceDisconnectTypeNotification
                                                            object:nil
...
- (mk_ch_centralManagerStatus )centralStatus {
    return ([MKBLEBaseCentralManager shared].centralStatus == MKCentralManagerStateEnable)
    ? mk_ch_centralManagerStatusEnable
    : mk_ch_centralManagerStatusUnable;
}

- (void)startScan {
    [[MKBLEBaseCentralManager shared] scanForPeripheralsWithServices:@[[CBUUID UUIDWithString:@"AA0C"]] options:nil];
}
...
- (void)addTaskWithTaskID:(mk_ch_taskOperationID)operationID
           characteristic:(CBCharacteristic *)characteristic
              commandData:(NSString *)commandData
             successBlock:(void (^)(id returnData))successBlock
             failureBlock:(void (^)(NSError *error))failureBlock {
    MKCHOperation <MKBLEBaseOperationProtocol>*operation = [self generateOperationWithOperationID:operationID
...
- (void)addReadTaskWithTaskID:(mk_ch_taskOperationID)operationID
               characteristic:(CBCharacteristic *)characteristic
                 successBlock:(void (^)(id returnData))successBlock
                 failureBlock:(void (^)(NSError *error))failureBlock {
    MKCHOperation <MKBLEBaseOperationProtocol>*operation = [self generateReadOperationWithOperationID:operationID
                                                                                       characteristic:characteristic
                                                                                         successBlock:successBlock
                                                                                         failureBlock:failureBlock];
    if (!operation) {
...
- (void)addOperation:(MKCHOperation *)operation {
    if (!operation) {
        return;
    }
    [[MKBLEBaseCentralManager shared] addOperation:operation];
...
- (void)connectPeripheral:(CBPeripheral *)peripheral
    ...
    MKCHPeripheral *trackerPeripheral = [[MKCHPeripheral alloc] initWithPeripheral:peripheral];
...
- (void)sendPasswordToDevice {
    ...
    __weak typeof(self) weakSelf = self;
    MKCHOperation *operation = [[MKCHOperation alloc] initOperationWithID:mk_ch_connectPasswordOperation commandBlock:^{
        [[MKBLEBaseCentralManager shared] sendDataToPeripheral:commandData characteristic:[MKBLEBaseCentralManager shared].peripheral.ch_password type:CBCharacteristicWriteWithResponse];
        ...
        //密码正确
        MKBLEBase_main_safe(^{
            sself.connectStatus = mk_ch_centralConnectStatusConnected;
            [[NSNotificationCenter defaultCenter] postNotificationName:mk_ch_peripheralConnectStateChangedNotification object:nil];
            if (sself.sucBlock) {
...
- (MKCHOperation <MKBLEBaseOperationProtocol>*)generateOperationWithOperationID:(mk_ch_taskOperationID)operationID
                                                                 characteristic:(CBCharacteristic *)characteristic
                                                                    commandData:(NSString *)commandData
                                                                   successBlock:(void (^)(id returnData))successBlock
                                                                   failureBlock:(void (^)(NSError *error))failureBlock{
    ...
    __weak typeof(self) weakSelf = self;
    MKCHOperation <MKBLEBaseOperationProtocol>*operation = [[MKCHOperation alloc] initOperationWithID:operationID commandBlock:^{
        __strong typeof(self) sself = weakSelf;
...
- (MKCHOperation <MKBLEBaseOperationProtocol>*)generateReadOperationWithOperationID:(mk_ch_taskOperationID)operationID
                                                                     characteristic:(CBCharacteristic *)characteristic
                                                                       successBlock:(void (^)(id returnData))successBlock
                                                                       failureBlock:(void (^)(NSError *error))failureBlock{
    if (![[MKBLEBaseCentralManager shared] readyToCommunication]) {
...
- (NSDictionary *)parseModelWithRssi:(NSNumber *)rssi advDic:(NSDictionary *)advDic peripheral:(CBPeripheral *)peripheral {
    ...
    NSData *manufacturerData = manuParams[[CBUUID UUIDWithString:@"AA0C"]];
    ...
    NSString *firmwareVersion = [[NSString alloc] initWithData:[normalData subdataWithRange:NSMakeRange(8, normalData.length - 8)] encoding:NSUTF8StringEncoding];
    if (!MKValidStr(firmwareVersion)) {
        firmwareVersion = @"";
    }
...
- (void)logToLocal:(NSString *)string {
    ...
    [MKBLEBaseLogManager saveDataWithFileName:mk_ch_logName dataList:@[string]];
}
