...
@implementation MKCHSDKDataAdopter

+ (NSString *)fetchWifiSecurity:(mk_ch_wifiSecurity)security {
    switch (security) {
        case mk_ch_wifiSecurity_personal:
            return @"00";
        case mk_ch_wifiSecurity_enterprise:
            return @"01";
    }
}

+ (NSString *)fetchWifiEapType:(mk_ch_eapType)eapType {
    switch (eapType) {
        case mk_ch_eapType_peap_mschapv2:
            return @"00";
        case mk_ch_eapType_ttls_mschapv2:
            return @"01";
        case mk_ch_eapType_tls:
            return @"02";
    }
}

+ (NSString *)fetchConnectModeString:(mk_ch_connectMode)mode {
    switch (mode) {
        case mk_ch_connectMode_TCP:
            return @"00";
        case mk_ch_connectMode_CASignedServerCertificate:
            return @"01";
        case mk_ch_connectMode_CACertificate:
            return @"02";
        case mk_ch_connectMode_SelfSignedCertificates:
            return @"03";
    }
}

+ (NSString *)fetchMqttServerQosMode:(mk_ch_mqttServerQosMode)mode {
    switch (mode) {
        case mk_ch_mqttQosLevelAtMostOnce:
            return @"00";
        case mk_ch_mqttQosLevelAtLeastOnce:
            return @"01";
        case mk_ch_mqttQosLevelExactlyOnce:
            return @"02";
    }
}
