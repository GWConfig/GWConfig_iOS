//
//  MKCHDeviceDataPageHeaderView.m
...
@implementation MKCHDeviceDataPageHeaderViewModel
@end

@interface MKCHDeviceDataPageManageBleButton : UIControl
...
@implementation MKCHDeviceDataPageManageBleButton
...
- (UIImageView *)rightIcon {
    if (!_rightIcon) {
        ...
        _rightIcon.image = LOADICON(@"CommureGatewayPlus", @"MKCHDeviceDataPageManageBleButton", @"ch_goNextButton.png");
    }
    return _rightIcon;
}
...
@interface MKCHDeviceDataPageHeaderView ()
...
@property (nonatomic, strong)MKCHDeviceDataPageManageBleButton *manageBleButton;

@end

@implementation MKCHDeviceDataPageHeaderView
...
- (void)uploadButtonPressed {
    if ([self.delegate respondsToSelector:@selector(ch_updateLoadButtonAction)]) {
        [self.delegate ch_updateLoadButtonAction];
    }
}

- (void)scannerButtonPressed {
    if ([self.delegate respondsToSelector:@selector(ch_scannerStatusChanged:)]) {
        [self.delegate ch_scannerStatusChanged:!self.scannerButton.selected];
    }
}

- (void)manageBleButtonPressed {
    if ([self.delegate respondsToSelector:@selector(ch_manageBleDeviceAction)]) {
        [self.delegate ch_manageBleDeviceAction];
    }
}

- (void)filterTestButtonPressed {
    if ([self.delegate respondsToSelector:@selector(ch_filterTestButtonAction)]) {
        [self.delegate ch_filterTestButtonAction];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKCHDeviceDataPageHeaderViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCHDeviceDataPageHeaderViewModel.class]) {
        return;
    }
    self.scannerButton.selected = _dataModel.isOn;
    UIImage *image = (self.scannerButton.selected ? LOADICON(@"CommureGatewayPlus", @"MKCHDeviceDataPageHeaderView", @"ch_switchSelectedIcon.png") : LOADICON(@"CommureGatewayPlus", @"MKCHDeviceDataPageHeaderView", @"ch_switchUnselectedIcon.png"));
...
- (MKCHDeviceDataPageManageBleButton *)manageBleButton {
    if (!_manageBleButton) {
        _manageBleButton = [[MKCHDeviceDataPageManageBleButton alloc] init];
