...
@implementation MKCHMQTTSSLForDeviceViewModel
@end

@interface MKCHMQTTSSLForDeviceView ()<MKMQTTSSLCertificateViewDelegate>
...
@implementation MKCHMQTTSSLForDeviceView
...
- (void)mk_fileSelectedButtonPressed:(NSInteger)index {
    if (index == 0) {
        //CA File
        if ([self.delegate respondsToSelector:@selector(ch_mqtt_sslParams_device_caFilePressed)]) {
            [self.delegate ch_mqtt_sslParams_device_caFilePressed];
        }
        return;
    }
    if (index == 1) {
        //Client Key File
        if ([self.delegate respondsToSelector:@selector(ch_mqtt_sslParams_device_clientKeyPressed)]) {
            [self.delegate ch_mqtt_sslParams_device_clientKeyPressed];
        }
        return;
    }
    //Client Cert File
    if ([self.delegate respondsToSelector:@selector(ch_mqtt_sslParams_device_clientCertPressed)]) {
        [self.delegate ch_mqtt_sslParams_device_clientCertPressed];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(ch_mqtt_sslParams_device_sslStatusChanged:)]) {
        [self.delegate ch_mqtt_sslParams_device_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(ch_mqtt_sslParams_device_certificateChanged:)]) {
            [self.delegate ch_mqtt_sslParams_device_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKCHMQTTSSLForDeviceViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCHMQTTSSLForDeviceViewModel.class]) {
        return;
    }
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"CommureGatewayPlus", @"MKCHMQTTSSLForDeviceView", @"ch_switchSelectedIcon.png") : LOADICON(@"CommureGatewayPlus", @"MKCHMQTTSSLForDeviceView", @"ch_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"CommureGatewayPlus", @"MKCHMQTTSSLForDeviceView", @"ch_switchUnselectedIcon.png") forState:UIControlStateNormal];
