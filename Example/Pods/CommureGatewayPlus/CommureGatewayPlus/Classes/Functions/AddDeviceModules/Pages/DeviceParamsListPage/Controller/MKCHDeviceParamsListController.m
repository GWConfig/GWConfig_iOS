//
//  MKCHDeviceParamsListController.m
...
@interface MKCHDeviceParamsListController ()<UITableViewDelegate, UITableViewDataSource>
...
@property (nonatomic, strong)MKCHDeviceParamListModel *dataModel;

@end

@implementation MKCHDeviceParamsListController

- (void)dealloc {
    NSLog(@"MKCHDeviceParamsListController销毁");
...
- (void)viewDidLoad {
    ...
    [MKCHDeviceMQTTParamsModel shared].deviceModel.deviceType = @"20";
...
- (void)leftButtonMethod {
    ...
    [[MKCHCentralManager shared] disconnect];
    [MKCHDeviceMQTTParamsModel sharedDealloc];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0) {
        //WIFI Settings
        MKCHBleWifiSettingsController *vc = [[MKCHBleWifiSettingsController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 1) {
        //MQTT settings
        MKCHServerForDeviceController *vc = [[MKCHServerForDeviceController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 1 && indexPath.row == 0) {
        //Network Settings
        MKCHBleNetworkSettingsController *vc = [[MKCHBleNetworkSettingsController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 1 && indexPath.row == 1) {
        //NTP & Timezone
        MKCHBleNTPTimezoneController *vc = [[MKCHBleNTPTimezoneController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 1 && indexPath.row == 2) {
        //Device Information
        MKCHBleDeviceInfoController *vc = [[MKCHBleDeviceInfoController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
}
...
- (void)receiveDeviceOnline:(NSNotification *)note {
    ...
    if (!ValidDict(user) || !ValidStr(user[@"macAddress"]) || ![[MKCHDeviceMQTTParamsModel shared].deviceModel.macAddress isEqualToString:user[@"macAddress"]]) {
        return;
    }
...
- (void)connectButtonPressed {
    ...
    if (![MKCHDeviceMQTTParamsModel shared].wifiConfig || ![MKCHDeviceMQTTParamsModel shared].mqttConfig) {
...
- (void)startMqttProcess {
    [[NSNotificationCenter defaultCenter] removeObserver:self
                                                    name:mk_ch_peripheralConnectStateChangedNotification
                                                  object:nil];
    NSString *topic = [MKCHDeviceMQTTParamsModel shared].deviceModel.publishedTopic;
    [self.progressView show];
    [[MKCHMQTTDataManager shared] subscriptions:@[topic]];
    [[NSNotificationCenter defaultCenter] addObserver:self
...
- (void)connectSuccess {
    ...
    deviceModel.clientID = [MKCHDeviceMQTTParamsModel shared].deviceModel.clientID;
    deviceModel.subscribedTopic = [MKCHDeviceMQTTParamsModel shared].deviceModel.subscribedTopic;
    deviceModel.publishedTopic = [MKCHDeviceMQTTParamsModel shared].deviceModel.publishedTopic;
    deviceModel.macAddress = [MKCHDeviceMQTTParamsModel shared].deviceModel.macAddress;
    deviceModel.deviceName = [MKCHDeviceMQTTParamsModel shared].deviceModel.deviceName;
    
        
    MKCHConnectSuccessController *vc = [[MKCHConnectSuccessController alloc] init];
...
- (UIView *)footerView {
    if (!_footerView) {
        ...
        retainButton.hidden = ![MKCHDeviceMQTTParamsModel shared].cloud;
...
- (MKCHDeviceParamListModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCHDeviceParamListModel alloc] init];
    }
    return _dataModel;
}
