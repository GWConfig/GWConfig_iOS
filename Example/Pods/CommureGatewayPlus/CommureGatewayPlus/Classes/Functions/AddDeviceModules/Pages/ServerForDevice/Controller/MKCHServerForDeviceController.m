//
//  MKCHServerForDeviceController.m
...
@interface MKCHServerForDeviceController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKCHServerConfigDeviceFooterViewDelegate,
...
@property (nonatomic, strong)MKCHServerForDeviceModel *dataModel;

@property (nonatomic, strong)MKCHServerConfigDeviceFooterView *sslParamsView;

@property (nonatomic, strong)MKCHServerConfigDeviceFooterViewModel *sslParamsModel;

@property (nonatomic, strong)UIView *footerView;

@end

@implementation MKCHServerForDeviceController

- (void)dealloc {
    NSLog(@"MKCHServerForDeviceController销毁");
}
...
#pragma mark - MKCHServerConfigDeviceFooterViewDelegate
/// 用户改变了开关状态
/// @param isOn isOn
/// @param statusID 0:cleanSession   1:ssl
- (void)ch_mqtt_serverForDevice_switchStatusChanged:(BOOL)isOn statusID:(NSInteger)statusID {
    if (statusID == 0) {
        //cleanSession
...
- (void)ch_mqtt_serverForDevice_textFieldValueChanged:(NSString *)text textID:(NSInteger)textID {
    if (textID == 0) {
        //keepAlive
...
- (void)ch_mqtt_serverForDevice_qosChanged:(NSInteger)qos qosID:(NSInteger)qosID{
    if (qosID == 0) {
        //qos
...
- (void)ch_mqtt_serverForDevice_certificateChanged:(NSInteger)certificate {
...
- (void)ch_mqtt_serverForDevice_fileButtonPressed:(NSInteger)fileType {
    if (fileType == 0) {
        //caFile
...
- (void)ch_selectedServerParams:(NSString *)fileName {
    [MKCHExcelDataManager parseDeviceExcel:fileName
                                   sucBlock:^(NSDictionary * _Nonnull returnData) {
        MKCHServerForDeviceModel *model = [MKCHServerForDeviceModel mk_modelWithJSON:returnData];
...
- (void)saveDataToDevice {
    ...
    [self.dataModel configDataWithSucBlock:^{
        ...
        [MKCHDeviceMQTTParamsModel shared].mqttConfig = YES;
        [MKCHDeviceMQTTParamsModel shared].deviceModel.clientID = self.dataModel.clientID;
        [MKCHDeviceMQTTParamsModel shared].deviceModel.deviceName = self.dataModel.deviceName;
        [MKCHDeviceMQTTParamsModel shared].deviceModel.subscribedTopic = self.dataModel.subscribeTopic;
        [MKCHDeviceMQTTParamsModel shared].deviceModel.publishedTopic = self.dataModel.publishTopic;
        [MKCHDeviceMQTTParamsModel shared].deviceModel.macAddress = self.dataModel.macAddress;
    } failedBlock:^(NSError * _Nonnull error) {
        @strongify(self);
        [[MKHudManager share] hide];
        [self.view showCentralToast:error.userInfo[@"errorInfo"]];
        [MKCHDeviceMQTTParamsModel shared].deviceModel.clientID = @"";
        [MKCHDeviceMQTTParamsModel shared].deviceModel.deviceName = @"";
        [MKCHDeviceMQTTParamsModel shared].deviceModel.subscribedTopic = @"";
        [MKCHDeviceMQTTParamsModel shared].deviceModel.publishedTopic = @"";
        [MKCHDeviceMQTTParamsModel shared].deviceModel.macAddress = @"";
    }];
}
...
//    }
    ...
    [MKCHExcelDataManager exportDeviceExcel:self.dataModel
                                    sucBlock:^{
        [[MKHudManager share] hide];
        [self sharedExcel];
    }
...
- (void)clearAllParams {
    MKCHServerForDeviceModel *emptyModel = [[MKCHServerForDeviceModel alloc] init];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"CommureGatewayPlus", @"MKCHServerForDeviceController", @"ch_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCHServerForDeviceModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCHServerForDeviceModel alloc] init];
    }
    return _dataModel;
}
...
- (MKCHServerConfigDeviceFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKCHServerConfigDeviceFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 380.f)];
...
- (MKCHServerConfigDeviceFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKCHServerConfigDeviceFooterViewModel alloc] init];
    }
