//
//  MKCHBleNetworkSettingsController.m
...
@interface MKCHBleNetworkSettingsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCHBleNetworkSettingsModel *dataModel;

@end

@implementation MKCHBleNetworkSettingsController

- (void)dealloc {
    NSLog(@"MKCHBleNetworkSettingsController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"CommureGatewayPlus", @"MKCHBleNetworkSettingsController", @"ch_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCHBleNetworkSettingsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCHBleNetworkSettingsModel alloc] init];
    }
    return _dataModel;
}
