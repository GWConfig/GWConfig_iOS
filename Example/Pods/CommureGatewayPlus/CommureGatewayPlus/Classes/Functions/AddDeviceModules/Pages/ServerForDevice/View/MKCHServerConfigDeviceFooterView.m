//
//  MKCHServerConfigDeviceFooterView.m
...
@property (nonatomic, strong)MKCHMQTTSSLForDeviceView *sslView;

@property (nonatomic, assign)NSInteger index;

@end

@implementation MKCHServerConfigDeviceFooterView
...
- (void)mk_mqtt_generalParams_cleanSessionStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(ch_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate ch_mqtt_serverForDevice_switchStatusChanged:isOn statusID:0];
    }
}

- (void)mk_mqtt_generalParams_qosChanged:(NSInteger)qos {
    if ([self.delegate respondsToSelector:@selector(ch_mqtt_serverForDevice_qosChanged:qosID:)]) {
        [self.delegate ch_mqtt_serverForDevice_qosChanged:qos qosID:0];
    }
}

- (void)mk_mqtt_generalParams_KeepAliveChanged:(NSString *)keepAlive {
    if ([self.delegate respondsToSelector:@selector(ch_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate ch_mqtt_serverForDevice_textFieldValueChanged:keepAlive textID:0];
    }
}
...
- (void)mk_mqtt_userCredentials_userNameChanged:(NSString *)userName {
    if ([self.delegate respondsToSelector:@selector(ch_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate ch_mqtt_serverForDevice_textFieldValueChanged:userName textID:1];
    }
}

- (void)mk_mqtt_userCredentials_passwordChanged:(NSString *)password {
    if ([self.delegate respondsToSelector:@selector(ch_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate ch_mqtt_serverForDevice_textFieldValueChanged:password textID:2];
    }
}

#pragma mark - MKCHMQTTSSLForDeviceViewDelegate
- (void)ch_mqtt_sslParams_device_sslStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(ch_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate ch_mqtt_serverForDevice_switchStatusChanged:isOn statusID:1];
    }
}

/// 用户选择了加密方式
/// @param certificate 0:CA certificate     1:Self signed certificates
- (void)ch_mqtt_sslParams_device_certificateChanged:(NSInteger)certificate {
    if ([self.delegate respondsToSelector:@selector(ch_mqtt_serverForDevice_certificateChanged:)]) {
        [self.delegate ch_mqtt_serverForDevice_certificateChanged:certificate];
    }
}

/// 用户点击选择了caFaile按钮
- (void)ch_mqtt_sslParams_device_caFilePressed {
    if ([self.delegate respondsToSelector:@selector(ch_mqtt_serverForDevice_fileButtonPressed:)]) {
        [self.delegate ch_mqtt_serverForDevice_fileButtonPressed:0];
    }
}

/// 用户点击选择了Client Key按钮
- (void)ch_mqtt_sslParams_device_clientKeyPressed {
    if ([self.delegate respondsToSelector:@selector(ch_mqtt_serverForDevice_fileButtonPressed:)]) {
        [self.delegate ch_mqtt_serverForDevice_fileButtonPressed:1];
    }
}

/// 用户点击了Client Cert File按钮
- (void)ch_mqtt_sslParams_device_clientCertPressed {
    if ([self.delegate respondsToSelector:@selector(ch_mqtt_serverForDevice_fileButtonPressed:)]) {
        [self.delegate ch_mqtt_serverForDevice_fileButtonPressed:2];
    }
}
...
#pragma mark - setter
- (void)setDataModel:(MKCHServerConfigDeviceFooterViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCHServerConfigDeviceFooterViewModel.class]) {
        return;
    }
    ...
    MKCHMQTTSSLForDeviceViewModel *sslModel = [[MKCHMQTTSSLForDeviceViewModel alloc] init];
...
- (MKCHMQTTSSLForDeviceView *)sslView {
    if (!_sslView) {
        _sslView = [[MKCHMQTTSSLForDeviceView alloc] init];
