//
//  MKCHBleWifiSettingsController.m
...
#include "MKCHBleWifiSettingsCertCell.h"
#include "MKCHBleWifiSettingsBandCell.h"
...
@interface MKCHBleWifiSettingsController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextButtonCellDelegate,
MKTextFieldCellDelegate,
mk_textSwitchCellDelegate,
MKCHBleWifiSettingsCertCellDelegate,
MKCAFileSelectControllerDelegate,
MKCHBleWifiSettingsBandCellDelegate>
...
@property (nonatomic, strong)MKCHBleWifiSettingsModel *dataModel;

@end

@implementation MKCHBleWifiSettingsController

- (void)dealloc {
    NSLog(@"MKCHBleWifiSettingsController销毁");
}
...
#pragma mark - MKCHBleWifiSettingsCertCellDelegate
...
- (void)mk_certSelectedMethod:(mk_certListPageType)certType certName:(NSString *)certName {
    if (certType == mk_caCertSelPage) {
        ...
        MKCHBleWifiSettingsCertCellModel *cellModel = self.section8List[0];
    ...
    if (certType == mk_clientCertSelPage) {
        ...
        MKCHBleWifiSettingsCertCellModel *cellModel = self.section9List[0];
        cellModel.fileName = certName;
    ...
    if (certType == mk_clientKeySelPage) {
        ...
        MKCHBleWifiSettingsCertCellModel *cellModel = self.section10List[0];
        cellModel.fileName = certName;
...
#pragma mark - MKCHBleWifiSettingsBandCellDelegate
- (void)ch_bleWifiSettingsBandCell_countryChanged:(NSInteger)country {
    MKCHBleWifiSettingsBandCellModel *cellModel = self.section11List[0];
...
- (void)saveDataToDevice {
    ...
    [self.dataModel configDataWithSucBlock:^{
        ...
        [MKCHDeviceMQTTParamsModel shared].wifiConfig = YES;
    } failedBlock:^(NSError * _Nonnull error) {
        @strongify(self);
        [[MKHudManager share] hide];
        [self.view showCentralToast:error.userInfo[@"errorInfo"]];
        [MKCHDeviceMQTTParamsModel shared].wifiConfig = NO;
    }];
}
...
- (UITableViewCell *)cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 8) {
        //CA certificate.
        MKCHBleWifiSettingsCertCell *cell = [MKCHBleWifiSettingsCertCell initCellWithTableView:self.tableView];
    ...
    //Country&Band
    MKCHBleWifiSettingsBandCell *cell = [MKCHBleWifiSettingsBandCell initCellWithTableView:self.tableView];
...
- (void)loadSection8Datas {
    MKCHBleWifiSettingsCertCellModel *cellModel = [[MKCHBleWifiSettingsCertCellModel alloc] init];
    cellModel.index = 0;
...
- (void)loadSection11Datas {
    MKCHBleWifiSettingsBandCellModel *cellModel = [[MKCHBleWifiSettingsBandCellModel alloc] init];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"CommureGatewayPlus", @"MKCHBleWifiSettingsController", @"ch_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCHBleWifiSettingsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCHBleWifiSettingsModel alloc] init];
    }
    return _dataModel;
}
