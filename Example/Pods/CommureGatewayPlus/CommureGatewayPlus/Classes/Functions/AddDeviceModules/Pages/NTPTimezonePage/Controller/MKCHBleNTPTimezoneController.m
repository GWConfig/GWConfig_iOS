//
//  MKCHBleNTPTimezoneController.m
...
@interface MKCHBleNTPTimezoneController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCHBleNTPTimezoneModel *dataModel;

@property (nonatomic, strong)NSMutableArray *timezoneList;

@end

@implementation MKCHBleNTPTimezoneController

- (void)dealloc {
    NSLog(@"MKCHBleNTPTimezoneController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"CommureGatewayPlus", @"MKCHBleNTPTimezoneController", @"ch_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCHBleNTPTimezoneModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCHBleNTPTimezoneModel alloc] init];
    }
    return _dataModel;
}
