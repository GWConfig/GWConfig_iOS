...
@interface MKCHServerForAppController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKCHServerConfigAppFooterViewDelegate,
...
@property (nonatomic, strong)MKCHServerForAppModel *dataModel;

@property (nonatomic, strong)MKCHServerConfigAppFooterView *sslParamsView;

@property (nonatomic, strong)MKCHServerConfigAppFooterViewModel *sslParamsModel;
...
@implementation MKCHServerForAppController

- (void)dealloc {
    NSLog(@"MKCHServerForAppController销毁");
...
- (void)rightButtonMethod {
    ...
    [[MKCHMQTTDataManager shared] saveServerParams:self.dataModel];
    ...
    [[MKCHMQTTDataManager shared] connect];
}
...
#pragma mark - MKCHServerConfigAppFooterViewDelegate
/// 用户改变了开关状态
/// @param isOn isOn
/// @param statusID 0:cleanSession   1:ssl
- (void)ch_mqtt_serverForApp_switchStatusChanged:(BOOL)isOn statusID:(NSInteger)statusID {
    if (statusID == 0) {
        //cleanSession
...
- (void)ch_mqtt_serverForApp_qosChanged:(NSInteger)qos {
...
- (void)ch_mqtt_serverForApp_textFieldValueChanged:(NSString *)text textID:(NSInteger)textID {
    if (textID == 0) {
        //keep alive
...
- (void)ch_mqtt_serverForApp_certificateChanged:(NSInteger)certificate {
...
- (void)ch_mqtt_serverForApp_fileButtonPressed:(NSInteger)fileType {
    if (fileType == 0) {
        //caFaile
...
- (void)ch_selectedServerParams:(NSString *)fileName {
    [MKCHExcelDataManager parseAppExcel:fileName
                                sucBlock:^(NSDictionary * _Nonnull returnData) {
        MKCHServerForAppModel *model = [MKCHServerForAppModel mk_modelWithJSON:returnData];
...
- (void)serverManagerStateChanged {
    ...
    if ([MKCHMQTTDataManager shared].state == MKCHMQTTSessionManagerStateConnected) {
        ...
        [[NSNotificationCenter defaultCenter] removeObserver:self
                                                        name:MKCHMQTTSessionManagerStateChangedNotification
                                                      object:nil];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_ch_needReloadTopicsNotification"
                                                            object:nil];
...
//    }
    ...
    [MKCHExcelDataManager exportAppExcel:self.dataModel
                                 sucBlock:^{
        [[MKHudManager share] hide];
        [self sharedExcel];
    }
...
- (void)connectFailed {
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self
                                                    name:MKCHMQTTSessionManagerStateChangedNotification
                                                  object:nil];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"MKCHMQTTServerConnectFailedNotification" object:nil];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"CommureGatewayPlus", @"MKCHServerForAppController", @"ch_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCHServerForAppModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCHServerForAppModel alloc] init];
    }
    return _dataModel;
}
...
- (MKCHServerConfigAppFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKCHServerConfigAppFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 230.f)];
...
- (MKCHServerConfigAppFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKCHServerConfigAppFooterViewModel alloc] init];
    }
