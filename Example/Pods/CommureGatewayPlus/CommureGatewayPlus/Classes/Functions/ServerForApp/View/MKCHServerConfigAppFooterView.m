//
//  MKCHServerConfigAppFooterView.m
...
@implementation MKCHServerConfigAppFooterViewModel
@end
...
static CGFloat const defaultViewHeight = 220.f;

@interface MKCHServerConfigAppFooterView ()<UIScrollViewDelegate,
MKMQTTGeneralParamsViewDelegate,
MKMQTTUserCredentialsViewDelegate,
MKCHMQTTSSLForAppViewDelegate>
...
@property (nonatomic, strong)MKCHMQTTSSLForAppView *sslView;
...
@implementation MKCHServerConfigAppFooterView
...
- (void)mk_mqtt_generalParams_cleanSessionStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(ch_mqtt_serverForApp_switchStatusChanged:statusID:)]) {
        [self.delegate ch_mqtt_serverForApp_switchStatusChanged:isOn statusID:0];
    }
}

- (void)mk_mqtt_generalParams_qosChanged:(NSInteger)qos {
    if ([self.delegate respondsToSelector:@selector(ch_mqtt_serverForApp_qosChanged:)]) {
        [self.delegate ch_mqtt_serverForApp_qosChanged:qos];
    }
}

- (void)mk_mqtt_generalParams_KeepAliveChanged:(NSString *)keepAlive {
    if ([self.delegate respondsToSelector:@selector(ch_mqtt_serverForApp_textFieldValueChanged:textID:)]) {
        [self.delegate ch_mqtt_serverForApp_textFieldValueChanged:keepAlive textID:0];
    }
}
...
- (void)mk_mqtt_userCredentials_userNameChanged:(NSString *)userName {
    if ([self.delegate respondsToSelector:@selector(ch_mqtt_serverForApp_textFieldValueChanged:textID:)]) {
        [self.delegate ch_mqtt_serverForApp_textFieldValueChanged:userName textID:1];
    }
}

- (void)mk_mqtt_userCredentials_passwordChanged:(NSString *)password {
    if ([self.delegate respondsToSelector:@selector(ch_mqtt_serverForApp_textFieldValueChanged:textID:)]) {
        [self.delegate ch_mqtt_serverForApp_textFieldValueChanged:password textID:2];
    }
}

#pragma mark - MKCHMQTTSSLForAppViewDelegate
- (void)ch_mqtt_sslParams_app_sslStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(ch_mqtt_serverForApp_switchStatusChanged:statusID:)]) {
        [self.delegate ch_mqtt_serverForApp_switchStatusChanged:isOn statusID:1];
    }
}

/// 用户选择了加密方式
/// @param certificate 0:CA certificate     1:Self signed certificates
- (void)ch_mqtt_sslParams_app_certificateChanged:(NSInteger)certificate {
    if ([self.delegate respondsToSelector:@selector(ch_mqtt_serverForApp_certificateChanged:)]) {
        [self.delegate ch_mqtt_serverForApp_certificateChanged:certificate];
    }
}

/// 用户点击选择了caFaile按钮
- (void)ch_mqtt_sslParams_app_caFilePressed {
    if ([self.delegate respondsToSelector:@selector(ch_mqtt_serverForApp_fileButtonPressed:)]) {
        [self.delegate ch_mqtt_serverForApp_fileButtonPressed:0];
    }
}

/// 用户点击选择了P12证书按钮
- (void)ch_mqtt_sslParams_app_clientFilePressed {
    if ([self.delegate respondsToSelector:@selector(ch_mqtt_serverForApp_fileButtonPressed:)]) {
        [self.delegate ch_mqtt_serverForApp_fileButtonPressed:1];
    }
}
...
#pragma mark - setter
- (void)setDataModel:(MKCHServerConfigAppFooterViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCHServerConfigAppFooterViewModel.class]) {
        return;
    }
    ...
    MKCHMQTTSSLForAppViewModel *sslModel = [[MKCHMQTTSSLForAppViewModel alloc] init];
...
- (MKCHMQTTSSLForAppView *)sslView {
    if (!_sslView) {
        _sslView = [[MKCHMQTTSSLForAppView alloc] init];
