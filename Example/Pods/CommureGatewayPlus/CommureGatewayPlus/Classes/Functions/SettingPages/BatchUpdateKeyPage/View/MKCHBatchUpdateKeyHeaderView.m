//
//  MKCHBatchUpdateKeyHeaderView.m
...
@interface MKCHBatchUpdateKeyHeaderView ()
...
@implementation MKCHBatchUpdateKeyHeaderView
...
- (MKTextField *)keyTextField {
    if (!_keyTextField) {
        ...
        _keyTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(ch_encryptionKeyChanged:)]) {
                [self.delegate ch_encryptionKeyChanged:text];
            }
        };
...
- (UIButton *)excelButton {
    if (!_excelButton) {
        ...
        [_excelButton setImage:LOADICON(@"CommureGatewayPlus", @"MKCHBatchUpdateKeyHeaderView", @"ch_batchOtaListSelectIcon.png") forState:UIControlStateNormal];
...
- (UIButton *)scanButton {
    if (!_scanButton) {
        ...
        [_scanButton setImage:LOADICON(@"CommureGatewayPlus", @"MKCHBatchUpdateKeyHeaderView", @"ch_batchOtaQRCodeIcon.png") forState:UIControlStateNormal];
