//
//  MKCHBatchUpdateKeyController.m
...
@interface MKCHBatchUpdateKeyController ()<UITableViewDelegate,
UITableViewDataSource,
MKCHImportServerControllerDelegate,
MKCHBatchUpdateKeyHeaderViewDelegate,
...
@property (nonatomic, strong)MKCHBatchUpdateKeyModel *dataModel;

@property (nonatomic, strong)MKCHBatchUpdateKeyHeaderView *tableHeaderView;
...
@implementation MKCHBatchUpdateKeyController

- (void)dealloc {
    NSLog(@"MKCHBatchUpdateKeyController销毁");
...
#pragma mark - MKCHBatchUpdateKeyHeaderViewDelegate
...
- (void)addNotes {
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveUpdateKeyResult:)
                                                 name:MKCHReceiveBxpButtonUpdateKeyResultNotification
                                               object:nil];
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(receiveBatchUpdateKeyResult:)
                                                     name:MKCHReceiveBxpButtonBatchUpdateKeyResultNotification
                                                   object:nil];
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"CommureGatewayPlus", @"MKCHBatchUpdateKeyController", @"ch_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCHBatchUpdateKeyModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCHBatchUpdateKeyModel alloc] init];
    }
    return _dataModel;
}

- (MKCHBatchUpdateKeyHeaderView *)tableHeaderView {
    if (!_tableHeaderView) {
        _tableHeaderView = [[MKCHBatchUpdateKeyHeaderView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 160.f)];
