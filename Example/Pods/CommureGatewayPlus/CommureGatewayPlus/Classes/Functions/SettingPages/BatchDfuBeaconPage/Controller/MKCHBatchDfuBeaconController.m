//
//  MKCHBatchDfuBeaconController.m
...
@interface MKCHBatchDfuBeaconController ()<UITableViewDelegate,
UITableViewDataSource,
MKCHImportServerControllerDelegate,
MKCHBatchDfuBeaconHeaderViewDelegate,
...
@property (nonatomic, strong)MKCHBatchDfuBeaconModel *dataModel;

@property (nonatomic, strong)MKCHBatchDfuBeaconHeaderView *tableHeaderView;
...
@implementation MKCHBatchDfuBeaconController

- (void)dealloc {
    NSLog(@"MKCHBatchDfuBeaconController销毁");
...
- (void)ch_selectedServerParams:(NSString *)fileName {
    ...
    [MKCHExcelDataManager parseBeaconExcel:fileName sucBlock:^(NSArray<NSDictionary *> * _Nonnull beaconInfoList) {
        ...
        for (NSInteger i = 0; i < number; i ++) {
            ...
            cellModel.status = mk_ch_batchUpdateStatus_normal;
...
#pragma mark - MKCHBatchDfuBeaconHeaderViewDelegate
...
- (void)receiveBeaconDfuResult:(NSNotification *)note {
    ...
    MKCHBatchUpdateCellModel *cellModel = self.dataList[[indexNumber integerValue]];
    if (result == 0) {
        cellModel.status = mk_ch_batchUpdateStatus_upgrading;
    }else if (result == 1) {
        cellModel.status = mk_ch_batchUpdateStatus_success;
    }
    ...
    if (result == 1 && !ValidArray(failureList)) {
        ...
        for (NSInteger i = 0; i < failureList.count; i ++) {
            ...
            if (ValidNum(indexNumber) && [indexNumber integerValue] < self.dataList.count) {
                ...
                MKCHBatchUpdateCellModel *cellModel = self.dataList[[indexNumber integerValue]];
                cellModel.status = (reason == 3 ? mk_ch_batchUpdateStatus_failed : mk_ch_batchUpdateStatus_timeout);
            }
        }
    }
    [self.tableView reloadData];
...
- (void)addNotes {
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveBeaconDfuResult:)
                                                 name:MKCHReceiveBxpButtonDfuResultNotification
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveBeaconBatchDfuResult:)
                                                 name:MKCHReceiveBxpButtonBatchDfuResultNotification
                                               object:nil];
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"CommureGatewayPlus", @"MKCHBatchDfuBeaconController", @"ch_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCHBatchDfuBeaconModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCHBatchDfuBeaconModel alloc] init];
    }
    return _dataModel;
}

- (MKCHBatchDfuBeaconHeaderView *)tableHeaderView {
    if (!_tableHeaderView) {
        _tableHeaderView = [[MKCHBatchDfuBeaconHeaderView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 190.f)];
