//
//  MKCHBatchDfuBeaconHeaderView.m
...
@interface MKCHBatchDfuBeaconHeaderView ()
...
@implementation MKCHBatchDfuBeaconHeaderView
...
- (void)excelButtonPressed {
    if ([self.delegate respondsToSelector:@selector(ch_beaconListButtonPressed)]) {
        [self.delegate ch_beaconListButtonPressed];
    }
}
...
//        _firmwareTextField.text = defaultFirmwareUrl;
        ...
        _firmwareTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(ch_firmwareUrlChanged:)]) {
                [self.delegate ch_firmwareUrlChanged:text];
            }
        };
...
//        _dataTextField.text = defaultInitDataUrl;
        ...
        _dataTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(ch_dataFileUrlChanged:)]) {
                [self.delegate ch_dataFileUrlChanged:text];
            }
        };
...
- (MKTextField *)passwordTextField {
    if (!_passwordTextField) {
        ...
        _passwordTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(ch_dataPasswordChanged:)]) {
                [self.delegate ch_dataPasswordChanged:text];
            }
        };
...
- (UIButton *)excelButton {
    if (!_excelButton) {
        ...
        [_excelButton setImage:LOADICON(@"CommureGatewayPlus", @"MKCHBatchDfuBeaconHeaderView", @"ch_batchOtaListSelectIcon.png") forState:UIControlStateNormal];
...
- (UIButton *)scanButton {
    if (!_scanButton) {
        ...
        [_scanButton setImage:LOADICON(@"CommureGatewayPlus", @"MKCHBatchDfuBeaconHeaderView", @"ch_batchOtaQRCodeIcon.png") forState:UIControlStateNormal];
