//
//  MKCHMqttServerConfigFooterView.m
...
@implementation MKCHMqttServerConfigFooterViewModel
@end
...
@interface MKCHMqttServerConfigFooterView ()<UIScrollViewDelegate,
MKMQTTGeneralParamsViewDelegate,
MKMQTTUserCredentialsViewDelegate,
MKCHMqttServerSSLViewDelegate>
...
@property (nonatomic, strong)MKCHMqttServerSSLView *sslView;

@property (nonatomic, assign)NSInteger index;

@end

@implementation MKCHMqttServerConfigFooterView
...
#pragma mark - MKCHMqttServerSSLViewDelegate
- (void)ch_mqtt_sslParams_modifyDevice_sslStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(ch_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
...
- (void)ch_mqtt_sslParams_modifyDevice_certificateChanged:(NSInteger)certificate {
...
/// @param value value
- (void)ch_mqtt_sslParams_modifyDevice_textFieldValueChanged:(NSInteger)index value:(NSString *)value {
    ...
    if ([self.delegate respondsToSelector:@selector(ch_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate ch_mqtt_serverForDevice_textFieldValueChanged:value textID:tempIndex];
    }
}
...
#pragma mark - setter
- (void)setDataModel:(MKCHMqttServerConfigFooterViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCHMqttServerConfigFooterViewModel.class]) {
        return;
    }
    ...
    MKCHMqttServerSSLViewModel *sslModel = [[MKCHMqttServerSSLViewModel alloc] init];
...
- (MKCHMqttServerSSLView *)sslView {
    if (!_sslView) {
        _sslView = [[MKCHMqttServerSSLView alloc] init];
