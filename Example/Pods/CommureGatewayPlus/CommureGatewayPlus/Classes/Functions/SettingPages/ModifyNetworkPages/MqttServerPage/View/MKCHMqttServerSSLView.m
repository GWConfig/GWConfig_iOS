...
@implementation MKCHMqttServerSSLViewModel
@end

@interface MKCHMqttServerSSLView ()<MKCHMqttServerSSLTextFieldDelegate>
...
@property (nonatomic, strong)MKCHMqttServerSSLTextField *caFileView;

@property (nonatomic, strong)MKCHMqttServerSSLTextField *clientKeyView;

@property (nonatomic, strong)MKCHMqttServerSSLTextField *clientCertView;

@end

@implementation MKCHMqttServerSSLView
...
#pragma mark - MKCHMqttServerSSLTextFieldDelegate
/// textField内容发送改变时的回调事件
/// @param index 当前cell所在的index
/// @param value 当前textField的值
- (void)ch_modifyServerSSLTextFieldValueChanged:(NSInteger)index textValue:(NSString *)value {
    if ([self.delegate respondsToSelector:@selector(ch_mqtt_sslParams_modifyDevice_textFieldValueChanged:value:)]) {
        [self.delegate ch_mqtt_sslParams_modifyDevice_textFieldValueChanged:index value:value];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(ch_mqtt_sslParams_modifyDevice_sslStatusChanged:)]) {
        [self.delegate ch_mqtt_sslParams_modifyDevice_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(ch_mqtt_sslParams_modifyDevice_certificateChanged:)]) {
            [self.delegate ch_mqtt_sslParams_modifyDevice_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKCHMqttServerSSLViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCHMqttServerSSLViewModel.class]) {
        return;
    }
    ...
    MKCHMqttServerSSLTextFieldModel *caModel = [[MKCHMqttServerSSLTextFieldModel alloc] init];
    caModel.index = 0;
    ...
    MKCHMqttServerSSLTextFieldModel *clientKeyModel = [[MKCHMqttServerSSLTextFieldModel alloc] init];
    ...
    MKCHMqttServerSSLTextFieldModel *clientModel = [[MKCHMqttServerSSLTextFieldModel alloc] init];
    clientModel.index = 2;
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"CommureGatewayPlus", @"MKCHMqttServerSSLView", @"ch_switchSelectedIcon.png") : LOADICON(@"CommureGatewayPlus", @"MKCHMqttServerSSLView", @"ch_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"CommureGatewayPlus", @"MKCHMqttServerSSLView", @"ch_switchUnselectedIcon.png") forState:UIControlStateNormal];
...
- (MKCHMqttServerSSLTextField *)caFileView {
    if (!_caFileView) {
        _caFileView = [[MKCHMqttServerSSLTextField alloc] init];
        _caFileView.delegate = self;
    }
    return _caFileView;
}

- (MKCHMqttServerSSLTextField *)clientKeyView {
    if (!_clientKeyView) {
        _clientKeyView = [[MKCHMqttServerSSLTextField alloc] init];
        _clientKeyView.delegate = self;
    }
    return _clientKeyView;
}

- (MKCHMqttServerSSLTextField *)clientCertView {
    if (!_clientCertView) {
        _clientCertView = [[MKCHMqttServerSSLTextField alloc] init];
