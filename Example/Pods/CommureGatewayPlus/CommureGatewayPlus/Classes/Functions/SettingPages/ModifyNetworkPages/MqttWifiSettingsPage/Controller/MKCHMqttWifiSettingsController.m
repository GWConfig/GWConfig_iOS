//
//  MKCHMqttWifiSettingsController.m
...
@interface MKCHMqttWifiSettingsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCHMqttWifiSettingsModel *dataModel;

@end

@implementation MKCHMqttWifiSettingsController

- (void)dealloc {
    NSLog(@"MKCHMqttWifiSettingsController销毁");
}
            ...
            if (((self.dataModel.eapType == 0 || self.dataModel.eapType == 1) && self.dataModel.verifyServer) || self.dataModel.eapType == 2) {
                ...
                [[NSNotificationCenter defaultCenter] addObserver:self
                                                         selector:@selector(receiveUpdateEAPCerts:)
                                                             name:MKCHReceiveDeviceUpdateEapCertsResultNotification
                                                           object:nil];
                return;
            }
        }
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"CommureGatewayPlus", @"MKCHMqttWifiSettingsController", @"ch_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCHMqttWifiSettingsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCHMqttWifiSettingsModel alloc] init];
    }
    return _dataModel;
}
