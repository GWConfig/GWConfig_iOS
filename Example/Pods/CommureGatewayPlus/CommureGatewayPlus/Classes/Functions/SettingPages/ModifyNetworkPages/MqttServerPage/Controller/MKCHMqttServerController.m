...
@interface MKCHMqttServerController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKCHMqttServerConfigFooterViewDelegate,
...
@property (nonatomic, strong)MKCHMqttServerModel *dataModel;

@property (nonatomic, strong)MKCHMqttServerConfigFooterView *sslParamsView;

@property (nonatomic, strong)MKCHMqttServerConfigFooterViewModel *sslParamsModel;

@property (nonatomic, strong)UIView *footerView;

@end

@implementation MKCHMqttServerController

- (void)dealloc {
    NSLog(@"MKCHMqttServerController销毁");
}
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveUpdateCerts:)
                                                 name:MKCHReceiveDeviceUpdateMqttCertsResultNotification
                                               object:nil];
}
...
#pragma mark - MKCHMqttServerConfigFooterViewDelegate
/// 用户改变了开关状态
/// @param isOn isOn
...
- (void)ch_mqtt_serverForDevice_bottomButtonPressed:(NSInteger)index {
    if (index == 0) {
        //Export Demo File
...
- (void)ch_selectedServerParams:(NSString *)fileName {
    [MKCHExcelDataManager parseDeviceExcel:fileName
        ...
        MKCHMqttServerModel *model = [MKCHMqttServerModel mk_modelWithJSON:returnData];
...
- (void)clearAllParams {
    MKCHMqttServerModel *model = [[MKCHMqttServerModel alloc] init];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"CommureGatewayPlus", @"MKCHMqttServerController", @"ch_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCHMqttServerModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCHMqttServerModel alloc] init];
    }
    return _dataModel;
}
...
- (MKCHMqttServerConfigFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKCHMqttServerConfigFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 380.f)];
...
- (MKCHMqttServerConfigFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKCHMqttServerConfigFooterViewModel alloc] init];
    }
