...
@implementation MKCHSettingModel
...
- (BOOL)readAdvStatus {
    ...
    [MKCHMQTTInterface ch_readBleAdvStatusWithMacAddress:[MKCHDeviceModeManager shared].macAddress topic:[MKCHDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configAdvStatus {
    __block BOOL success = NO;
    [MKCHMQTTInterface ch_configBleAdvStatus:self.advIsOn advTime:[self.advTime integerValue] macAddress:[MKCHDeviceModeManager shared].macAddress topic:[MKCHDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readPowerSwitchStatus {
    __block BOOL success = NO;
    [MKCHMQTTInterface ch_readPowerSwitchStatusWithMacAddress:[MKCHDeviceModeManager shared].macAddress topic:[MKCHDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
