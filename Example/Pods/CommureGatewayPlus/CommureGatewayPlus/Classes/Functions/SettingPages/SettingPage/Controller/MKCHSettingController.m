...
@interface MKCHSettingController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCHSettingModel *dataModel;

@property (nonatomic, copy)NSString *localNameAsciiStr;

@end

@implementation MKCHSettingController

- (void)dealloc {
    NSLog(@"MKCHSettingController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0) {
        ...
        MKCHNetworkStatusController *vc = [[MKCHNetworkStatusController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 1) {
        //Communicate timeout
        MKCHCommunicateController *vc = [[MKCHCommunicateController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 2) {
        //Data report timout
        MKCHDataReportController *vc = [[MKCHDataReportController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 3) {
        //Connect Beacon timeout
        MKCHConnectBeaconTimeoutController *vc = [[MKCHConnectBeaconTimeoutController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 4) {
        //System time
        MKCHSystemTimeController *vc = [[MKCHSystemTimeController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 5) {
        //Reset device by button
        MKCHResetByButtonController *vc = [[MKCHResetByButtonController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    ...
    if (indexPath.section == 2 && indexPath.row == 0) {
        //OTA
        MKCHOTAController *vc = [[MKCHOTAController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 1) {
        //Modify network settings
        MKCHMqttConfigSelectController *vc = [[MKCHMqttConfigSelectController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    ...
    if (indexPath.section == 3 && indexPath.row == 0) {
        //Batch DFU Beacon
        MKCHBatchDfuBeaconController *vc = [[MKCHBatchDfuBeaconController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 3 && indexPath.row == 1) {
        //Batch update Beacon key
        MKCHBatchUpdateKeyController *vc = [[MKCHBatchUpdateKeyController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    ...
    if (indexPath.section == 4 && indexPath.row == 0) {
        //Device information
        MKCHDeviceInfoController *vc = [[MKCHDeviceInfoController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
}
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    if (section == 1) {
        return ([[MKCHDeviceModeManager shared].deviceType isEqualToString:@"10"] ? 1 : self.section1List.count);
    }
    if (section == 2) {
...
- (void)rootButtonPressed {
    ...
    [alertView showAlertWithTitle:@"Reboot Device" message:msg notificationName:@"mk_ch_needDismissAlert"];
}
...
- (void)resetButtonPressed {
    ...
    [alertView addAction:cancelAction];
    [alertView addAction:confirmAction];
    [alertView showAlertWithTitle:@"Reset Device" message:msg notificationName:@"mk_ch_needDismissAlert"];
}
...
- (void)configLocalName{
    ...
    self.localNameAsciiStr = SafeStr([MKCHDeviceModeManager shared].deviceName);
    MKAlertViewTextField *textField = [[MKAlertViewTextField alloc] initWithTextValue:SafeStr([MKCHDeviceModeManager shared].deviceName)
    ...
    [alertView showAlertWithTitle:@"Edit Local Name" message:msg notificationName:@"mk_ch_needDismissAlert"];
}
...
- (void)saveDeviceLocalName {
    ...
    [MKCHDeviceDatabaseManager updateLocalName:self.localNameAsciiStr
                                    macAddress:[MKCHDeviceModeManager shared].macAddress
                                      sucBlock:^{
        [[MKHudManager share] hide];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_ch_deviceNameChangedNotification"
                                                            object:nil
                                                          userInfo:@{
                                                              @"macAddress":[MKCHDeviceModeManager shared].macAddress,
...
- (void)resetDevice {
    ...
    [MKCHMQTTInterface ch_resetDeviceWithMacAddress:[MKCHDeviceModeManager shared].macAddress topic:[MKCHDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        [self removeDevice];
    } failedBlock:^(NSError * _Nonnull error) {
        [[MKHudManager share] hide];
...
- (void)removeDevice {
    ...
    [MKCHDeviceDatabaseManager deleteDeviceWithMacAddress:[MKCHDeviceModeManager shared].macAddress sucBlock:^{
        [[MKHudManager share] hide];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_ch_deleteDeviceNotification"
                                                            object:nil
                                                          userInfo:@{@"macAddress":[MKCHDeviceModeManager shared].macAddress}];
...
- (void)rebootDevice {
    [[MKHudManager share] showHUDWithTitle:@"Waiting..." inView:self.view isPenetration:NO];
    [MKCHMQTTInterface ch_rebootDeviceWithMacAddress:[MKCHDeviceModeManager shared].macAddress topic:[MKCHDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
...
- (void)showAdvTimeAlert {
    ...
    [alertView addAction:confirmAction];
    [alertView addTextField:textField];
    [alertView showAlertWithTitle:@"Advertising time" message:@"" notificationName:@"mk_ch_needDismissAlert"];
}
...
- (void)configPowerSwitchStatus:(BOOL)isOn {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKCHMQTTInterface ch_configPowerSwitchStatus:isOn macAddress:[MKCHDeviceModeManager shared].macAddress topic:[MKCHDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"CommureGatewayPlus", @"MKCHSettingController", @"ch_editIcon.png") forState:UIControlStateNormal];
...
- (MKCHSettingModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCHSettingModel alloc] init];
    }
    return _dataModel;
}
