...
@implementation MKCHDataReportModel
...
- (BOOL)readDataReportTimeout {
    ...
    [MKCHMQTTInterface ch_readDataReportTimeoutWithMacAddress:[MKCHDeviceModeManager shared].macAddress topic:[MKCHDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configDataReportTimeout {
    __block BOOL success = NO;
    [MKCHMQTTInterface ch_configDataReportTimeout:[self.timeout integerValue] macAddress:[MKCHDeviceModeManager shared].macAddress topic:[MKCHDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
