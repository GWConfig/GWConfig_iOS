//
//  MKCHConnectBeaconTimeoutController.m
...
@interface MKCHConnectBeaconTimeoutController ()

@property (nonatomic, strong)MKTextField *textField;

@property (nonatomic, strong)UILabel *noteLabel;

@property (nonatomic, strong)MKCHConnectBeaconTimeoutModel *dataModel;

@end

@implementation MKCHConnectBeaconTimeoutController

- (void)dealloc {
    NSLog(@"MKCHConnectBeaconTimeoutController销毁");
}
...
#pragma mark - UI
- (void)loadSubViews {
    self.defaultTitle = @"Connect Beacon timeout";
    [self.rightButton setImage:LOADICON(@"CommureGatewayPlus", @"MKCHConnectBeaconTimeoutController", @"ch_saveIcon.png") forState:UIControlStateNormal];
...
- (MKTextField *)textField {
    if (!_textField) {
        ...
        _textField.placeholder = @"1-200, unit: second";
...
- (UILabel *)noteLabel {
    if (!_noteLabel) {
        ...
        _noteLabel.text = @"If the gateway can not connect to Beacon device in the timeout, it will give up the connection.";
        _noteLabel.numberOfLines = 0;
    }
    return _noteLabel;
}

- (MKCHConnectBeaconTimeoutModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCHConnectBeaconTimeoutModel alloc] init];
    }
    return _dataModel;
}
