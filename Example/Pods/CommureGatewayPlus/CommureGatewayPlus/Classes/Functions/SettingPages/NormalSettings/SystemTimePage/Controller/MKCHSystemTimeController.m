...
@interface MKCHSystemTimeController ()<UITableViewDelegate,
UITableViewDataSource,
MKCHSystemTimeCellDelegate,
...
@implementation MKCHSystemTimeController

- (void)dealloc {
    NSLog(@"MKCHSystemTimeController销毁");
    if (self.readTimer) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0)  {
        //Sync Time From NTP
        MKCHNTPServerController *vc = [[MKCHNTPServerController alloc] init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKCHSystemTimeCell *cell = [MKCHSystemTimeCell initCellWithTableView:tableView];
...
#pragma mark - MKCHSystemTimeCellDelegate
...
- (void)readDataFromServer {
    ...
    [MKCHMQTTInterface ch_readDeviceUTCTimeWithMacAddress:[MKCHDeviceModeManager shared].macAddress topic:[MKCHDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)syncTimeZoneToDevice:(NSInteger)timeZone {
    ...
    [MKCHMQTTInterface ch_configDeviceTimeZone:timeZone - 24 timestamp:[[NSDate date] timeIntervalSince1970] macAddress:[MKCHDeviceModeManager shared].macAddress topic:[MKCHDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)loadSection1Datas {
    MKCHSystemTimeCellModel *cellModel = [[MKCHSystemTimeCellModel alloc] init];
    cellModel.index = 0;
