...
@interface MKCHCommunicateModel ()
...
@implementation MKCHCommunicateModel
...
- (BOOL)readCommunicateTimeout {
    ...
    [MKCHMQTTInterface ch_readCommunicateTimeoutWithMacAddress:[MKCHDeviceModeManager shared].macAddress topic:[MKCHDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configCommunicateTimeout {
    __block BOOL success = NO;
    [MKCHMQTTInterface ch_configCommunicationTimeout:[self.timeout integerValue] macAddress:[MKCHDeviceModeManager shared].macAddress topic:[MKCHDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
