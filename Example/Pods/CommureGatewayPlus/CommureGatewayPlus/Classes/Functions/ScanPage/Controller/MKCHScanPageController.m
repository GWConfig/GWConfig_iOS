...
static NSString *const localPasswordKey = @"mk_ch_passwordKey";

static NSTimeInterval const kRefreshInterval = 0.5f;

@interface MKCHScanPageController ()<UITableViewDelegate,
UITableViewDataSource,
mk_ch_centralManagerScanDelegate>
...
@implementation MKCHScanPageController

- (void)dealloc {
    NSLog(@"MKCHScanPageController销毁");
    //移除runloop的监听
    CFRunLoopRemoveObserver(CFRunLoopGetCurrent(), self.observerRef, kCFRunLoopCommonModes);
    [[MKCHCentralManager shared] stopScan];
    [MKCHCentralManager sharedDealloc];
}
...
- (void)viewDidLoad {
    ...
    [MKCHCentralManager shared].delegate = self;
}

#pragma mark - super method
- (void)rightButtonMethod {
    if ([MKCHCentralManager shared].centralStatus != mk_ch_centralManagerStatusEnable) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    MKCHScanPageCell *cell = [MKCHScanPageCell initCellWithTableView:tableView];
    cell.dataModel = self.dataList[indexPath.row];
    return cell;
}

#pragma mark - mk_ch_centralManagerScanDelegate

- (void)mk_ch_receiveDevice:(NSDictionary *)deviceModel {
    MKCHScanPageModel *dataModel = [MKCHScanPageModel mk_modelWithJSON:deviceModel];
...
- (void)addConfiguredDevice {
    MKCHConfiguredGatewayController *vc = [[MKCHConfiguredGatewayController alloc] init];
...
- (void)scanTimerRun{
    ...
    [[MKCHCentralManager shared] startScan];
...
- (void)updateDataWithDeviceModel:(MKCHScanPageModel *)deviceModel{
...
#pragma mark - 连接部分
- (void)connectDeviceWithModel:(MKCHScanPageModel *)deviceModel {
    //停止扫描
    [self.refreshIcon.layer removeAnimationForKey:@"mk_refreshAnimationKey"];
    [[MKCHCentralManager shared] stopScan];
    if (self.scanTimer) {
...
- (void)connectDevice:(MKCHScanPageModel *)deviecModel {
    ...
    [[MKCHCentralManager shared] connectPeripheral:deviecModel.peripheral password:password sucBlock:^(CBPeripheral * _Nonnull peripheral) {
...
- (void)pushMQTTForDevicePage:(NSString *)deviceType {
    ...
    //MK110 Plus 03
    MKCHBleMqttConfigSelectController *vc = [[MKCHBleMqttConfigSelectController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}
...
- (UIImageView *)refreshIcon {
    if (!_refreshIcon) {
        ...
        _refreshIcon.image = LOADICON(@"CommureGatewayPlus", @"MKCHScanPageController", @"ch_scanRefresh.png");
    }
    return _refreshIcon;
}
