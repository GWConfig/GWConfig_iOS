...
@interface MKCHAccelerometerController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCHAccelerometerModel *dataModel;

@end

@implementation MKCHAccelerometerController

- (void)dealloc {
    NSLog(@"MKCHAccelerometerController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"CommureGatewayPlus", @"MKCHAccelerometerController", @"ch_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCHAccelerometerModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCHAccelerometerModel alloc] init];
