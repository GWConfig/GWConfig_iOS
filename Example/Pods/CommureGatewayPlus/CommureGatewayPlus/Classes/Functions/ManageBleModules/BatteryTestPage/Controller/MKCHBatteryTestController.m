...
@interface MKCHBatteryTestController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCHBatteryTestModel *dataModel;

@end

@implementation MKCHBatteryTestController

- (void)dealloc {
    NSLog(@"MKCHBatteryTestController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"CommureGatewayPlus", @"MKCHBatteryTestController", @"ch_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCHBatteryTestModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCHBatteryTestModel alloc] init];
