//
//  MKCHSelfTestTriggerController.m
...
@interface MKCHSelfTestTriggerController ()

@property (nonatomic, strong)MKTextField *textField;

@end

@implementation MKCHSelfTestTriggerController

- (void)dealloc {
    NSLog(@"MKCHSelfTestTriggerController销毁");
}
...
- (void)readDataFromDevice {
    ...
    [MKCHMQTTInterface ch_readSelfTestTriggeredByButtonWithBleMacAddress:self.bleMac macAddress:[MKCHDeviceModeManager shared].macAddress topic:[MKCHDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)configDatas {
    ...
    [MKCHMQTTInterface ch_configSelfTestTriggeredByButton:value bleMac:self.bleMac macAddress:[MKCHDeviceModeManager shared].macAddress topic:[MKCHDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
            [self.view showCentralToast:@"Read Failed"];
            return;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"CommureGatewayPlus", @"MKCHSelfTestTriggerController", @"ch_saveIcon.png") forState:UIControlStateNormal];
