...
@implementation MKCHManageBleDevicesCellModel
@end

@interface MKCHManageBleDevicesCell ()
...
@implementation MKCHManageBleDevicesCell

+ (MKCHManageBleDevicesCell *)initCellWithTableView:(UITableView *)tableView {
    MKCHManageBleDevicesCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCHManageBleDevicesCellIdenty"];
    if (!cell) {
        cell = [[MKCHManageBleDevicesCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCHManageBleDevicesCellIdenty"];
    }
    return cell;
}
...
- (void)connectButtonPressed {
    if ([self.delegate respondsToSelector:@selector(ch_manageBleDevicesCellConnectButtonPressed:typeCode:)]) {
        [self.delegate ch_manageBleDevicesCellConnectButtonPressed:SafeStr(self.dataModel.macAddress)
                                                          typeCode:self.dataModel.typeCode];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKCHManageBleDevicesCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCHManageBleDevicesCellModel.class]) {
        return;
    }
