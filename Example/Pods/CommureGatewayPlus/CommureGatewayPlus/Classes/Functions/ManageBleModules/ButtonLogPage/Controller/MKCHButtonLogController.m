...
@interface MKCHButtonLogController ()<MFMailComposeViewControllerDelegate,
MKCHButtonLogHeaderDelegate>

@property (nonatomic, strong)MKCHButtonLogHeader *headerView;
...
@implementation MKCHButtonLogController

- (void)dealloc {
    NSLog(@"MKCHButtonLogController销毁");
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveButtonLog:)
                                                 name:MKCHReceiveButtonLogNotification
                                               object:nil];
}
...
#pragma mark - MKCHButtonLogHeaderDelegate
- (void)ch_syncButtonPressed {
    self.textView.text = @"";
    [[MKHudManager share] showHUDWithTitle:@"Reading..." inView:self.view isPenetration:NO];
    [MKCHMQTTInterface ch_readButtonLogWithBleMacAddress:self.bleMac macAddress:[MKCHDeviceModeManager shared].macAddress topic:[MKCHDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)ch_deleteButtonPressed {
    ...
    [alertView showAlertWithTitle:@"Warning!" message:msg notificationName:@"mk_ch_needDismissAlert"];
}
...
- (void)sendClearCommand {
    [[MKHudManager share] showHUDWithTitle:@"Reading..." inView:self.view isPenetration:NO];
    [MKCHMQTTInterface ch_clearButtonLogWithBleMacAddress:self.bleMac macAddress:[MKCHDeviceModeManager shared].macAddress topic:[MKCHDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        self.textView.text = @"";
        [[MKHudManager share] hide];
        [self.view showCentralToast:@"Setup succeed!"];
    } failedBlock:^(NSError * _Nonnull error) {
...
#pragma mark - getter
- (MKCHButtonLogHeader *)headerView {
    if (!_headerView) {
        _headerView = [[MKCHButtonLogHeader alloc] init];
