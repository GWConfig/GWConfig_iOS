//
//  MKCHAdvParamsConfigController.m
...
@interface MKCHAdvParamsConfigController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCHAdvParamsConfigModel *dataModel;

@end

@implementation MKCHAdvParamsConfigController

- (void)dealloc {
    NSLog(@"MKCHAdvParamsConfigController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"CommureGatewayPlus", @"MKCHAdvParamsConfigController", @"ch_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCHAdvParamsConfigModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCHAdvParamsConfigModel alloc] init];
