...
@implementation MKCHAboutController

- (void)dealloc {
    NSLog(@"MKCHAboutController销毁");
}
...
- (void)feedButtonPressed {
    MKCHAppLogController *vc = [[MKCHAppLogController alloc] init];
...
- (UIImageView *)aboutIcon {
    if (!_aboutIcon) {
        ...
        _aboutIcon.image = LOADICON(@"CommureGatewayPlus", @"MKCHAboutController", @"ch_deviceList_centerIcon.png");
    }
    return _aboutIcon;
}
...
- (UILabel *)versionLabel {
    if (!_versionLabel) {
        ...
        _versionLabel.text = @"Version: V2.0.6";
    }
