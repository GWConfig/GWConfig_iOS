...
@interface MKCHFilterByMacController ()<UITableViewDelegate,
UITableViewDataSource,
MKCHFilterEditSectionHeaderViewDelegate,
...
@property (nonatomic, strong)MKCHFilterByMacModel *dataModel;

@end

@implementation MKCHFilterByMacController

- (void)dealloc {
    NSLog(@"MKCHFilterByAdvNameController销毁");
}
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    if (section == 1) {
        MKCHFilterEditSectionHeaderViewModel *headerModel = [[MKCHFilterEditSectionHeaderViewModel alloc] init];
        headerModel.index = 0;
        headerModel.msg = @"Edit Mac Address";
        MKCHFilterEditSectionHeaderView *headerView = [MKCHFilterEditSectionHeaderView initHeaderViewWithTableView:tableView];
...
#pragma mark - MKCHFilterEditSectionHeaderViewDelegate
/// 加号点击事件
/// @param index 所在index
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"CommureGatewayPlus", @"MKCHFilterByMacController", @"ch_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCHFilterByMacModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCHFilterByMacModel alloc] init];
    }
    return _dataModel;
}
