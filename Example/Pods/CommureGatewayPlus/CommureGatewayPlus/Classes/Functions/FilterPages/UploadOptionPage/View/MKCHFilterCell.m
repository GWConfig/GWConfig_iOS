...
@implementation MKCHFilterCellModel
@end

@interface MKCHFilterCell ()
...
@implementation MKCHFilterCell

+ (MKCHFilterCell *)initCellWithTableView:(UITableView *)tableView {
    MKCHFilterCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCHFilterCellIdenty"];
    if (!cell) {
        cell = [[MKCHFilterCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCHFilterCellIdenty"];
    }
    return cell;
}
...
- (void)rightButtonPressed {
    ...
    [pickView showPickViewWithDataList:self.dataModel.dataList selectedRow:row block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(ch_filterValueChanged:index:)]) {
            [self.delegate ch_filterValueChanged:currentRow index:self.dataModel.index];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKCHFilterCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCHFilterCellModel.class]) {
        return;
    }
