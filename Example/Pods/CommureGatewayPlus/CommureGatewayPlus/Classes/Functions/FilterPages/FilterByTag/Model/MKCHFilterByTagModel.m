...
@interface MKCHFilterByTagModel ()
...
@implementation MKCHFilterByTagModel
...
- (BOOL)readFilterByTag {
    ...
    [MKCHMQTTInterface ch_readFilterBXPTagWithMacAddress:[MKCHDeviceModeManager shared].macAddress topic:[MKCHDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterByTag:(NSArray <NSString *>*)tagList {
    __block BOOL success = NO;
    [MKCHMQTTInterface ch_configFilterByTag:self.precise reverseFilter:self.reverse tagIDList:tagList macAddress:[MKCHDeviceModeManager shared].macAddress topic:[MKCHDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
