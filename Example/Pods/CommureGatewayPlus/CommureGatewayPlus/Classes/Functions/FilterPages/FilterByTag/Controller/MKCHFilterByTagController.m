...
@interface MKCHFilterByTagController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCHFilterByTagModel *dataModel;

@end

@implementation MKCHFilterByTagController

- (void)dealloc {
    NSLog(@"MKCHFilterByTagController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"CommureGatewayPlus", @"MKCHFilterByTagController", @"ch_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCHFilterByTagModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCHFilterByTagModel alloc] init];
    }
    return _dataModel;
}
