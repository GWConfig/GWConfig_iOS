...
static MKCHDeviceModeManager *manager = nil;
static dispatch_once_t onceToken;

@interface MKCHDeviceModeManager ()

@property (nonatomic, strong)id <MKCHDeviceModeManagerDataProtocol>protocol;

@end

@implementation MKCHDeviceModeManager

+ (MKCHDeviceModeManager *)shared {
    dispatch_once(&onceToken, ^{
        if (!manager) {
            manager = [MKCHDeviceModeManager new];
        }
    });
    return manager;
}
...
- (void)addDeviceModel:(id <MKCHDeviceModeManagerDataProtocol>)protocol {
    self.protocol = nil;
