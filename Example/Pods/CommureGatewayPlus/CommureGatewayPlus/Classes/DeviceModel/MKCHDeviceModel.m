...
NSString *const MKCHDeviceModelOfflineNotification = @"MKCHDeviceModelOfflineNotification";

@interface MKCHDeviceModel ()
...
@implementation MKCHDeviceModel

- (void)dealloc{
    NSLog(@"MKCHDeviceModel销毁");
}

#pragma mark - public method

- (void)startStateMonitoringTimer{
    if (self.onLineState == MKCHDeviceModelStateOffline) {
        return;
    }
    ...
    dispatch_source_set_event_handler(self.receiveTimer, ^{
        ...
        if (self.receiveTimerCount >= 62.f) {
            ...
            self.receiveTimerCount = 0;
            self.onLineState = MKCHDeviceModelStateOffline;
            dispatch_async(dispatch_get_main_queue(), ^{
                [[NSNotificationCenter defaultCenter] postNotificationName:MKCHDeviceModelOfflineNotification
                                                                    object:nil
                                                                  userInfo:@{@"macAddress":self.macAddress}];
                if ([self.delegate respondsToSelector:@selector(ch_deviceOfflineWithMacAddress:)]) {
                    [self.delegate ch_deviceOfflineWithMacAddress:self.macAddress];
                }
            });
            return ;
        }
