...
@implementation MKCHFilterBeaconCellModel
@end

@interface MKCHFilterBeaconCell ()
...
@implementation MKCHFilterBeaconCell

+ (MKCHFilterBeaconCell *)initCellWithTableView:(UITableView *)tableView {
    MKCHFilterBeaconCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCHFilterBeaconCellIdenty"];
    if (!cell) {
        cell = [[MKCHFilterBeaconCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCHFilterBeaconCellIdenty"];
    }
    return cell;
}
...
#pragma mark - setter
- (void)setDataModel:(MKCHFilterBeaconCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCHFilterBeaconCellModel.class]) {
        return;
    }
...
- (MKTextField *)minTextField {
    if (!_minTextField) {
        ...
        _minTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(mk_ch_beaconMinValueChanged:index:)]) {
                [self.delegate mk_ch_beaconMinValueChanged:text index:self.dataModel.index];
            }
        };
    }
...
- (MKTextField *)maxTextField {
    if (!_maxTextField) {
        ...
        _maxTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(mk_ch_beaconMaxValueChanged:index:)]) {
                [self.delegate mk_ch_beaconMaxValueChanged:text index:self.dataModel.index];
            }
        };
    }
