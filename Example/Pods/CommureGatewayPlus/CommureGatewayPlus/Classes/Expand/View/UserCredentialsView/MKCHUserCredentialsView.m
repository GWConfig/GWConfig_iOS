...
@implementation MKCHUserCredentialsViewModel
@end

@interface MKCHUserCredentialsView ()
...
@implementation MKCHUserCredentialsView
...
#pragma mark - setter
- (void)setDataModel:(MKCHUserCredentialsViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCHUserCredentialsViewModel.class]) {
        return;
    }
...
- (MKTextField *)userNameTextField {
    if (!_userNameTextField) {
        ...
        _userNameTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(ch_mqtt_userCredentials_userNameChanged:)]) {
                [self.delegate ch_mqtt_userCredentials_userNameChanged:text];
            }
        };
...
- (MKTextField *)passwordTextField {
    if (!_passwordTextField) {
        ...
        _passwordTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(ch_mqtt_userCredentials_passwordChanged:)]) {
                [self.delegate ch_mqtt_userCredentials_passwordChanged:text];
            }
        };
